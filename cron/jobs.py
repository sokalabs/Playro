...
# Safe cron job ids: hex from create_job() or validated gateway API input.
_JOB_ID_RE = re.compile(r"^[a-f0-9]{12}$")


def _safe_job_id(job_id: str) -> Optional[str]:
    """Return job_id when it is a safe single path segment under OUTPUT_DIR."""
...
def _normalize_script_path(script: Optional[Any]) -> Optional[str]:
    """Normalize and validate the persisted cron script path.

    Cron scripts are intentionally constrained to relative paths under
    HERMES_HOME/scripts.  This keeps direct Python callers aligned with the
    tool boundary and prevents persisted jobs from bypassing scheduler
    containment checks with absolute, home-relative, or traversal paths.
    """
    if script is None:
        return None
    ...
    if not text:
        return None
    path = Path(text)
    if path.is_absolute() or text.startswith("~") or (len(text) >= 2 and text[1] == ":"):
        raise ValueError("cron script path must be relative to HERMES_HOME/scripts")
    if ".." in path.parts:
        raise ValueError("cron script path must not contain traversal components")
    return text
...
def create_job(
    ...
    Args:
        ...
        base_url: Deprecated and ignored. Cron jobs must use configured provider endpoints.
        script: Optional path to a script whose stdout feeds the job. With
                ...
                change-detection pattern). Must be a relative path under
                ~/.hermes/scripts/; absolute, home-relative, and traversal
                paths are rejected. ``.sh`` / ``.bash`` files run via bash,
    ...
    # Per-job base_url overrides are intentionally ignored. Cron jobs can be
    # created by model/tool-call paths, so endpoint selection must come from
    # trusted provider configuration rather than persisted job data.
...
def update_job(job_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    for i, job in enumerate(jobs):
        ...
        # Validate / normalize script if present in updates. Empty string or
        # None both mean "clear the field".
        if "script" in updates:
            updates["script"] = _normalize_script_path(updates["script"])

        # Per-job base_url overrides are not executable and should not remain in
        # newly updated records. Keep accepting the key for compatibility, but
        # normalize any submitted value to None.
...
def remove_job(job_id: str) -> bool:
    ...
    if not safe_id:
        logger.warning("Refusing to remove cron job with unsafe id: %r", job_id)
        return False
    jobs = load_jobs()
    ...
    if len(jobs) < original_len:
        ...
        job_output_dir = (OUTPUT_DIR / safe_id).resolve()
        try:
            job_output_dir.relative_to(OUTPUT_DIR.resolve())
        except ValueError:
            logger.warning("Refusing unsafe cron output path for job %s", safe_id)
            return True
