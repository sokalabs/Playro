...
def _origin_scope(origin: Optional[Dict[str, Any]]) -> Optional[tuple]:
    """Return the cron security scope for origin/delivery-boundary checks."""
...
def _same_origin_scope(left: Optional[Dict[str, Any]], right: Optional[Dict[str, Any]]) -> bool:
    """True when two cron jobs share the same origin/delivery boundary."""
    return _origin_scope(left) == _origin_scope(right)
...
def _resolve_single_delivery_target(job: dict, deliver_value: str) -> Optional[dict]:
    ...
    if deliver_value == "origin":
        ...
        # Origin missing — do not fan out to arbitrary platform home channels.
        logger.warning(
            "Job '%s' has deliver=origin but no origin metadata; skipping delivery",
...
# Cron-delivered MEDIA tags are produced by non-interactive agent output, so
# only attach files that look like Hermes-generated media/cache artifacts.
_CRON_MEDIA_MAX_BYTES = 25 * 1024 * 1024
_CRON_MEDIA_DOCUMENT_EXTS = frozenset({
    ".pdf", ".md", ".txt", ".csv", ".log", ".json", ".xml", ".yaml", ".yml",
    ".toml", ".ini", ".cfg", ".zip", ".docx", ".xlsx", ".pptx",
})


def _cron_media_allowed_roots() -> tuple[Path, ...]:
    """Return local cache roots from which cron MEDIA files may be attached.

    Cron responses are model-controlled text.  Treating every ``MEDIA:path`` as
    uploadable lets prompt-injected jobs exfiltrate arbitrary readable files.
    Restrict native attachment delivery to Hermes cache directories used by
    generated/downloaded media artifacts, including legacy cache names.
    """
    home = _get_hermes_home().expanduser()
    candidates = (
        home / "cache" / "audio",
        home / "cache" / "images",
        home / "cache" / "videos",
...
def _validate_cron_media_path(media_path: str) -> Path | None:
    """Return a safe resolved cron media path, or ``None`` if rejected."""
    try:
        candidate = Path(media_path).expanduser()
        resolved = candidate.resolve(strict=True)
        ext = resolved.suffix.lower()
        allowed_exts = _IMAGE_EXTS | _VIDEO_EXTS | _CRON_MEDIA_DOCUMENT_EXTS
        # Import lazily to avoid gateway imports at scheduler module load.
        from gateway.platforms.base import _AUDIO_EXTS
        allowed_exts = allowed_exts | _AUDIO_EXTS

        if ext not in allowed_exts:
            return None
        if not resolved.is_file():
            return None
        if resolved.stat().st_size > _CRON_MEDIA_MAX_BYTES:
            return None
        if not any(resolved.is_relative_to(root) for root in _cron_media_allowed_roots()):
            return None
        return resolved
...
def _filter_cron_media_files(media_files: list, job: dict) -> list:
    """Drop unsafe cron MEDIA paths before native platform upload."""
    safe_media = []
    for media_path, is_voice in media_files:
        safe_path = _validate_cron_media_path(str(media_path))
        if safe_path is None:
            logger.warning(
                "Job '%s': blocked unsafe MEDIA attachment path %s",
                job.get("id", "?"), media_path,
            )
            continue
        safe_media.append((str(safe_path), is_voice))
    return safe_media
...
def _cron_media_safe_roots() -> tuple[Path, ...]:
    """Return directories cron may dereference from MEDIA: tags.

    Cron output is untrusted model/tool text.  Only files under Hermes' media
    cache locations may be opened and sent as attachments; arbitrary absolute
    paths (for example /etc/passwd or ~/.hermes/.env) must remain inert.
    """
    return (
...
def _is_safe_cron_media_path(media_path: str, safe_roots: tuple[Path, ...] | None = None) -> bool:
    """Return True if *media_path* resolves inside an allowed media cache."""
    if not media_path or "\x00" in media_path:
        return False
    try:
    ...
    roots = safe_roots if safe_roots is not None else _cron_media_safe_roots()
    for root in roots:
        try:
...
def _filter_cron_media_files(media_files: list) -> list:
    """Drop extracted MEDIA paths that would read outside safe cache roots."""
    safe_roots = _cron_media_safe_roots()
    safe_media = []
    for media_path, is_voice in media_files:
        if _is_safe_cron_media_path(media_path, safe_roots):
            safe_media.append((media_path, is_voice))
        else:
            logger.warning("Blocked unsafe cron MEDIA path: %s", media_path)
    return safe_media
...
def _send_media_via_adapter(
    ...
    for media_path, _is_voice in _filter_cron_media_files(media_files, job):
        try:
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    # Extract MEDIA: tags so safe cached attachments are forwarded as files,
    # not raw text.  Cron responses are untrusted model/tool output, so never
    # dereference arbitrary paths embedded in MEDIA: tags.
    from gateway.platforms.base import BasePlatformAdapter
    media_files, cleaned_delivery_content = BasePlatformAdapter.extract_media(delivery_content)
    media_files = _filter_cron_media_files(media_files, job)
    media_files = _filter_cron_media_files(media_files)
...
_MAX_SCRIPT_OUTPUT_CHARS = 12000
_SAFE_SCRIPT_ENV_KEYS = {
    "HOME",
    "HERMES_HOME",
    "LANG",
    "LC_ALL",
...
def _truncate_script_output(text: str) -> str:
    """Limit script output before injecting it into prompts or deliveries."""
    if len(text) <= _MAX_SCRIPT_OUTPUT_CHARS:
        return text
    omitted = len(text) - _MAX_SCRIPT_OUTPUT_CHARS
    return f"{text[:_MAX_SCRIPT_OUTPUT_CHARS]}\n...[truncated {omitted} characters]"


def _safe_script_env() -> dict[str, str]:
    """Return a minimal environment for cron scripts.

    Credential-bearing process variables are intentionally not forwarded to
    scheduled scripts.  This keeps script output/error injection from becoming
    a trivial environment-secret disclosure path.
    """
    env = {key: value for key, value in os.environ.items() if key in _SAFE_SCRIPT_ENV_KEYS}
...
def _validate_script_file_trust(path: Path, scripts_dir: Path) -> Optional[str]:
    """Require script files/directories to be owned by this user and not writable by others."""
    if os.name == "nt":
        return None
    ...
    checked_paths = [scripts_dir]
    try:
        relative_parent = path.parent.relative_to(scripts_dir)
    except ValueError:
        return f"Script path escapes the scripts directory: {path}"
    current = scripts_dir
    ...
    for checked in checked_paths:
        try:
            stat_result = checked.stat()
        except OSError as exc:
            return f"Script path cannot be inspected safely: {exc}"
        if stat_result.st_uid != expected_uid:
            return f"Script path is not owned by the Hermes user: {checked}"
        if stat_result.st_mode & 0o022:
            return f"Script path is writable by group/others: {checked}"
    return None
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must be relative paths within HERMES_HOME/scripts/.  Paths are
    resolved and validated against this directory to prevent arbitrary script
    execution via path traversal, absolute path injection, or symlink escape.
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script, relative to HERMES_HOME/scripts/.
            Absolute and ~-prefixed paths are rejected.
    ...
    raw_text = str(script_path or "").strip()
    if not raw_text:
        return False, "Script path is empty"
    raw = Path(raw_text)
    if raw.is_absolute() or raw_text.startswith("~") or (len(raw_text) >= 2 and raw_text[1] == ":"):
        return False, "Blocked: script path must be relative to the scripts directory"
    if ".." in raw.parts:
        return False, f"Blocked: script path contains traversal: {script_path!r}"

    candidate = scripts_dir_resolved / raw
    if _has_symlink_component(candidate, scripts_dir_resolved):
        return False, f"Blocked: script path uses a symlink: {script_path!r}"

    # Guard against path traversal and symlink escape — scripts MUST reside
    ...
    try:
        ...
        return False, (
            ...
            f"({scripts_dir_resolved}): {script_path!r} ({exc})"
        )
    ...
    trust_error = _validate_script_file_trust(path, scripts_dir_resolved)
    if trust_error:
        return False, f"Blocked: {trust_error}"
    ...
    try:
        ...
        # Redact before truncation so delimiter-based patterns (PEM blocks,
        # quoted JSON secrets) are not broken by an early cutoff.
        try:
        ...
        stdout = _truncate_script_output(stdout)
        stderr = _truncate_script_output(stderr)
        ...
        return False, f"Script execution failed: {type(exc).__name__}"
...
def _build_job_prompt(job: dict, prerun_script: Optional[tuple] = None) -> str:
    ...
    if context_from:
        ...
        for source_job_id in context_from:
            ...
            if not source_job:
                logger.warning("context_from: skipping missing job_id %r", source_job_id)
                continue
            if not _same_origin_scope(job.get("origin"), source_job.get("origin")):
                logger.warning(
                    "context_from: skipping cross-origin reference from job %r to %r",
                    job.get("id"),
                    source_job_id,
                )
                continue
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        try:
            ...
            # Cron jobs may be created or updated by model/tool-call paths.
            # Never honor a persisted per-job base_url here: doing so would let
            # untrusted cronjob tool input redirect future scheduled runs (with
            # API credentials, prompts, tool schemas, and tool outputs) to an
            # attacker-controlled OpenAI-compatible endpoint. Custom endpoints
            # must come from the user's provider configuration instead.
            if job.get("base_url"):
                logger.warning(
                    "Job '%s': ignoring unsupported per-job base_url override",
                    job_id,
                )
