...
def format_session_db_unavailable(prefix: str = "Session database not available") -> str:
    ...
    Callers such as ``session_search`` must pass an explicit ``SessionDB``;
    this module does not auto-open a global database for tool execution.
...
class SessionDB:
    ...
    def session_belongs_to_user(
        self, session_id: str, source: str = None, user_id: str = None
    ) -> bool:
        """Return True if a session or one of its ancestors belongs to a user.

        Gateway compression continuations may inherit lineage via
        ``parent_session_id`` without duplicating all owner columns. Walking the
        parent chain preserves legitimate resumes while preventing unrelated
        users from switching into another user's session.
        """
        if not session_id or source is None or user_id is None:
            return False
        ...
        for _ in range(64):
            ...
            with self._lock:
                row = self._conn.execute(
                    "SELECT id, source, user_id, parent_session_id "
            ...
            if row is None:
                return False
            if (
                str(row["source"] or "") == str(source)
                and str(row["user_id"] or "") == str(user_id)
            ):
                return True
    ...
    def get_session_by_title(
        ...
        Optional ``source`` and ``user_id`` filters let gateway callers scope
        title lookups to sessions owned by the requesting platform user.
        """
        clauses = ["title = ?"]
        params = [title]
    ...
    def resolve_session_by_title(
        ...
        ``source`` and ``user_id`` filters scope gateway lookups to the caller.
        """
    ...
    def resolve_resume_session_id(self, session_id: str) -> str:
        """Redirect a resume target to its validated compression continuation.

        Context compression ends the current session with
        ``end_reason = 'compression'`` and forks a child session linked via
        ``parent_session_id``. Reuse ``get_compression_tip()`` so /resume only
        follows descendants that satisfy the same compression-continuation
        checks as the rest of session listing/resume code instead of walking
        arbitrary child sessions.
        """
        if not session_id:
            return session_id
        try:
