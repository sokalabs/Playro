...
def _hermes_home_trusted_for_root_ops() -> bool:
    """Return False when root must not trust the current HERMES_HOME path."""
...
def _get_pid_path() -> Path:
    ...
    if not _hermes_home_trusted_for_root_ops():
        return Path.home() / ".hermes" / "gateway.pid"
    home = get_hermes_home()
...
def _get_takeover_marker_path() -> Path:
    ...
    if not _hermes_home_trusted_for_root_ops():
        return Path.home() / ".hermes" / _TAKEOVER_MARKER_FILENAME
    home = get_hermes_home()
...
def _get_planned_stop_marker_path() -> Path:
    ...
    if not _hermes_home_trusted_for_root_ops():
        return Path.home() / ".hermes" / _PLANNED_STOP_MARKER_FILENAME
    home = get_hermes_home()
