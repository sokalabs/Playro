...
from gateway.platforms.security import (
    compare_ascii_secrets,
    redact_api_tool_output,
    redact_response_store_history,
    safe_tool_preview_label,
    sanitize_sandbox_task_id,
    sanitize_stream_delta_for_api,
...
class _IdempotencyCache:
    ...
    async def get_or_set(self, key: str, fingerprint: str, compute_coro):
        ...
        # Shield so one disconnecting waiter cannot cancel shared inflight work.
...
def _make_request_fingerprint(
    ...
    *,
    ...
    if session_key:
        subset["_hermes_session_key"] = session_key
    if conversation_history is not None:
        subset["_hermes_conversation_history"] = conversation_history
    normalized_gateway_context = _normalize_gateway_context_for_fingerprint(gateway_context)
    if normalized_gateway_context is not None:
        subset["_hermes_gateway_context"] = normalized_gateway_context
    return sha256(repr(subset).encode("utf-8")).hexdigest()


def _normalize_gateway_context_for_fingerprint(gateway_context: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    raw_toolsets = gateway_context.get("enabled_toolsets")
    if isinstance(raw_toolsets, list):
        normalized["enabled_toolsets"] = sorted(
            {item.strip() for item in raw_toolsets if isinstance(item, str) and item.strip()}
        )
    return normalized or None


def _new_chat_session_id() -> str:
    """Return an unguessable API chat session ID for a new conversation.

    OpenAI-compatible callers that need multi-turn continuity should persist
    and resend the X-Hermes-Session-Id header returned on the first response.
    Requests without that explicit header are treated as new conversations so
    unrelated users cannot collide on predictable prompt-derived IDs or share
    the same tool sandbox.
    """
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._gateway_proxy_key: str = extra.get(
            "gateway_proxy_key",
            os.getenv("API_SERVER_GATEWAY_PROXY_KEY", ""),
        )
    ...
    def _check_auth(self, request: "web.Request") -> Optional["web.Response"]:
        ...
        if auth_header.startswith("Bearer "):
            ...
            if compare_ascii_secrets(token, self._api_key):
                return None  # Auth OK
    ...
    # ------------------------------------------------------------------
    # Gateway proxy and session header helpers
    # ------------------------------------------------------------------

    def _check_gateway_context_trust(
        self, request: "web.Request"
    ) -> Optional["web.Response"]:
        """Require an out-of-band trusted proxy key for body gateway context.

        ``Authorization`` authenticates generic API clients.  Gateway context
        carries an upstream platform authorization decision, so accepting it
        requires a separate secret that ordinary API clients do not need.
        """
        if not self._gateway_proxy_key:
            return web.json_response(
                _openai_error(
                    "hermes_gateway_context requires a trusted gateway proxy key. "
                    "Configure API_SERVER_GATEWAY_PROXY_KEY on the API server "
                    "and send it as X-Hermes-Gateway-Proxy-Key from the gateway."
                ),
                status=403,
            )

        supplied = request.headers.get("X-Hermes-Gateway-Proxy-Key", "").strip()
        if not supplied or not hmac.compare_digest(supplied, self._gateway_proxy_key):
            return web.json_response(
                _openai_error("Invalid gateway proxy key"),
                status=403,
            )

        return None

    def _validate_session_key_value(
        self, raw: str, *, error_label: str = "session key"
    ) -> tuple[Optional[str], Optional["web.Response"]]:
        """Validate a caller-supplied stable session/memory scope."""
        value = raw.strip()
        if not value:
            return None, None

        # Reject control characters that could enable header/log injection on
        # echo paths, regardless of whether the key arrived in a header or in
        # a trusted gateway context body.
        if re.search(r'[\r\n\x00]', value):
            return None, web.json_response(
                {
                    "error": {
                        "message": f"Invalid {error_label}",
        ...
        if len(value) > self._MAX_SESSION_HEADER_LEN:
            return None, web.json_response(
                {
                    "error": {
                        "message": f"{error_label.capitalize()} too long",
                        "type": "invalid_request_error",
                    }
                },
                status=400,
    ...
    def _parse_session_key_header(
        ...
        return self._validate_session_key_value(raw)
    ...
    def _create_agent(
        ...
        gateway_context = gateway_context if isinstance(gateway_context, dict) else None
        proxied_toolsets = None
        proxied_platform = None
        proxied_user_id = None
        proxied_user_name = None
        proxied_chat_id = None
        proxied_chat_name = None
        proxied_chat_type = None
        proxied_thread_id = None
        if gateway_context:
            # A trusted gateway context preserves an upstream platform
            # authorization decision, but the API server still applies its own
            # configured policy as an upper bound.  This prevents a compromised
            # or misconfigured proxy from enabling toolsets that the server
            # owner disabled globally or did not allow for that platform.
            raw_platform = gateway_context.get("platform")
            if isinstance(raw_platform, str) and raw_platform.strip():
                proxied_platform = raw_platform.strip().lower()

            raw_toolsets = gateway_context.get("enabled_toolsets")
            if isinstance(raw_toolsets, list):
                server_allowed_toolsets = _get_platform_tools(
                    user_config, proxied_platform or "api_server"
                )
                forwarded_toolsets = {
                    ts.strip() for ts in raw_toolsets if isinstance(ts, str) and ts.strip()
                }
                # Empty means intentionally no tools.  Invalid or server-denied
                # entries are ignored so malformed proxy context fails closed.
                proxied_toolsets = sorted(forwarded_toolsets & server_allowed_toolsets)
            proxied_user_id = (
                gateway_context.get("user_id")
                if isinstance(gateway_context.get("user_id"), str)
                else None
            )
            proxied_user_name = (
                gateway_context.get("user_name")
                if isinstance(gateway_context.get("user_name"), str)
                else None
            )
            proxied_chat_id = (
                gateway_context.get("chat_id")
                if isinstance(gateway_context.get("chat_id"), str)
                else None
            )
            proxied_chat_name = (
                gateway_context.get("chat_name")
                if isinstance(gateway_context.get("chat_name"), str)
                else None
            )
            proxied_chat_type = (
                gateway_context.get("chat_type")
                if isinstance(gateway_context.get("chat_type"), str)
                else None
            )
            proxied_thread_id = (
                gateway_context.get("thread_id")
                if isinstance(gateway_context.get("thread_id"), str)
                else None
            )

        enabled_toolsets = (
            proxied_toolsets
            if proxied_toolsets is not None
            else sorted(_get_platform_tools(user_config, "api_server"))
        )
        # API-server agents must not search all persisted gateway sessions.
        enabled_toolsets = [
            ts for ts in enabled_toolsets if ts != "session_search"
        ]
        agent_platform = proxied_platform or "api_server"

        max_iterations = int(os.getenv("HERMES_MAX_ITERATIONS", "90"))

        if stream_delta_callback is not None:
            _orig_stream_cb = stream_delta_callback

            def stream_delta_callback(delta):  # type: ignore[no-redef]
                if delta is None:
                    _orig_stream_cb(None)
                    return
                cleaned = sanitize_stream_delta_for_api(delta)
                if cleaned:
                    _orig_stream_cb(cleaned)
        ...
        agent = AIAgent(
            ...
            **runtime_kwargs,
            ...
            user_name=proxied_user_name,
            chat_id=proxied_chat_id,
            chat_name=proxied_chat_name,
            chat_type=proxied_chat_type,
            thread_id=proxied_thread_id,
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        """GET /health/detailed — authenticated operational status snapshot."""
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        gateway_proxy_context = body.get("hermes_gateway_context")
        if gateway_proxy_context is not None and not isinstance(gateway_proxy_context, dict):
            return web.json_response(
                _openai_error("hermes_gateway_context must be an object"),
                status=400,
            )
        if isinstance(gateway_proxy_context, dict):
            trust_err = self._check_gateway_context_trust(request)
            if trust_err is not None:
                return trust_err

            # Copy before normalizing so request-body data is never forwarded
            # to the agent with an unvalidated body-provided session key.
            gateway_proxy_context = dict(gateway_proxy_context)
            raw_session_key = gateway_proxy_context.get("session_key")
            if isinstance(raw_session_key, str):
                context_session_key, context_key_err = self._validate_session_key_value(
                    raw_session_key
                )
                if context_key_err is not None:
                    return context_key_err
                if context_session_key:
                    gateway_proxy_context["session_key"] = context_session_key
                    if not gateway_session_key:
                        gateway_session_key = context_session_key
                else:
                    gateway_proxy_context.pop("session_key", None)
        ...
        if provided_session_id:
            ...
            session_id = validate_api_session_id(provided_session_id)
            ...
            # No explicit session header means this is a fresh conversation.
            # Use an unguessable ID instead of a prompt-derived fingerprint so
            # unrelated API users cannot collide on session/tool sandbox state.
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                ...
                label = safe_tool_preview_label(function_name, function_args)
            ...
            agent_task = asyncio.ensure_future(self._run_agent(
                ...
                agent_ref=agent_ref,
                gateway_session_key=gateway_session_key,
                gateway_context=gateway_proxy_context,
            ))
    ...
    async def _write_sse_responses(
        ...
        async def _abort_stream_on_write_failure() -> None:
            nonlocal _stream_failed, _batch_timer
            if _stream_failed:
                return
            _stream_failed = True
            if _batch_timer is not None and not _batch_timer.done():
                _batch_timer.cancel()
                _batch_timer = None
            agent = agent_ref[0] if agent_ref else None
            if agent is not None:
        ...
        def _persist_response_snapshot(
            ...
            *,
            ...
            conversation_history_snapshot = redact_response_store_history(
        ...
        try:
            ...
            async def _emit_text_delta(delta_text: str) -> None:
                ...
                except _STREAM_WRITE_ERRORS:
                    await _abort_stream_on_write_failure()
                    raise
            ...
            async def _emit_tool_completed(payload: Dict[str, Any]) -> None:
                ...
                result_str = redact_api_tool_output(
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        session_id = validate_api_session_id(stored_session_id or "") or str(uuid.uuid4())
        ...
        if store:
            self._response_store.put(response_id, {
                ...
                "conversation_history": redact_response_store_history(full_history),
                "instructions": instructions,
                "session_id": session_id,
            })
        ...
        for msg in messages:
            ...
            if role == "assistant" and msg.get("tool_calls"):
                ...
                items.append({
                    ...
                    "output": redact_api_tool_output(msg.get("content", "")),
                })
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            if agent_ref is not None:
                agent_ref[0] = agent
            safe_session_id = (
                validate_api_session_id(session_id) if session_id else None
            )
            effective_task_id = sanitize_sandbox_task_id(
                safe_session_id or str(uuid.uuid4())
            )
            session_tokens = []
            approval_token = None
            effective_platform = "api_server"
            effective_session_key = gateway_session_key
            if isinstance(gateway_context, dict):
                raw_platform = gateway_context.get("platform")
                if isinstance(raw_platform, str) and raw_platform.strip():
                    effective_platform = raw_platform.strip().lower()
                raw_session_key = gateway_context.get("session_key")
                if not effective_session_key and isinstance(raw_session_key, str):
                    validated_key, _validation_err = self._validate_session_key_value(
                        raw_session_key
                    )
                    if validated_key:
                        effective_session_key = validated_key
            try:
                if gateway_context:
                    from gateway.session_context import clear_session_vars, set_session_vars
                    from tools.approval import reset_current_session_key, set_current_session_key

                    approval_token = set_current_session_key(effective_session_key or effective_task_id)
                    session_tokens = set_session_vars(
                        platform=effective_platform,
                        session_key=effective_session_key or effective_task_id,
                    )
        ...
        def _callback(event_type: str, tool_name: str = None, preview: str = None, args=None, **kwargs):
            ...
            if event_type == "tool.started":
                _push({
                    ...
                    "timestamp": ts,
                    "tool": tool_name,
                    "preview": safe_tool_preview_label(tool_name or "", args or {}),
                })
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        raw_session = body.get("session_id") or stored_session_id or run_id
        session_id = validate_api_session_id(str(raw_session or "").strip()) or run_id
        approval_session_key = f"api-run:{run_id}"
        ephemeral_system_prompt = instructions
        loop = asyncio.get_running_loop()
