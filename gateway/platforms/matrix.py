...
_MATRIX_LOCAL_FILE_MAX_BYTES = int(
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Mention/thread gating — environment variables override config.extra.
        from gateway.platforms.security import env_bool, env_csv_set
    ...
    async def send_exec_approval(
        ...
        requester_user_id = str((metadata or {}).get("mention_user_id") or "").strip() or None
    ...
    async def _send_local_file(
        ...
        try:
            ...
            return SendResult(success=False, error=f"file is not accessible: {exc}")

        if not p.is_file():
            return SendResult(success=False, error=f"not a regular file: {file_path}")

        try:
            size = p.stat().st_size
        except OSError as exc:
            return SendResult(success=False, error=f"file is not accessible: {exc}")
        if size > _MATRIX_LOCAL_FILE_MAX_BYTES:
            return SendResult(
                success=False,
                error=(
                    f"file is too large for Matrix upload: "
                    f"{size} bytes > {_MATRIX_LOCAL_FILE_MAX_BYTES} bytes"
                ),
            )
        ...
        try:
            ...
            return SendResult(success=False, error=f"file read failed: {exc}")
        ...
        if content:
            ...
            if prompt and not prompt.resolved:
                if room_id != prompt.chat_id:
                    return
                if not self._is_approval_reaction_authorized(sender, prompt):
                    logger.info(
    ...
    def _is_approval_reaction_authorized(
        self, sender: str, prompt: "_MatrixApprovalPrompt"
    ) -> bool:
        """Return whether a Matrix reaction may resolve an approval prompt.

        Reaction approval must not be more permissive than the normal gateway
        authorization path.  Prefer the original prompt requester when the
        gateway provided it in metadata (covers paired users), otherwise fall
        back to the configured Matrix/global allowlists and allow-all flags.
        """
        ...
        allowed_ids: Set[str] = set(self._allowed_user_ids)
        global_allowed_users = os.getenv("GATEWAY_ALLOWED_USERS", "").strip()
        if global_allowed_users:
            allowed_ids.update(
                u.strip() for u in global_allowed_users.split(",") if u.strip()
            )
        ...
        check_ids = {sender}
        if "@" in sender:
            check_ids.add(sender.split("@", 1)[0])
    ...
    def _text_batch_key(self, event: MessageEvent) -> str:
        """Sender-scoped key for text message batching.

        Matrix threaded group sessions intentionally share the underlying
        conversation/session key by default. Text batching is an authorization
        boundary, though: the flushed MessageEvent is authorized using its
        source identity, so chunks from different Matrix senders must never be
        aggregated into one pending event.
        """
