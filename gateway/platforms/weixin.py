...
class WeixinAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        # Low-level send() is text-only.  Higher-level gateway code is
        # responsible for extracting trusted, structured attachments from agent
        # responses; parsing arbitrary text here could turn user-reflected
        # MEDIA:<path> strings into local file uploads.
        try:
    ...
    async def _send_file(
        ...
        safe_path = validate_outbound_media_path(path)
        plaintext = safe_path.read_bytes()
        path = str(safe_path)
