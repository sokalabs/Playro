...
def _cache_cleanup_dirs(
    selected_dir: Path, new_subpath: str, old_name: str
) -> list[Path]:
    """Return selected, new, and legacy cache directories that should be pruned.

    Cache locations are resolved at module import for backward compatibility,
    but external bridges may create the opposite layout after import.  Cleanup
    therefore checks both layouts when they exist so stale media cannot bypass
    expiry because one process chose a different cache path.
    """
    home = get_hermes_home()
    candidates = [selected_dir, home / new_subpath, home / old_name]
    dirs: list[Path] = []
    seen: set[Path] = set()
    for candidate in candidates:
        ...
        if key in seen:
            continue
        seen.add(key)
        if candidate == selected_dir or candidate.is_dir():
            dirs.append(candidate)
    return dirs


def _cleanup_cache_dirs(cache_dirs: list[Path], max_age_hours: int) -> int:
    """Delete stale files from cache directories and return the removal count."""
    import time
...
# Local MEDIA attachments may be emitted by trusted Hermes tools (image,
# audio, video, and document caches).  Do not let prompt-controlled
# MEDIA:<path> tags turn arbitrary process-readable files into outbound
# attachments.  Additional roots are an explicit opt-in for tests or
# deployments that generate media elsewhere.
_DEFAULT_MEDIA_ATTACHMENT_MAX_BYTES = 25 * 1024 * 1024
try:
    MEDIA_ATTACHMENT_MAX_BYTES = int(
        os.getenv("HERMES_MEDIA_ATTACHMENT_MAX_BYTES", str(_DEFAULT_MEDIA_ATTACHMENT_MAX_BYTES))
    )
except ValueError:
    MEDIA_ATTACHMENT_MAX_BYTES = _DEFAULT_MEDIA_ATTACHMENT_MAX_BYTES
_MEDIA_ATTACHMENT_EXTRA_ROOTS_ENV = "HERMES_MEDIA_ATTACHMENT_ROOTS"
_MEDIA_ATTACHMENT_EXTS = frozenset({
    ".png", ".jpg", ".jpeg", ".gif", ".webp",
    ".mp4", ".mov", ".avi", ".mkv", ".webm", ".3gp",
    ".ogg", ".opus", ".mp3", ".wav", ".m4a", ".flac",
    ".epub", ".pdf", ".zip", ".rar", ".7z", ".doc", ".docx",
    ".xls", ".xlsx", ".ppt", ".pptx", ".txt", ".csv", ".md",
    ".log", ".apk", ".ipa",
})


def _media_attachment_roots() -> List[Path]:
    roots = [IMAGE_CACHE_DIR, AUDIO_CACHE_DIR, VIDEO_CACHE_DIR, DOCUMENT_CACHE_DIR]
    for raw_root in os.getenv(_MEDIA_ATTACHMENT_EXTRA_ROOTS_ENV, "").split(os.pathsep):
...
def validate_media_attachment_path(path: str) -> Tuple[Optional[str], Optional[str]]:
    """Return a safe local media path or a warning describing why it is blocked.

    MEDIA tags can be influenced by model output/user prompts, so outbound
    attachment sinks must only open regular, bounded-size files from Hermes'
    generated media caches or explicitly configured media roots.
    """
    try:
        candidate = Path(path).expanduser().resolve(strict=True)
    except FileNotFoundError:
        return None, f"Media file not found, skipping: {path}"
    except OSError as exc:
        return None, f"Media file unavailable, skipping: {path}: {exc}"

    if not candidate.is_file():
        return None, f"Media path is not a regular file, skipping: {path}"

    if candidate.suffix.lower() not in _MEDIA_ATTACHMENT_EXTS:
        return None, f"Media file type is not allowed, skipping: {path}"

    try:
        size = candidate.stat().st_size
    except OSError as exc:
        return None, f"Media file unavailable, skipping: {path}: {exc}"
    if size > MEDIA_ATTACHMENT_MAX_BYTES:
        return None, f"Media file too large, skipping: {path}"

    roots = _media_attachment_roots()
    if not any(candidate == root or candidate.is_relative_to(root) for root in roots):
        return None, f"Media file is outside allowed media roots, skipping: {path}"
...
def cache_image_from_bytes(data: bytes, ext: str = ".jpg") -> str:
    ...
    """
    from gateway.platforms.security import cache_image_bytes_within_limit

    if not cache_image_bytes_within_limit(data):
        raise ValueError(
            f"Refusing to cache image larger than allowed limit ({len(data)} bytes)"
        )
...
def cleanup_image_cache(max_age_hours: int = 24) -> int:
    ...
    """
    return _cleanup_cache_dirs(
        _cache_cleanup_dirs(IMAGE_CACHE_DIR, "cache/images", "image_cache"),
        max_age_hours,
    )
...
def cache_video_from_bytes(data: bytes, ext: str = ".mp4") -> str:
    ...
    if len(data) > MAX_VIDEO_BYTES:
        raise ValueError(f"Video exceeds maximum size of {MAX_VIDEO_BYTES} bytes")
...
# Outbound MEDIA:<path> delivery should only read files that Hermes created
# inside its media caches.  Model/chat text can be attacker-influenced, so this
# allowlist prevents MEDIA:/etc/passwd-style local file disclosure when adapters
# turn response text into native attachments.
_OUTBOUND_MEDIA_MAX_BYTES_DEFAULT = 25 * 1024 * 1024
_OUTBOUND_MEDIA_EXTS = frozenset({
    ".jpg", ".jpeg", ".png", ".webp", ".gif",
    ".ogg", ".opus", ".mp3", ".wav", ".m4a", ".flac", ".silk",
    ".mp4", ".mov", ".avi", ".mkv", ".webm", ".3gp",
    ".epub", ".pdf", ".zip", ".rar", ".7z",
    ".doc", ".docx", ".xls", ".xlsx", ".ppt", ".pptx",
    ".txt", ".csv", ".md", ".json", ".xml", ".yaml", ".yml",
    ".toml", ".ini", ".cfg", ".log", ".apk", ".ipa",
})


def get_outbound_media_roots() -> List[Path]:
    """Return the trusted local roots from which MEDIA paths may be sent.

    These roots are controlled by Hermes cache helpers (or their legacy cache
    aliases) and are intended for generated/downloaded media artifacts, not
    ...
    roots = [
        ...
        Path(get_hermes_dir("cache/screenshots", "browser_screenshots")),
    ]
...
def validate_outbound_media_path(
    ...
    *,
    ...
    allowed_extensions: Optional[set[str] | frozenset[str]] = None,
) -> Path:
    """Validate and resolve a local path before native attachment upload.

    Raises ``ValueError`` unless the path is an existing regular file under a
    trusted Hermes media cache root, has an expected attachment extension, and
    is within the outbound size limit.
    """
    candidate = Path(path).expanduser()
    try:
        resolved = candidate.resolve(strict=True)
    except FileNotFoundError as exc:
        raise ValueError(f"Outbound media file does not exist: {path!r}") from exc
    except OSError as exc:
        raise ValueError(f"Outbound media path is not readable: {path!r}") from exc

    roots = get_outbound_media_roots()
    if not any(resolved.is_relative_to(root) for root in roots):
        raise ValueError("Outbound media path is outside the trusted Hermes media cache")

    extensions = allowed_extensions or _OUTBOUND_MEDIA_EXTS
    if resolved.suffix.lower() not in extensions:
        raise ValueError(f"Outbound media extension is not allowed: {resolved.suffix or '<none>'}")

    if not resolved.is_file():
        raise ValueError(f"Outbound media path is not a regular file: {path!r}")

    limit = max_bytes
    if limit is None:
        raw_limit = os.getenv("HERMES_OUTBOUND_MEDIA_MAX_BYTES", "").strip()
        if raw_limit:
            try:
                limit = max(0, int(raw_limit))
            except ValueError:
                limit = _OUTBOUND_MEDIA_MAX_BYTES_DEFAULT
        else:
            limit = _OUTBOUND_MEDIA_MAX_BYTES_DEFAULT

    size = resolved.stat().st_size
    if limit and size > limit:
        raise ValueError(f"Outbound media file exceeds size limit ({size} > {limit} bytes)")
...
def cleanup_document_cache(max_age_hours: int = 24) -> int:
    ...
    """
    return _cleanup_cache_dirs(
        _cache_cleanup_dirs(DOCUMENT_CACHE_DIR, "cache/documents", "document_cache"),
        max_age_hours,
    )
...
class MessageEvent:
    ...
    reply_to_user_id: Optional[str] = None  # Author of the replied-to message, when available
    reply_to_user_name: Optional[str] = None
    reply_to_is_bot: bool = False  # True when the reply target is the adapter's own bot account
        ...
        for image_url, alt_text in images:
            ...
            try:
                ...
                if image_url.startswith("file://"):
                    ...
                    try:
                        validate_outbound_media_path(local_path)
                    except ValueError as media_err:
                        logger.warning(
                            "[%s] Blocked untrusted file:// image path: %s",
                            self.name,
                            media_err,
                        )
                        continue
    ...
    def extract_local_files(content: str) -> Tuple[List[str], str]:
        ...
        for match in path_re.finditer(content):
            ...
            if not os.path.isfile(expanded):
                continue
            try:
                validate_outbound_media_path(expanded)
            except ValueError:
                logger.debug(
                    "Skipping untrusted local media path in response: %s",
                    raw,
                )
                continue
            found.append((raw, str(validate_outbound_media_path(expanded))))
        ...
        if task is not None and not task.done():
            ...
            try:
                ...
                logger.warning(
                    ...
                    "keeping session task tracked while it unwinds",
                    ...
                    try:
                        ...
                        if should_send_media_as_audio(self.platform, ext, is_voice=is_voice):
                            ...
                            media_result = await self.send_video(
                                ...
                                metadata={**(_thread_metadata or {}), "_from_media_directive": True},
                            )
                        else:
        ...
        for _ in range(MAX_DRAIN_ROUNDS):
            ...
            try:
                ...
                logger.warning(
                    ...
                    "retaining tracking for still-running tasks",
                    self.name,
                    len(still_running),
                )
