...
class DingTalkAdapter(BasePlatformAdapter):
    ...
    async def _on_message(
        ...
        if chat_id:
            ...
            msg_id_for_reaction = getattr(message, "message_id", "") or ""
            if msg_id_for_reaction and conversation_id:
                self._spawn_bg(
                    self._send_emotion(
                        msg_id_for_reaction, conversation_id, "🤔Thinking", recall=False,
                    )
                )
    ...
    async def send_image(
        ...
        from gateway.platforms.security import (
            ...
            is_public_http_image_url,
        )

        if not is_public_http_image_url(image_url):
            logger.warning("[%s] Blocked unsafe DingTalk image URL", self.name)
            return await super().send_image(
                chat_id, image_url, caption, reply_to, metadata=metadata
            )
        safe_url = escape_markdown_link_destination(image_url)
        image_block = f"![image]({safe_url})"
