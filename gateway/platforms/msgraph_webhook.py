...
from gateway.platforms.security import (
    ...
    build_notification_receipt_key,
...
class MSGraphWebhookAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._max_body_bytes = max(
            1024, int(extra.get("max_body_bytes", DEFAULT_MAX_WEBHOOK_BODY_BYTES))
        )
        if webhook_auth_required(
            client_state=self._client_state,
            bind_host=self._host,
        ):
            raise ValueError(
                "[msgraph_webhook] client_state is required when binding to a "
                "non-loopback host or in production. Generate one with "
                "'openssl rand -hex 32' and set platforms.msgraph_webhook.extra.client_state."
            )
    ...
    def _build_receipt_key(notification: Dict[str, Any]) -> Optional[str]:
        return build_notification_receipt_key(notification)
    ...
    def _verify_client_state(self, notification: Dict[str, Any]) -> bool:
        ...
        if expected is None:
            return not webhook_auth_required(
                client_state=None,
                bind_host=self._host,
            )
        ...
        if provided is None:
            return False
        return compare_ascii_secrets(provided, expected)
