...
class QQAdapter(BasePlatformAdapter):
    ...
    async def _default_interaction_dispatch(
        ...
        if not self._is_interaction_authorized(event):
            logger.warning(
                "[%s] Rejecting interaction %s from unauthorized operator %s",
                self._log_tag,
                event.id,
    ...
    def _trusted_local_upload_roots(self) -> List[Path]:
        """Return directories from which QQ local uploads are allowed.

        MEDIA tags can originate in model/handler text, so QQ must not upload
        arbitrary filesystem paths. By default only Hermes-managed media cache
        directories are trusted. Deployments that intentionally stage outbound
        media elsewhere may add explicit directories via
        ``extra.qq_local_upload_roots`` or ``extra.local_upload_roots``.
        """
        roots: List[Path] = [
            get_image_cache_dir(),
            get_audio_cache_dir(),
        ...
        extra = self.config.extra or {}
        configured_roots = _coerce_list(
            extra.get("qq_local_upload_roots")
            or extra.get("qqLocalUploadRoots")
            or extra.get("local_upload_roots")
            or extra.get("localUploadRoots")
        )
        roots.extend(Path(root).expanduser() for root in configured_roots)
        ...
        for root in roots:
            try:
                ...
                logger.warning(
                    "[%s] Ignoring invalid QQ local upload root: %s",
                    self._log_tag, root,
                )
        return resolved_roots

    def _validate_local_upload_path(self, local_path: Path) -> None:
        """Validate local QQ upload path origin and size before reading it."""
        trusted_roots = self._trusted_local_upload_roots()
        if not any(local_path.is_relative_to(root) for root in trusted_roots):
            raise PermissionError(
                "Refusing to upload local file outside trusted media cache roots: "
                f"{local_path}"
            )

        file_size = local_path.stat().st_size
        if file_size > MAX_CHUNKED_UPLOAD_BYTES:
            raise UploadFileTooLargeError(
                local_path.name, file_size, MAX_CHUNKED_UPLOAD_BYTES
            )
    ...
    async def _upload_local_file(
        ...
        self._validate_local_upload_path(local_path)
    ...
    def _is_interaction_authorized(self, event: InteractionEvent) -> bool:
        """Gate inline-keyboard clicks through the same ACL as inbound messages."""
        operator = event.operator_openid
        if not operator:
            return False
        if event.scene == "c2c":
            return self._is_dm_allowed(operator)
        group_key = event.group_openid or event.guild_id or event.channel_id
        return self._is_group_allowed(group_key, operator)
