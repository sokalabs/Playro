...
# QQ Bot currently advertises an approximately 100 MiB per-file ceiling for
# the chunked upload flow. Enforce it locally before hashing or reading file
# contents so an untrusted path cannot force expensive I/O for oversize files.
MAX_CHUNKED_UPLOAD_BYTES = 100 * 1024 * 1024
    ...
    async def upload(
        ...
        if file_size > MAX_CHUNKED_UPLOAD_BYTES:
            raise UploadFileTooLargeError(
                file_name, file_size, MAX_CHUNKED_UPLOAD_BYTES
            )
