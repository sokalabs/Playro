        ...
        for app in self._apps:
            try:
                ...
                if event is not None:
                    ...
                    if event.source:
                        # Use scoped chat_id (corp:user) — user_id is already scoped
                        # and must not be passed through _user_app_key again.
                        map_key = (
                            event.source.chat_id
                            or self._user_app_key(
                                str(app.get("corp_id") or ""),
                                event.source.user_id_alt or event.source.user_id,
                            )
                        )
                        if map_key:
