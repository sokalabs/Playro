...
from gateway.platforms.security import (
    ...
    DEFAULT_WEBHOOK_BIND_HOST,
    is_loopback_host,
)

DEFAULT_HOST = DEFAULT_WEBHOOK_BIND_HOST
DEFAULT_PORT = 8644
...
class WebhookAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Minimal delivery info keyed by session chat_id.
        #
        ...
        # back to the "log" deliver type.  Keep only values needed by
        # delivery targets; the potentially large webhook payload remains
        # on the MessageEvent and must not be retained here.
        ...
        self._max_body_bytes: int = int(
            config.extra.get("max_body_bytes", DEFAULT_MAX_WEBHOOK_BODY_BYTES)
        )
    ...
    async def connect(self) -> bool:
        ...
        for name, route in self._routes.items():
            ...
            if secret == _INSECURE_NO_AUTH and not is_loopback_host(self._host):
                raise ValueError(
    ...
    def _prune_delivery_info(self, now: float) -> None:
        ...
        Entries intentionally contain only small delivery metadata, not the
    ...
    def _reload_dynamic_routes(self) -> None:
        ...
        try:
            ...
            for name, route in data.items():
                if not isinstance(route, dict) or name in self._static_routes:
                    continue
                secret = route.get("secret", self._global_secret)
                if not secret:
                    logger.warning(
                        "[webhook] Skipping dynamic route %r without HMAC secret",
                        name,
                    )
                    continue
                if secret == _INSECURE_NO_AUTH and not is_loopback_host(self._host):
                    logger.warning(
                        "[webhook] Skipping dynamic route %r: INSECURE_NO_AUTH "
                        "is only allowed on loopback binds",
                        name,
                    )
                    continue
                loaded[name] = route
            self._dynamic_routes = loaded
    ...
    async def _handle_webhook(self, request: "web.Request") -> "web.Response":
        ...
        # Read body (must be done before signature validation)
        try:
        ...
        # Rate limit every POST (including invalid signatures) to prevent
        # unauthenticated brute-force/log-spam against the listener.
        now = time.time()
        ...
        # Store only the delivery metadata needed by send().  Read by every
        # send() invocation for this chat_id (interim status messages and the
        # final response), so we do NOT pop on send.  TTL-based cleanup keeps
        # the dict bounded without retaining the potentially large webhook
        # payload for the full TTL window.
        deliver_config = {
            "deliver": route_config.get("deliver", "log"),
            "deliver_extra": self._render_delivery_extra(
                route_config.get("deliver_extra", {}), payload
