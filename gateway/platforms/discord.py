...
def _is_relative_to(path: _Path, root: _Path) -> bool:
    """Compatibility wrapper for Path.is_relative_to on resolved paths."""
    try:
...
def _is_safe_discord_media_directive_path(file_path: str, *, allowed_exts: set[str]) -> bool:
    """Return whether an untrusted MEDIA path may be uploaded to Discord.

    Discord MEDIA directives are model-controlled text, so document/video
    uploads must be restricted to Hermes-managed media caches rather than
    arbitrary host paths such as /etc/passwd or ~/.env.
    """
    try:
        candidate = _Path(os.path.expanduser(file_path)).resolve(strict=True)
    ...
    return any(_is_relative_to(candidate, root) for root in safe_roots)


def _is_media_directive_metadata(metadata: Optional[Dict[str, Any]]) -> bool:
    """Return True for paths extracted from model-authored MEDIA directives."""
    return bool(metadata and metadata.get("_from_media_directive"))
...
class VoiceReceiver:
    ...
    def _infer_user_for_ssrc(self, ssrc: int) -> int:
        ...
        SPEAKING events for users already speaking. Only infer identity when
        there is exactly one non-bot member in the channel and that member is
        authorized by the configured user allowlist.
        """
        try:
            ...
            members = [
                ...
                if m.id != bot_id and not getattr(m, "bot", False)
            ]
            if len(members) != 1:
                return 0
            ...
            logger.info("Auto-mapped ssrc=%d -> user=%d (sole authorized member)", ssrc, uid)
            return uid
        except Exception:
            pass
        return 0
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            async def on_message(message: DiscordMessage):
                ...
                # Run this before the user/role allowlist so DISCORD_ALLOW_BOTS
                # can admit bots when no Discord allowlist is configured, but do
                # not let bot/webhook authors bypass an explicit allowlist.
                ...
                # Enforce the configured user/role allowlists for humans and
                # bots alike. With no Discord allowlist configured,
                # _is_allowed_user returns True for backwards compatibility.
    ...
    async def _send_file_attachment(
        ...
        if self._is_forum_parent(channel):
            safe_file_path, warning = validate_media_attachment_path(file_path)
            if warning:
                return SendResult(success=False, error=warning)

        filename = file_name or os.path.basename(safe_file_path)
        with open(safe_file_path, "rb") as fh:
            ...
            try:
                for image_url, alt_text in chunk:
                    ...
                    if image_url.startswith("file://"):
                        ...
                        safe_path, warning = validate_media_attachment_path(local_path)
                        if warning:
                            logger.warning("[%s] %s", self.name, warning)
                            continue
                        files.append(_discord_mod.File(safe_path, filename=os.path.basename(safe_path)))
                    else:
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        try:
            ...
            if self._is_forum_parent(channel):
                safe_audio_path, warning = validate_media_attachment_path(audio_path)
                if warning:
                    return SendResult(success=False, error=warning)
            elif not os.path.exists(audio_path):
                return SendResult(success=False, error=f"Audio file not found: {audio_path}")

            filename = os.path.basename(safe_audio_path)
    ...
    async def send_video(
        ...
        if _is_media_directive_metadata(metadata) and not _is_safe_discord_media_directive_path(
            video_path,
            allowed_exts=set(SUPPORTED_VIDEO_TYPES),
        ):
            logger.warning("[%s] Blocked unsafe Discord MEDIA video path: %s", self.name, video_path)
            return SendResult(success=False, error="Blocked unsafe MEDIA video path")
    ...
    async def send_document(
        ...
        if _is_media_directive_metadata(metadata) and not _is_safe_discord_media_directive_path(
            ...
            logger.warning("[%s] Blocked unsafe Discord MEDIA document path: %s", self.name, file_path)
            return SendResult(success=False, error="Blocked unsafe MEDIA document path")
