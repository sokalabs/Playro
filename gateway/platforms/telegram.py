...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _is_callback_user_authorized(
        ...
        *,
        ...
        platform_allowlist = os.getenv("TELEGRAM_ALLOWED_USERS", "").strip()
        group_user_allowlist = ""
        group_chat_allowlist = ""
        normalized_chat_type = str(chat_type or "dm").strip().lower() or "dm"
        if normalized_chat_type in {"group", "forum", "supergroup"}:
            group_user_allowlist = os.getenv("TELEGRAM_GROUP_ALLOWED_USERS", "").strip()
            group_chat_allowlist = os.getenv("TELEGRAM_GROUP_ALLOWED_CHATS", "").strip()
        ...
        if group_chat_allowlist and normalized_chat_type in {"group", "forum", "supergroup"}:
            allowed_chat_ids = {
                allowed_chat_id.strip()
                for allowed_chat_id in group_chat_allowlist.split(",")
                if allowed_chat_id.strip()
            }
            normalized_chat_id = str(chat_id or "").strip()
            if "*" in allowed_chat_ids or normalized_chat_id in allowed_chat_ids:
                return True
    ...
    def _telegram_require_mention(self) -> bool:
        ...
        from gateway.platforms.security import env_bool
    ...
    def _telegram_free_response_chats(self) -> set[str]:
        from gateway.platforms.security import env_csv_set
    ...
    async def _handle_media_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if msg.voice:
            ...
            try:
                ...
                if not isinstance(video_size, int) or video_size > MAX_VIDEO_BYTES:
                    event.text = (
    ...
    def _build_message_event(
        ...
        if message.reply_to_message:
            ...
            if reply_user is not None:
                reply_to_user_id = str(reply_user.id) if getattr(reply_user, "id", None) else None
                reply_to_user_name = getattr(reply_user, "full_name", None)
            reply_to_is_bot = self._is_reply_to_bot(message)
