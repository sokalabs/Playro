...
class WeComAdapter(BasePlatformAdapter):
    ...
    async def _on_message(self, payload: Dict[str, Any]) -> None:
        ...
        # Only batch plain, non-command text messages. Commands, media, and
        # reply-context messages dispatch immediately so their authorization and
        # command arguments cannot be mutated by later messages in the batch
        # window.
        if (
    ...
    def _text_batch_key(self, event: MessageEvent) -> str:
        """Sender-scoped key for safe text message batching.

        ``build_session_key`` can intentionally omit the participant id for
        shared group/thread sessions. Batching must remain stricter than
        transcript session routing because gateway authorization happens later
        against the preserved ``event.source``. Include the authenticated sender
        identity so a later group participant cannot append text to a pending
        event from another sender.
        """
        ...
        # In DMs the chat id is the sender identity. For group/thread events
        # without a sender, fail closed by making the batch message-specific
        # instead of merging unknown authors together.
        if not sender_id:
            if getattr(source, "chat_type", None) == "dm":
                sender_id = getattr(source, "chat_id", "")
            else:
                sender_id = f"message:{event.message_id or uuid.uuid4().hex}"
