...
user is seen through different apps in the future.  Authorization and
pairing keep using ``open_id`` as the primary ID because bare ``user_id``
values are only tenant-scoped.
"""
...
def _to_boolean(value: Any) -> bool:
    ...
    return parse_strict_boolean(value, default=False)
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Exec approval button state (approval_id → {session_key, message_id, chat_id, token})
    ...
    def _is_approval_card_action_authorized(
        self,
        *,
        event: Any,
        ...
        """Validate approval card callbacks before resolving gateway approvals."""
        approval_id = action_value.get("approval_id")
        expected_token = state.get("token", "")
        supplied_token = str(action_value.get("approval_token", "") or "")
        if expected_token and not hmac.compare_digest(supplied_token, expected_token):
            logger.warning("[Feishu] Rejecting approval %s with invalid approval token", approval_id)
            return False

        chat_id = self._card_action_chat_id(event)
        expected_chat_id = state.get("chat_id", "")
        if expected_chat_id and chat_id != expected_chat_id:
            logger.warning(
                "[Feishu] Rejecting approval %s from mismatched chat_id=%s expected=%s",
                approval_id,
                chat_id or "<missing>",
                expected_chat_id,
            )
        ...
        message_id = self._card_action_message_id(event)
        expected_message_id = state.get("message_id", "")
        if expected_message_id and message_id != expected_message_id:
            logger.warning(
                "[Feishu] Rejecting approval %s from mismatched message_id=%s expected=%s",
                approval_id,
        ...
        if not open_id:
            logger.warning("[Feishu] Rejecting approval %s with missing operator open_id", approval_id)
            return False
        ...
        if self._allow_group_message(sender_id, chat_id):
            return True

        logger.warning(
            "[Feishu] Rejecting approval %s from unauthorized operator %s in chat %s",
            approval_id,
            open_id,
            chat_id or "<missing>",
        )
    ...
    def _handle_approval_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        empty_response = P2CardActionTriggerResponse() if P2CardActionTriggerResponse else None
        approval_id = action_value.get("approval_id")
        ...
        if not state:
            logger.debug("[Feishu] Approval %s already resolved or unknown", approval_id)
            return P2CardActionTriggerResponse() if P2CardActionTriggerResponse else None
        if not self._is_approval_card_action_authorized(event=event, action_value=action_value, state=state):
            return P2CardActionTriggerResponse() if P2CardActionTriggerResponse else None

        choice = _APPROVAL_CHOICE_MAP.get(action_value.get("hermes_action"), "deny")

        hermes_action = action_value.get("hermes_action")
        choice = _APPROVAL_CHOICE_MAP.get(hermes_action)
        if choice is None:
            logger.debug("[Feishu] Card action has invalid approval action=%r", hermes_action)
            return empty_response
        ...
        if not chat_id or not open_id:
            logger.debug("[Feishu] Approval card action missing chat_id or operator open_id, dropping")
            return empty_response
        if chat_id != state.get("chat_id"):
            logger.warning("[Feishu] Approval %s clicked from unexpected chat_id=%s", approval_id, chat_id)
            return empty_response
    ...
    def _handle_update_prompt_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        chat_id = self._card_action_chat_id(event)
        if not open_id:
            logger.warning("[Feishu] Unauthorized update prompt click with missing operator")
            return P2CardActionTriggerResponse() if P2CardActionTriggerResponse else None
        sender_id = SimpleNamespace(open_id=open_id, user_id=None, union_id=None)
        if chat_id:
            if not self._allow_group_message(sender_id, chat_id):
                logger.warning(
                    "[Feishu] Unauthorized update prompt click by %s in chat %s",
                    open_id,
                    chat_id,
                )
                return P2CardActionTriggerResponse() if P2CardActionTriggerResponse else None
        elif not self._is_interactive_operator_authorized(open_id):
            logger.warning("[Feishu] Unauthorized update prompt click by %s", open_id)
            return P2CardActionTriggerResponse() if P2CardActionTriggerResponse else None
    ...
    async def _resolve_sender_profile(
        ...
        *,
        ...
        The primary ``user_id`` field is used by generic gateway authorization
        and pairing, so it must not contain a bare tenant-scoped Feishu
        ``user_id``.  Prefer ``open_id`` because it is app-scoped and always
        present in event payloads; fall back to ``user_id`` only when Feishu
        does not provide an ``open_id``.
        ...
        when the primary ID is the app-scoped open_id.
        """
        open_id = getattr(sender_id, "open_id", None) or None
        user_id = getattr(sender_id, "user_id", None) or None
        union_id = getattr(sender_id, "union_id", None) or None
        # Authorization/pairing compare this field globally per platform.
        # Prefer app-scoped open_id over bare tenant-scoped user_id to avoid
        # cross-tenant user_id collisions in multi-tenant Feishu/Lark setups.
