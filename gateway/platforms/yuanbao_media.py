...
_ALWAYS_BLOCKED_IPS = frozenset({
    ipaddress.ip_address("169.254.169.254"),  # cloud metadata endpoints
...
def _is_blocked_download_ip(ip: ipaddress.IPv4Address | ipaddress.IPv6Address) -> bool:
    """Return True for IP ranges that Yuanbao media downloads must never fetch."""
...
def _validate_download_url(url: str) -> None:
    """Validate that a Yuanbao media URL cannot target internal services.

    Yuanbao media URLs can be supplied by model output or decoded inbound
    messages.  Treat them as untrusted and fail closed before every request and
    redirect hop.
    """
    parsed = urllib.parse.urlparse(url)
    scheme = (parsed.scheme or "").lower()
    if scheme not in {"http", "https"}:
        raise ValueError("Blocked media URL: only http and https URLs are allowed")

    hostname = (parsed.hostname or "").strip().lower().rstrip(".")
    if not hostname:
        raise ValueError("Blocked media URL: missing hostname")
    if hostname in _BLOCKED_HOSTNAMES:
        raise ValueError("Blocked media URL: internal metadata hostname")
    ...
    if literal_ip is not None:
        if _is_blocked_download_ip(literal_ip):
            raise ValueError("Blocked media URL: private or internal IP address")
        return
    ...
    try:
        ...
        raise ValueError("Blocked media URL: hostname did not resolve") from exc

    if not addr_info:
        raise ValueError("Blocked media URL: hostname did not resolve")

    for _family, _socktype, _proto, _canonname, sockaddr in addr_info:
        try:
            resolved_ip = ipaddress.ip_address(sockaddr[0])
        except (IndexError, ValueError):
            raise ValueError("Blocked media URL: invalid resolved address")
        if _is_blocked_download_ip(resolved_ip):
            raise ValueError(
                "Blocked media URL: hostname resolves to private or internal IP address"
            )


async def _ssrf_redirect_guard(response: httpx.Response) -> None:
    """Re-validate redirect targets before httpx follows them."""
    if response.is_redirect and response.next_request is not None:
        _validate_download_url(str(response.next_request.url))
