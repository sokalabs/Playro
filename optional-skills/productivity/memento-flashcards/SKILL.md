...
Question text, answer text, generated quiz JSON, transcript-derived content, collection names, and user review answers are untrusted text. Do not place these values directly in shell command arguments, even inside single or double quotes. Shell command substitution such as `$()` and backticks can run before Python receives the arguments.

For dynamic card content, use the documented `--stdin-json` examples with a single-quoted here-document delimiter (`<<'MEMENTO_JSON'`). The shell does not expand text inside that here-document. If you choose a custom delimiter, choose one that does not appear on a line by itself inside the JSON payload.

Static IDs, ratings, and trusted local file paths may still be passed as ordinary command arguments where shown.
...
**Step 2:** Call the script to store the card using JSON on stdin:

```bash
python3 ~/.hermes/skills/productivity/memento-flashcards/scripts/memento_cards.py add --stdin-json <<'MEMENTO_JSON'
{"question":"What year did World War 2 end?","answer":"1945","collection":"History"}
MEMENTO_JSON
```
...
Then call `memento_cards.py add --stdin-json` as above.
...
> *(agent calls: memento_cards.py rate --stdin-json with id, rating, and user_answer in JSON on stdin)*
>
...
```bash
python3 ~/.hermes/skills/productivity/memento-flashcards/scripts/memento_cards.py rate --stdin-json <<'MEMENTO_JSON'
{"id":"CARD_ID","rating":"easy","user_answer":"what the user said"}
MEMENTO_JSON
```
...
```bash
python3 ~/.hermes/skills/productivity/memento-flashcards/scripts/memento_cards.py add-quiz --stdin-json <<'MEMENTO_JSON'
{"video_id":"VIDEO_ID","questions":[{"question":"...","answer":"..."}],"collection":"Quiz - Episode Title"}
MEMENTO_JSON
```
...
python3 ~/.hermes/skills/productivity/memento-flashcards/scripts/memento_cards.py stats
python3 ~/.hermes/skills/productivity/memento-flashcards/scripts/memento_cards.py add --stdin-json <<'MEMENTO_JSON'
{"question":"Capital of France?","answer":"Paris","collection":"General"}
MEMENTO_JSON
python3 ~/.hermes/skills/productivity/memento-flashcards/scripts/memento_cards.py due
```
