...
def _read_stdin_json() -> dict:
    try:
        ...
        _out({"ok": False, "error": f"Invalid JSON on stdin: {exc}"})
        sys.exit(1)
    if not isinstance(payload, dict):
        _out({"ok": False, "error": "stdin JSON must be an object"})
        sys.exit(1)
    return payload


def _apply_stdin_json(args: argparse.Namespace, allowed_keys: set[str]) -> None:
    if not getattr(args, "stdin_json", False):
        return
    payload = _read_stdin_json()
    unknown = set(payload) - allowed_keys
    if unknown:
        _out({"ok": False, "error": f"Unsupported stdin JSON keys: {', '.join(sorted(unknown))}"})
        sys.exit(1)
    for key, value in payload.items():
        setattr(args, key, value)


def _require_text_arg(args: argparse.Namespace, name: str) -> str:
    value = getattr(args, name, None)
    if not isinstance(value, str) or not value:
        _out({"ok": False, "error": f"Missing required text field: {name}"})
        sys.exit(1)
    return value
...
def cmd_add(args: argparse.Namespace) -> None:
    _apply_stdin_json(args, {"question", "answer", "collection"})
    question = _require_text_arg(args, "question")
    answer = _require_text_arg(args, "answer")
    collection = getattr(args, "collection", None) or "General"
...
def cmd_add_quiz(args: argparse.Namespace) -> None:
    _apply_stdin_json(args, {"video_id", "questions", "collection"})
    video_id = _require_text_arg(args, "video_id")
    raw_questions = getattr(args, "questions", None)
    ...
    if not isinstance(questions, list):
        _out({"ok": False, "error": "questions must be a JSON array"})
        sys.exit(1)
    collection = getattr(args, "collection", None) or "Quiz"
    ...
    if video_id in existing_ids:
        existing = [c for c in data["cards"] if c.get("video_id") == video_id]
...
def cmd_rate(args: argparse.Namespace) -> None:
    _apply_stdin_json(args, {"id", "rating", "user_answer"})
    card_id = _require_text_arg(args, "id")
    rating = _require_text_arg(args, "rating")
    ...
    for c in data["cards"]:
        if c["id"] == card_id:
            ...
            break
    if not card:
        _out({"ok": False, "error": f"Card not found: {card_id}"})
        sys.exit(1)
...
def main() -> None:
    ...
    p_add.add_argument("--stdin-json", action="store_true", help="Read question, answer, and collection from a JSON object on stdin")

    p_quiz = sub.add_parser("add-quiz", help="Batch-add quiz cards")
    p_quiz.add_argument("--video-id")
    p_quiz.add_argument("--questions", help="JSON array of {question, answer}")
    p_quiz.add_argument("--collection", default="Quiz")
    p_quiz.add_argument("--stdin-json", action="store_true", help="Read video_id, questions, and collection from a JSON object on stdin")
    ...
    p_rate.add_argument("--rating", choices=["easy", "good", "hard", "retire"])
    p_rate.add_argument("--user-answer", default=None)
    p_rate.add_argument("--stdin-json", action="store_true", help="Read id, rating, and user_answer from a JSON object on stdin")
