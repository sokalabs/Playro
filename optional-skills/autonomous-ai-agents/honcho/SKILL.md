...
All 5 tools accept an optional `peer` parameter scoped to the current session:
- `peer: "user"` (default) — operates on the current session's user peer
- `peer: "ai"` — operates on this profile's AI peer

Arbitrary workspace peer IDs are intentionally rejected to prevent cross-user memory access.
