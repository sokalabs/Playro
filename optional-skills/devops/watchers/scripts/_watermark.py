...
_DELIVERY_DIRECTIVES = ("[[audio_as_voice]]", "[[as_document]]")


def neutralize_delivery_directives(value: Any) -> str:
    """Return watcher text that cannot trigger Hermes delivery controls.

    Watchers render content from external feeds and APIs.  Cron delivery later
    scans stdout for internal Hermes directives such as ``MEDIA:<path>`` and
    bracketed attachment options.  Escape just that control syntax so
    attacker-controlled items remain readable text instead of becoming local
    file uploads or delivery flags.
    """
    text = str(value)
    text = _MEDIA_DIRECTIVE_RE.sub("MEDIA\\:", text)
    for directive in _DELIVERY_DIRECTIVES:
        text = text.replace(directive, directive.replace("[[", "[\\[", 1))
    return text
...
def format_items_as_markdown(
    ...
    *,
    ...
    lines: List[str] = []
    for item in items:
        title = neutralize_delivery_directives(
            item.get(title_key) or "(no title)"
        ).strip()
        url = neutralize_delivery_directives(item.get(url_key) or "").strip()
        lines.append(f"## {title}")
        if url:
            lines.append(url)
        if body_key:
            body = neutralize_delivery_directives(item.get(body_key) or "").strip()
            if body:
