...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": (
            "Import MCP server definitions from OpenClaw into Hermes config.yaml "
...
class Migrator:
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        for name, srv in mcp_raw.items():
            ...
            # Common fields
            # MCP server definitions are executable configuration: stdio entries
            # run command/args during tool discovery, and HTTP entries can carry
            # sensitive headers/auth.  Preserve the migrated definition for
            # review, but never enable OpenClaw-sourced MCP servers implicitly.
            ...
            self.record(
                ...
                "migrated",
                "MCP server migrated disabled; review and explicitly enable before use",
                servers_added=added,
                enabled=False,
