...
## Path 1 — Local demo only (no bookmarklet, no arbitrary-site injection)

**Security:** Do not use bookmarklets or paste third-party scripts into authenticated production pages. DOM contents and UI actions may be sent to the configured LLM provider.

For a quick local evaluation, serve a throwaway HTML page from `http://127.0.0.1` and install `page-agent` via npm (Path 2). Do **not** load the public jsDelivr demo bundle on sites with real user sessions or credentials.
...
**Test locally only** — open a throwaway page you control (for example `http://127.0.0.1:5174/` or a static fixture in this repo), then load the dev bundle from the same origin:

```html
<script src="http://127.0.0.1:5174/page-agent.demo.js"></script>
```

Run `npm run dev:demo` first. Do **not** use bookmarklets or inject the demo script into authenticated third-party sites; the bundle may inline secrets from `.env` during dev builds.

**Warning:** your `.env` `LLM_API_KEY` is inlined into the IIFE bundle during dev builds. Don't share the bundle. Don't commit it.
...
2. `curl -I http://127.0.0.1:5174/page-agent.demo.js` returns `HTTP/1.1 200 OK` with `Content-Type: application/javascript`
3. Load the script from a local test page on `127.0.0.1`; panel appears
