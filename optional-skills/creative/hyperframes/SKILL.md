...
2. Installs the pinned `hyperframes` CLI globally (`hyperframes@0.4.2`) with npm lifecycle scripts disabled.
3. Pre-caches `chrome-headless-shell` via pinned `puppeteer@24.19.0` — **required** for best-quality rendering via Chrome's `HeadlessExperimental.beginFrame` capture path.
4. Runs `hyperframes doctor` and reports the result.

The setup intentionally avoids `@latest`, unpinned `npx`, and `sudo npm install` so official-skill setup does not execute future registry contents or npm lifecycle scripts with elevated privileges.
...
- **`HeadlessExperimental.beginFrame' wasn't found`** — Chromium 147+ removed this protocol. Run the pinned setup script to install `hyperframes@0.4.2` (auto-detects and falls back to screenshot mode). Escape hatch: `export PRODUCER_FORCE_SCREENSHOT=true`. See [hyperframes#294](https://github.com/heygen-com/hyperframes/issues/294) and [references/troubleshooting.md](references/troubleshooting.md).
- **System Chrome (not `chrome-headless-shell`)** — renders hang for 120s then timeout. Re-run `scripts/setup.sh`; it uses pinned `puppeteer@24.19.0` to install `chrome-headless-shell`. `hyperframes doctor` reports which binary will be used.
...
2. **Animation choreography** — for new compositions or significant animation changes, run the animation map only through a trusted HyperFrames entry point. Do **not** execute project-local copies such as `skills/hyperframes/scripts/animation-map.mjs`; project contents are not a trusted code root. If your installed `hyperframes` version does not provide a trusted animation-map command, skip this optional check rather than running copied JavaScript from the project.
   ```bash
   npx hyperframes animation-map <composition-dir> \
     --out <composition-dir>/.hyperframes/anim-map
   ```
   Outputs a single `animation-map.json` with per-tween summaries, ASCII Gantt timeline, stagger detection, dead zones (>1s with no animation), element lifecycles, and flags (`offscreen`, `collision`, `invisible`, `paced-fast` <0.2s, `paced-slow` >2s). Scan summaries and flags — fix or justify each. Skip on small edits or when no trusted installed command is available.
