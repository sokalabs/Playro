...
Everything runs through `npx hyperframes` in project-local workflows, or the globally installed pinned `hyperframes@0.4.2` from `scripts/setup.sh`. Requires Node.js >= 22 and FFmpeg. Avoid floating global npm installs or sudo-based npm installs; official setup pins the CLI and disables npm lifecycle scripts during install.
...
Check for and install updates. For official-skill setup, prefer re-running `scripts/setup.sh` so the reviewed CLI version remains pinned. The auto-detect fix shipped in `hyperframes@0.4.2` (commit 4c72ba4, March 2026).
