...
#
# Verifies Node >= 22 and FFmpeg, installs the pinned `hyperframes` CLI
# globally, pre-caches `chrome-headless-shell` with a pinned Puppeteer CLI,
# and runs `hyperframes doctor`.
#
...
MIN_NODE_MAJOR=22
HYPERFRAMES_VERSION="0.4.2"
PUPPETEER_VERSION="24.19.0"
...
bold "==> Installing pinned hyperframes CLI (${HYPERFRAMES_VERSION})"
...
# Install an exact reviewed version instead of a floating tag/range.
# --ignore-scripts prevents package lifecycle hooks from running during install;
# the setup only needs the published CLI files. If npm reports a permissions
# error, use a user-scoped npm prefix rather than sudo.
if ! npm install -g --ignore-scripts --no-audit --fund=false "hyperframes@${HYPERFRAMES_VERSION}" >/dev/null 2>&1; then
  red "✗ npm install -g --ignore-scripts hyperframes@${HYPERFRAMES_VERSION} failed."
  echo "   Avoid sudo for npm installs; it can run package code as root."
  echo "   Use a user-scoped npm prefix instead:"
...
if [ "$installed_version" != "$HYPERFRAMES_VERSION" ]; then
  red "✗ expected hyperframes ${HYPERFRAMES_VERSION}, but found ${installed_version}."
  echo "   Re-run setup after removing conflicting hyperframes installations from PATH."
  exit 1
fi
...
if ! npm_config_ignore_scripts=true npm exec --yes --package "puppeteer@${PUPPETEER_VERSION}" -- puppeteer browsers install chrome-headless-shell >/dev/null 2>&1; then
...
if hyperframes doctor; then
  ...
  echo
  echo "   Scaffold a project:   hyperframes init my-video"
  echo "   Preview:              hyperframes preview"
  echo "   Render:               hyperframes render"
else
