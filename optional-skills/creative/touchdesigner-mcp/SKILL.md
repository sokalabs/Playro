...
Run the setup script with a trusted SHA-256 digest for the `twozero.tox` artifact:

```bash
TWOZERO_TOX_SHA256=<trusted sha256> bash "${HERMES_HOME:-$HOME/.hermes}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Hermes config with `enabled: false` (if missing)
...
3. **Review and explicitly enable the MCP server only when needed:** set `mcp_servers.twozero_td.enabled: true` in `~/.hermes/config.yaml`
4. **Restart Hermes session** to pick up the MCP server
...
- `setup.sh` writes the Hermes MCP entry with `enabled: false`; enable it only for trusted sessions after reviewing this risk.
- `td_execute_python` has unrestricted access to the TD Python environment and filesystem as the TD process user.
- `setup.sh` requires `TWOZERO_TOX_SHA256` before it downloads or accepts `twozero.tox`; it rejects cached/downloaded artifacts that do not match the trusted digest.
- `setup.sh` does not automatically enable `twozero_td` in Hermes config because that would persist trust in an unauthenticated local endpoint. Enable it manually only after verifying TouchDesigner owns `http://localhost:40404/mcp`.
