...
function serveStatic(req, res) {
  ...
  if (!filePath) return;
  if (!fs.existsSync(filePath) && !path.extname(filePath)) filePath = path.join(DIST_ROOT, 'index.html');
...
}).listen(PORT, '127.0.0.1', () => console.log(`GitNexus proxy on http://127.0.0.1:${PORT}`));
```
