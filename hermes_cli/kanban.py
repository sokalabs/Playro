...
``kanban_db``.  This module adds:
  ...
  * A short helper that parses a single CLI slash-style string
    and forwards it to the argparse surface.
"""
...
# ---------------------------------------------------------------------------
# Slash-command entry point (used by local CLI /kanban helpers)
# ---------------------------------------------------------------------------

def run_slash(rest: str) -> str:
    """Execute a ``/kanban …`` string and return captured stdout/stderr.

    ``rest`` is everything after ``/kanban`` (may be empty).  This is
    intentionally a local CLI helper; gateway surfaces must not expose
    the unrestricted board without an authoritative tenant scope.
    """
    import io
    import contextlib
