...
def _print_tui_exit_summary(
    ...
    print(f"  hermes --tui --resume {_quote_shell_arg(target)}")
    if title:
        print(f"  hermes --tui -c {_quote_shell_arg(title)}")
    print()
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    def verify_sha256(path: Path, expected: str) -> None:
        ...
        if actual != expected:
            raise RuntimeError(
                "Downloaded psutil sdist SHA-256 mismatch: "
    ...
    def safe_extract_tar(tar: tarfile.TarFile, destination: Path) -> None:
        ...
        for member in tar.getmembers():
            if member.islnk() or member.issym():
                raise RuntimeError(
                    f"Refusing to extract link from psutil sdist: {member.name}"
                )
            if not (member.isdir() or member.isreg()):
                raise RuntimeError(
                    f"Refusing to extract special file from psutil sdist: {member.name}"
                )

            target = (destination / member.name).resolve()
            if target != destination and destination not in target.parents:
                raise RuntimeError(
                    "Refusing to extract psutil sdist member outside temp dir: "
                    f"{member.name}"
                )
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        verify_sha256(archive, psutil_sha256)
...
def _root_home_dir() -> str:
    """Return root's passwd(5) home without trusting HOME from the environment."""
    try:
        import pwd
...
def _ensure_fhs_path_guard() -> None:
    ...
    where a startup-free bash probe resolves ``hermes``.  Idempotent.
    """
    ...
    # Probe bash's clean default PATH without sourcing .bashrc/.profile. Root's
    # home comes from passwd(5), not the environment, so sudo/su configurations
    # that preserve HOME cannot make this helper trust a user-controlled path.
    home = _root_home_dir()
    try:
