...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    ...
    Refuses symlinked unit homes: a service user's writable tree must not
    redirect root-run ``gateway stop`` / ``restart`` PID and marker files.
    """
    if not system:
        return
    ...
    if not unit_home:
        return
    try:
        home_path = Path(unit_home)
        if home_path.is_symlink():
            print(
                "⚠ Refusing to trust symlinked HERMES_HOME from the systemd unit "
                f"for root gateway operations: {unit_home}"
            )
            return
        if not home_path.is_dir():
            print(
                f"⚠ Refusing non-directory HERMES_HOME from systemd unit: {unit_home}"
            )
            return
    except OSError as exc:
        print(f"⚠ Could not validate systemd HERMES_HOME ({unit_home}): {exc}")
        return
...
def _setup_dingtalk():
    ...
    if not get_env_value("DINGTALK_CLIENT_ID"):
        return

    # ── Sender access policy ──
    print()
    access_choices = [
        "Use pairing approval (recommended)",
        "Allow all DingTalk senders",
        "Only allow listed DingTalk user IDs",
    ]
    access_idx = prompt_choice("  How should DingTalk senders be authorized?", access_choices, 0)
    if access_idx == 0:
        save_env_value("DINGTALK_ALLOW_ALL_USERS", "false")
        save_env_value("DINGTALK_ALLOWED_USERS", "")
        print_success("  Pairing approval enabled for DingTalk.")
        print_info("  Unknown senders can request access; approve with `hermes pairing approve`.")
    elif access_idx == 1:
        save_env_value("DINGTALK_ALLOW_ALL_USERS", "true")
        save_env_value("DINGTALK_ALLOWED_USERS", "")
        print_warning("  Open DingTalk access enabled — anyone who can reach the bot can use it.")
    else:
        allowlist = prompt("  Allowed DingTalk user IDs (comma-separated)", password=False).replace(" ", "")
        save_env_value("DINGTALK_ALLOW_ALL_USERS", "false")
        save_env_value("DINGTALK_ALLOWED_USERS", allowlist)
        print_success("  DingTalk allowlist saved.")
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        if user_openid:
            print_info(
                "  The QR scanner was not added to QQ_ALLOWED_USERS; "
                "approve it through pairing if needed."
            )
    elif access_idx == 1:
