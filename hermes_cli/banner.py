...
def _check_via_rev(local_rev: str) -> Optional[int]:
    ...
    """
    try:
        with tempfile.TemporaryDirectory(prefix="hermes-update-check-") as git_cwd:
            result = subprocess.run(
                ...
                env={
                    **os.environ,
                    # Prevent git from walking above our fresh temporary directory
                    # and loading repository-local config from the caller's CWD.
                    "GIT_CEILING_DIRECTORIES": str(Path(git_cwd).parent),
                },
            )
    except Exception:
        return None
