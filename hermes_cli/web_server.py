...
def _ws_client_is_allowed(ws: "WebSocket") -> bool:
    ...
    WebSocket endpoints expose privileged dashboard functionality and their
    query-string session token is injected into the unauthenticated SPA HTML.
    Keep them loopback-only even when the dashboard is explicitly bound to all
    interfaces with ``--insecure``.
    """
...
# Narrow, channel-bound credentials for PTY-side publishers. These tokens are
# intentionally distinct from the dashboard session token because the sidecar
# URL is passed to the spawned TUI process through its environment. Code/tools
# running in that child may inspect env vars, so a leaked publisher token must
# only authenticate writes to /api/pub for its own channel.
_event_publisher_tokens: dict[str, str] = {}
...
def _publisher_token_for_channel(channel: str) -> str:
    """Return a narrow write-only /api/pub credential for ``channel``."""
    token = _event_publisher_tokens.get(channel)

    if token is None:
        token = secrets.token_urlsafe(32)
        _event_publisher_tokens[channel] = token

    return token


def _publisher_token_is_valid(channel: str, token: str) -> bool:
    """True when ``token`` is the channel-bound /api/pub credential."""
    expected = _event_publisher_tokens.get(channel)
...
def _build_sidecar_url(channel: str) -> Optional[str]:
    ...
    qs = urllib.parse.urlencode(
        {
            "token": _publisher_token_for_channel(channel),
            "channel": channel,
        }
    )
...
# HERMES_TUI_SIDECAR_URL set in /api/pty's PTY env) with a narrow publisher
# credential that is scoped to that channel only. It writes every dispatcher
# emit through it. The dashboard fans those frames out to any subscriber that
# opened /api/events on the same channel id. This is what
...
async def pub_ws(ws: WebSocket) -> None:
    ...
    if not _publisher_token_is_valid(channel, token):
        await ws.close(code=4401)
        return
