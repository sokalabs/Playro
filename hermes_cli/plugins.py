...
EXTRA_ENTRYPOINT_PATHS_ENV = "HERMES_PLUGIN_PYTHONPATH"
...
class PluginManager:
    ...
    def _extra_entrypoint_paths(self) -> List[str]:
        """Return Nix-provided plugin site-packages paths.

        These paths are intentionally not read from ``PYTHONPATH``.  The Nix
        wrapper exposes them through ``HERMES_PLUGIN_PYTHONPATH`` so Python
        does not process them during interpreter startup (for example via
        ``sitecustomize.py``) and they cannot shadow core modules before
        Hermes has applied the ``plugins.enabled`` opt-in gate.
        """
        raw = os.getenv(EXTRA_ENTRYPOINT_PATHS_ENV, "")
        return [part for part in raw.split(os.pathsep) if part]

    def _entry_points_for_plugins(self) -> List[importlib.metadata.EntryPoint]:
        """Return entry points from installed packages and Nix plugin paths."""
        group_eps: List[importlib.metadata.EntryPoint] = []

        eps = importlib.metadata.entry_points()
        # Python 3.12+ returns a SelectableGroups; earlier returns dict
        if hasattr(eps, "select"):
            group_eps.extend(eps.select(group=ENTRY_POINTS_GROUP))
        elif isinstance(eps, dict):
            group_eps.extend(eps.get(ENTRY_POINTS_GROUP, []))
        else:
            group_eps.extend(ep for ep in eps if ep.group == ENTRY_POINTS_GROUP)

        extra_paths = self._extra_entrypoint_paths()
        if extra_paths:
            for dist in importlib.metadata.distributions(path=extra_paths):
                group_eps.extend(
                    ep for ep in dist.entry_points if ep.group == ENTRY_POINTS_GROUP
                )

        return group_eps

    def _activate_extra_entrypoint_paths(self) -> None:
        """Append Nix plugin paths to ``sys.path`` after opt-in checks pass."""
        for path in self._extra_entrypoint_paths():
            if path not in sys.path:
                sys.path.append(path)

    def _scan_entry_points(self) -> List[PluginManifest]:
        """Check ``importlib.metadata`` for pip/Nix-installed plugins."""
        manifests: List[PluginManifest] = []
        try:
            for ep in self._entry_points_for_plugins():
                manifest = PluginManifest(
                    name=ep.name,
                    source="entrypoint",
                    path=ep.value,
    ...
    def _load_entrypoint_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Load a pip/Nix-installed plugin via its entry-point reference."""
        self._activate_extra_entrypoint_paths()
