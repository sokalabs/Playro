...
def _default_windows_editor() -> str:
    ...
    1. The trusted absolute path to ``notepad.exe`` in the Windows system
       directory — notepad ships with every Windows install, has no deps, and
       works as a blocking editor (``subprocess.call([notepad_path, file])``
       blocks until the user closes the window).  Using the absolute system
       path avoids current-directory or ``PATH`` executable hijacking when
       users run Hermes from an untrusted project folder.
    ...
    """
    return _trusted_windows_notepad_path()


def _trusted_windows_notepad_path() -> str:
    """Return ``%SystemRoot%\\System32\\notepad.exe`` via Win32 APIs.

    Do not use ``shutil.which("notepad")`` here: it resolves through search
    paths that can include the application/current directory and user-controlled
    ``PATH`` entries.  ``GetSystemDirectoryW`` points directly at the trusted
    Windows system directory for the running process.
    """
    import ctypes
    ...
    while True:
        ...
        if length < size:
            ...
            if candidate.is_file():
                # Forward slashes keep the absolute Windows path safe for
                # prompt_toolkit's shlex-based editor splitting while still
                # executing the trusted system binary on Windows.
