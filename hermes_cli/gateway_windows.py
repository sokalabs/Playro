...
def _quote_cmd_script_arg(value: str) -> str:
    ...
    if not value:
        return '""'
    if _CMD_METACHAR_RE.search(value) or re.search(r'[ \t"]', value):
        return '"' + value.replace('"', '""') + '"'
    return value


def _quote_cmd_set_value(value: str) -> str:
    """Escape a value embedded in ``set "VAR=value"`` lines."""
    if "\r" in value or "\n" in value:
        raise ValueError(f"refusing to quote value containing newline: {value!r}")
    return value.replace('"', '""')
...
def _build_gateway_cmd_script(
    ...
    lines.append(f'set "HERMES_HOME={_quote_cmd_set_value(hermes_home)}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "HERMES_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports hermes_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={_quote_cmd_set_value(venv_dir)}"')
