...
# They remain configurable/opt-in, but the setup checklist won't pre-select
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        import subprocess
        # Use only the lockfile-installed agent-browser binary so setup
        # never bypasses package-lock integrity pins with an unpinned npx
        # download.
        ...
        if not local_ab.exists():
            from hermes_constants import display_hermes_home
            _print_warning(
                "    agent-browser is not installed locally; not running unpinned npx fallback"
            )
            _print_info(
                f"    Run manually: cd {display_hermes_home()}/hermes-agent && npm install"
            )
            return
        install_cmd = [str(local_ab), "install", "--with-deps"]
        try:
            result = subprocess.run(
                install_cmd,
            ...
            if result.returncode == 0:
                ...
                _print_info("    Run manually: node_modules/.bin/agent-browser install --with-deps")
        ...
        if not shutil.which("curl"):
            ...
            return
        _print_warning("    Automatic cua-driver curl|bash installation is disabled for security.")
        _print_info("    Review upstream instructions, then install manually from:")
        _print_info("      https://github.com/trycua/cua/blob/main/libs/cua-driver/README.md")
        _print_info("    After installation, grant macOS permissions:")
        _print_info("      System Settings > Privacy & Security > Accessibility")
        _print_info("      System Settings > Privacy & Security > Screen Recording")
        ...
        _print_warning("    KittenTTS automatic installation is disabled for security.")
        _print_info(
            "    Install kittentts and soundfile from trusted, pinned artifacts "
            "before selecting this provider."
        )
        _print_info("    Then rerun Hermes setup to use the local KittenTTS provider.")
...
def _get_platform_tools(
    ...
    *,
    ...
    has_platform_toolset_allowlist = isinstance(configured_toolset_names, list)
    ...
    # MCP servers are available by default only for platforms without an
    # explicit platform_toolsets list. Once an operator configures a platform's
    # toolsets, that list is the complete MCP allowlist: include only MCP
    # server names explicitly present there. This keeps restricted messaging
    # platforms from inheriting sensitive globally enabled MCP servers.
    ...
    if include_default_mcp_servers:
        if explicit_mcp_servers or "no_mcp" in toolset_names or has_platform_toolset_allowlist:
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude built-in runtime toolsets and aliases. These entries are
    # resolved at runtime and can expand back into tools the user unchecked
    # (for example hermes-discord, debugging, all, or *). Only unknown
    # passthrough entries such as MCP server names should be preserved.
    from toolsets import TOOLSETS
    platform_default_keys = {p["default_toolset"] for p in PLATFORMS.values()}
    runtime_toolset_keys = set(TOOLSETS) | {"all", "*"} | platform_default_keys
    ...
    # Preserve entries that are not managed by the checklist and are not valid
    # built-in runtime toolsets. This keeps MCP server names and no_mcp-style
    # sentinels, while preventing hidden presets or aliases from bypassing the
    # user's restricted selection.
    preserved_entries = {
        entry for entry in existing_toolsets
        if entry not in configurable_keys and entry not in runtime_toolset_keys
    }
    # Preserve the "no_mcp" sentinel when it already exists. It is an explicit
    # per-platform MCP opt-out; dropping it during an unrelated built-in
    # toolset save would silently expose globally enabled MCP servers.
...
_POST_SETUP_INSTALLED: dict = {
    ...
    #
    # Only entries here are gated; other post_setup hooks (piper,
