...
def _install_kittentts_deps() -> bool:
    """Decline automatic KittenTTS installs that cannot verify all artifacts."""
    print()
    print_warning("KittenTTS automatic installation is disabled for security.")
    print_info(
        "Install kittentts and soundfile from trusted, pinned artifacts "
        "before selecting this provider."
    )
    print_info("Then rerun Hermes setup to use the local KittenTTS provider.")
    return False
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        if already_installed:
            ...
            _install_kittentts_deps()
            print_warning("KittenTTS is not installed. Falling back to Edge TTS.")
            selected = "edge"
