...
from hermes_cli.stdio import _default_windows_editor
...
CONTAINER_MODE_SYSTEM_FILE = Path("/etc/hermes-agent/container-mode")
_CONTAINER_MODE_TRUSTED_UID = 0
_ALLOWED_CONTAINER_BACKENDS = frozenset({"docker", "podman"})
_CONTAINER_NAME_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9_.-]{0,127}$")
_CONTAINER_USER_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_.-]{0,31}$")
_CONTAINER_HERMES_BIN = "/data/current-package/bin/hermes"


def _metadata_path_is_secure(path: Path) -> bool:
    """Return True when container routing metadata cannot be replaced by peers."""
    try:
    ...
    if stat.S_ISLNK(file_stat.st_mode):
        return False
    if file_stat.st_uid != _CONTAINER_MODE_TRUSTED_UID:
        return False
    if file_stat.st_mode & (stat.S_IWGRP | stat.S_IWOTH):
        return False

    # Every ancestor must also be root-owned and non-writable by group/others.
    # This rejects the legacy HERMES_HOME/.container-mode location when
    # HERMES_HOME points at the shared 2770 service state directory.
    for parent in [path.parent, *path.parents]:
        ...
        if not stat.S_ISDIR(parent_stat.st_mode):
            return False
        if parent_stat.st_uid != _CONTAINER_MODE_TRUSTED_UID:
            return False
...
def _read_container_mode_file(path: Path) -> Optional[dict]:
    if not _metadata_path_is_secure(path):
        return None
    ...
    try:
        ...
        if (
            file_stat.st_uid != _CONTAINER_MODE_TRUSTED_UID
...
def _validate_container_exec_info(info: dict) -> Optional[dict]:
    backend = info.get("backend", "docker")
    container_name = info.get("container_name", "hermes-agent")
    exec_user = info.get("exec_user", "hermes")
    hermes_bin = info.get("hermes_bin", _CONTAINER_HERMES_BIN)

    if backend not in _ALLOWED_CONTAINER_BACKENDS:
        return None
    if not _CONTAINER_NAME_RE.fullmatch(container_name):
        return None
    if exec_user == "root" or not _CONTAINER_USER_RE.fullmatch(exec_user):
        return None
    if hermes_bin != _CONTAINER_HERMES_BIN:
        return None
...
def get_container_exec_info() -> Optional[dict]:
    """Read trusted container mode metadata for NixOS container routing.

    Returns a dict with keys: backend, container_name, exec_user, hermes_bin
    or None if container mode is not active, we're already inside the
    container, HERMES_DEV=1 is set, or metadata fails security checks.

    The NixOS activation script writes metadata to a root-owned path outside
    shared Hermes state. Legacy HERMES_HOME/.container-mode files are only
    accepted if their ownership and ancestors are equivalently root-controlled.
    """
    ...
    candidates = [CONTAINER_MODE_SYSTEM_FILE, get_hermes_home() / ".container-mode"]
    seen = set()
    for container_mode_file in candidates:
        if container_mode_file in seen:
            continue
        seen.add(container_mode_file)
        ...
        validated = _validate_container_exec_info(info)
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # skipped. When this list is empty, Hermes does not source user
        # shell startup files unless ``auto_source_bashrc`` is explicitly
        # enabled.
        "shell_init_files": [],
        # When true, Hermes sources the user's shell rc files
        # (``~/.profile``, ``~/.bash_profile``, ``~/.bashrc``) in the
        # login shell used to build the environment snapshot. This can
        # capture PATH additions, shell functions, and aliases — which a
        # plain ``bash -l -c`` would otherwise miss — but it may also
        # capture secrets exported by those files. Keep disabled unless
        # you trust the rc files and need this compatibility behavior.
    ...
    "delegation": {
        ...
        # When delegate_task narrows child toolsets explicitly, do not preserve
        # parent MCP toolsets unless this setting is enabled. Keep this off by
        # default so an explicit toolsets=[...] list remains a strict
    ...
    "discord": {
        ...
        # Default (empty) = all non-destructive actions allowed (subject to bot
        # privileged intents). Destructive actions such as delete_message require
        # explicitly adding them to this setting. Accepts comma-separated string
        # ("list_guilds,list_channels,fetch_messages") or YAML list. Unknown
        # names are dropped with a warning at load time.
        # Actions: list_guilds, server_info, list_channels, channel_info,
        # list_roles, member_info, search_members, fetch_messages, list_pins,
        # pin_message, unpin_message, delete_message, create_thread, add_role,
        # remove_role.
        "server_actions": "",
    },
    ...
    "code_execution": {
        # Execution mode:
        #   strict (default) — scripts run in a temporary staging directory with
        #     hermes-agent's own python (sys.executable). Project-relative paths
        #     do not resolve unless explicitly accessed through allowed tools.
        #   project          — opt-in compatibility mode: scripts run in the
        #     session's working directory with the active virtualenv/conda env's
        #     python, so project deps and relative paths resolve.
...
OPTIONAL_ENV_VARS = {
    ...
    "GATEWAY_PROXY_CONTEXT_KEY": {
        "description": (
            "Trusted proxy-context token sent as X-Hermes-Gateway-Proxy-Key "
            "in proxy mode. Must match API_SERVER_GATEWAY_PROXY_KEY on the "
            "remote host before hermes_gateway_context is honored."
        ),
        "prompt": "Remote gateway context trust key",
        "url": None,
        "password": True,
        "category": "messaging",
        "advanced": True,
    },
    "API_SERVER_GATEWAY_PROXY_KEY": {
        "description": (
            "Trusted proxy-context token required before the API server honors "
            "hermes_gateway_context tool policy or session scope from request "
            "bodies. Must match GATEWAY_PROXY_CONTEXT_KEY on the proxy gateway."
        ),
        "prompt": "API server trusted gateway context key",
        "url": None,
        "password": True,
        "category": "messaging",
        "advanced": True,
...
def _ignore_user_config() -> bool:
    """True when ``hermes chat --ignore-user-config`` suppresses user YAML."""
...
def load_config() -> Dict[str, Any]:
    ...
    with _CONFIG_LOCK:
        ...
        if cache_key is not None and not _ignore_user_config():
            try:
                with open(config_path, encoding="utf-8") as f:
...
def edit_config():
    ...
    if not editor:
        # Try common editors. On Windows, only use the trusted absolute
        # system Notepad path; do not fall back to bare executable names that
        # can be hijacked through current-directory or PATH lookup. On POSIX,
        # prefer nano/vim over code/notepad because it's more likely to be
        # present on headless / server systems.
        import shutil
        import sys as _sys
        if _sys.platform == "win32":
            editor = _default_windows_editor()
            candidates = []
        else:
...
def _is_approval_bypass_config_set(key: str, value: str) -> bool:
    """Return True when a config-set request would persist approval bypass.

    Persistent approval bypass is intentionally not writable through
    ``hermes config set`` because that command can be executed by the agent
    during a session. Users who want persistent bypass must make an explicit
    out-of-band edit to their config file instead.
    """
    return key.strip().lower() == "approvals.mode" and value.strip().lower() in {
        "false",
        "no",
        "off",
    }
...
def _parse_terminal_docker_env_value(value: Any) -> Dict[str, str]:
    if isinstance(value, dict):
        ...
        try:
            ...
            try:
                ...
                raise ValueError("terminal.docker_env must be a JSON/YAML object") from exc
    else:
        raise ValueError("terminal.docker_env must be a JSON/YAML object")
    ...
    if invalid:
        raise ValueError("terminal.docker_env requires string keys and string values")
    return dict(parsed)
...
def set_config_value(key: str, value: str):
    ...
    if is_managed():
        ...
        return
    if _is_approval_bypass_config_set(key, value):
        print(
            "Refusing to persistently disable command approvals via "
            "`hermes config set`. For a trusted local session, start Hermes "
            "yourself with --yolo or edit config.yaml manually outside the agent."
        )
        return
    ...
    if key == "terminal.docker_env":
        value = _parse_terminal_docker_env_value(value)
    ...
    if key == "terminal.docker_env":
        ...
        write_docker_env(value if isinstance(value, dict) else {})
