...
def _assert_inside_staged_root(staged_root: Path, path: Path) -> None:
    """Reject paths that resolve outside the staged distribution root."""
    root = staged_root.resolve()
    resolved = path.resolve(strict=False)
    try:
        resolved.relative_to(root)
    except ValueError as exc:
        rel = path.relative_to(staged_root) if path.is_relative_to(staged_root) else path
        raise DistributionError(
            f"Refusing distribution path outside staged root: {rel}"
        ) from exc


def _validate_staged_entry(staged_root: Path, entry: Path) -> None:
    """Validate one staged filesystem entry before it can be inspected/copied.

    Git repositories can contain symlinks, and local-directory sources can also
    contain device nodes, FIFOs, or sockets.  The installer must not follow or
    copy any of them: a malicious ``SOUL.md -> /home/user/.ssh/id_rsa`` would be
    dereferenced by shutil's default copy helpers and turn into a regular file
    in the installed profile.
    """
    rel = entry.relative_to(staged_root)
    try:
        mode = entry.lstat().st_mode
    except OSError as exc:
        raise DistributionError(f"Cannot inspect distribution path {rel}: {exc}") from exc

    if stat.S_ISLNK(mode):
        raise DistributionError(f"Refusing symlink in profile distribution: {rel}")
    _assert_inside_staged_root(staged_root, entry)
    if not (stat.S_ISDIR(mode) or stat.S_ISREG(mode)):
        raise DistributionError(
            f"Refusing special file in profile distribution: {rel}"
        )


def _validate_staged_tree(staged: Path) -> None:
    """Ensure a staged distribution contains only regular files/directories."""
    staged = staged.resolve()
    _validate_staged_entry(staged, staged)
    stack = [staged]
    while stack:
        current = stack.pop()
        for entry in current.iterdir():
            _validate_staged_entry(staged, entry)
            if entry.is_dir():
                stack.append(entry)
