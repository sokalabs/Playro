...
Toolsets = disabled by default for script safety; explicit --toolsets opts in
to tool use for callers that trust the prompt source.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = non-interactive deny for dangerous local commands; no YOLO bypass is set.
...
def run_oneshot(
    ...
    """
    # Silence every stdlib logger only for this invocation.  AIAgent, tools,
    # and provider adapters can log to stderr through the root logger; stdout
    # and stderr are also redirected below so no bytes reach the terminal.
    ...
    # Script-oriented prompts are commonly assembled from untrusted content.
    # Keep the default one-shot surface tool-free; callers that trust the
    # prompt can still opt in with --toolsets.  "all" / "*" intentionally
    # keeps the AIAgent None sentinel that means all available toolsets.
    agent_toolsets = [] if normalized_toolsets is None else explicit_toolsets
    use_config_toolsets = False

    # Mark this run as non-interactive one-shot without enabling YOLO or hook
    # auto-accept.  The approval layer uses this to deny dangerous local
    # commands instead of prompting or silently approving.
    previous_oneshot = os.environ.get("HERMES_ONESHOT_SESSION")
    os.environ["HERMES_ONESHOT_SESSION"] = "1"
    ...
    try:
        ...
        if previous_oneshot is None:
            os.environ.pop("HERMES_ONESHOT_SESSION", None)
        else:
            os.environ["HERMES_ONESHOT_SESSION"] = previous_oneshot
        logging.disable(previous_logging_disable)
        try:
            devnull.close()
        except Exception:
...
def _run_agent(
    ...
    agent = AIAgent(
        ...
        #   - shell-hook approval → no auto-accept; non-tty fallback denies
        #   - dangerous-command approval → HERMES_ONESHOT_SESSION blocks
        #                dangerous local commands without prompting
