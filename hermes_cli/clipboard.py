...
# Native Windows uses an absolute, trusted PowerShell path only.  Never run
# bare executable names here: Windows process creation can search the current
# working directory before system locations, enabling binary planting.
# Discovery is cached per-process.
_WINDOWS_POWERSHELL_RELATIVE = (
    "System32", "WindowsPowerShell", "v1.0", "powershell.exe"
)
_WINDOWS_PWSH_RELATIVE = ("PowerShell", "*", "pwsh.exe")


def _get_windows_directory() -> str:
    """Return the Windows install directory for trusted system binaries."""
    if sys.platform == "win32":
        try:
            buf = ctypes.create_unicode_buffer(260)
            length = ctypes.windll.kernel32.GetWindowsDirectoryW(buf, len(buf))
            if 0 < length < len(buf):
                return buf.value
        except Exception as e:
            logger.debug("GetWindowsDirectoryW failed: %s", e)

    for env_name in ("SystemRoot", "WINDIR"):
        root = os.environ.get(env_name)
        if root and ntpath.isabs(root):
            return root
    return r"C:\Windows"
...
def _is_under_windows_dir(path: str, base_dir: str) -> bool:
    try:
        return (
            ntpath.commonpath([
                _normalize_windows_path(path),
                _normalize_windows_path(base_dir),
            ])
            == _normalize_windows_path(base_dir)
        )
    except ValueError:
...
def _trusted_powershell_candidates() -> list[str]:
    """Return trusted absolute PowerShell paths for native Windows use."""
    candidates: list[str] = []

    windows_dir = _get_windows_directory()
    if ntpath.isabs(windows_dir):
        candidates.append(ntpath.join(windows_dir, *_WINDOWS_POWERSHELL_RELATIVE))
    ...
    for env_name in ("ProgramFiles", "ProgramW6432", "ProgramFiles(x86)"):
        ...
        if not root or not ntpath.isabs(root):
            continue
        trusted_base = ntpath.join(root, "PowerShell")
        normalized_base = _normalize_windows_path(trusted_base)
        if normalized_base in seen_roots:
            continue
        seen_roots.add(normalized_base)

        pattern = ntpath.join(root, *_WINDOWS_PWSH_RELATIVE)
        for path in sorted(glob.glob(pattern), reverse=True):
            if _is_under_windows_dir(path, trusted_base):
...
def _find_powershell() -> str | None:
    """Return the first trusted absolute PowerShell executable, or None."""
    for exe in _trusted_powershell_candidates():
        try:
