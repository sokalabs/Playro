...
def _is_source_code_path(path: str | os.PathLike[str] | None) -> bool:
    """Return True only for paths that are clearly source code files.

    ``redact_sensitive_text(..., code_file=True)`` intentionally skips ENV and
    JSON secret redaction to avoid false positives while reading source code.
    Generic file tools can also read dotenv/config/service-account files, so the
    opt-in must stay conservative and path-based.
    """
    if not path:
        return False

    suffix = Path(str(path)).suffix.lower()
    return suffix in _SOURCE_CODE_EXTENSIONS
...
def _get_file_ops(task_id: str = "default") -> ShellFileOperations:
    ...
    Note: registered delegate_task child task_ids are routed through
    ``_resolve_container_task_id`` to the parent's container and cached
    file_ops. Unrelated task/session IDs and RL/benchmark task_ids keep
    their isolation.
    """
    ...
    try:
        ...
        if result.content:
            result.content = redact_sensitive_text(
                result.content,
                code_file=_is_source_code_path(path),
            )
            ...
            for m in result.matches:
                if hasattr(m, 'content') and m.content:
                    m.content = redact_sensitive_text(
                        m.content,
                        code_file=_is_source_code_path(getattr(m, 'path', None)),
                    )
        result_dict = result.to_dict()
