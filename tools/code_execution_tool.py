...
Platform: Linux / macOS use Unix domain sockets for local execution. Windows
uses an authenticated loopback TCP fallback. Remote execution additionally
...
# Availability gate. On Windows, execute_code uses an authenticated
# loopback TCP fallback for local sandbox RPC because AF_UNIX is unreliable
# on Windows Python. See ``_use_tcp_rpc`` in ``_execute_local`` below.
...
def _resolve_sandbox_tools(enabled_tools: Optional[List[str]]) -> frozenset:
    """Return the execute_code tool allow-list for a session.

    ``enabled_tools=None`` means no caller-specific tool list was provided, so
    preserve the historical default of exposing all sandbox-safe tools.  Any
    concrete iterable, including an empty list or a list containing only
    ``execute_code``, is an explicit session policy and must not be widened.
    """
...
# match either a safe prefix or, on Windows, an OS-essential name.  Do not
# allow the broad HERMES_ namespace here: execute_code runs LLM-generated
# scripts, and Hermes control variables such as HERMES_HOME can reveal local
# credential stores. Runtime-only Hermes vars (for example HERMES_RPC_SOCKET)
# are injected explicitly after scrubbing.
...
def _path_is_relative_to(path: str, parent: str) -> bool:
    """Return True when ``path`` is contained by ``parent`` after normalization."""
    try:
...
def _sanitize_existing_pythonpath(existing_pythonpath: str, hermes_root: str) -> List[str]:
    """Remove Hermes checkout entries from an inherited PYTHONPATH.

    execute_code child scripts are only supposed to call tools through the
    generated hermes_tools RPC module.  Inheriting entries that point at this
    repository would make internal dispatcher modules importable and bypass
    the RPC allow-list/tool-count controls.  Empty and relative entries are
    also dropped because they resolve against the child process CWD.
    """
    if not existing_pythonpath:
        return []

    hermes_root_real = os.path.realpath(hermes_root)
    sanitized = []
    for entry in existing_pythonpath.split(os.pathsep):
        if not entry or not os.path.isabs(os.path.expanduser(entry)):
            continue
        entry_real = os.path.realpath(os.path.expanduser(entry))
        if _path_is_relative_to(entry_real, hermes_root_real):
            continue
        sanitized.append(entry)
    return sanitized


def _build_child_pythonpath(tmpdir: str, existing_pythonpath: str, hermes_root: str) -> str:
    """Build the PYTHONPATH for the execute_code child process."""
    return os.pathsep.join([
        tmpdir,
        *_sanitize_existing_pythonpath(existing_pythonpath, hermes_root),
    ])
...
def _rpc_server_loop(
    ...
    """
    Accept one authenticated client connection and dispatch tool-call
    requests until the client disconnects or the call limit is reached.
    """
    ...
    try:
        ...
        while True:
            ...
            while b"\n" in buf:
                ...
                if not secrets.compare_digest(str(request.get("token", "")), rpc_token):
                    resp = json.dumps({"error": "Invalid RPC authentication token."})
                    conn.sendall((resp + "\n").encode())
                    return
...
def execute_code(
    ...
    try:
        ...
        # sandbox_tools is already the correct set (all sandbox tools only when
        # enabled_tools was omitted; otherwise the session-policy intersection).
        ...
        #   port.  Loopback is host-local but not user-scoped, so every RPC
        #   request must include the per-execution HERMES_RPC_TOKEN from the
        #   child environment.  HERMES_RPC_SOCKET is set to
        #   ``tcp://127.0.0.1:<port>`` which the generated client parses to
        #   pick AF_INET.
        if _use_tcp_rpc:
        ...
        # Prepend only the staging tmpdir so ``from hermes_tools import ...``
        # resolves when the subprocess CWD is not tmpdir (project mode).
        # Do not expose the Hermes checkout itself: child scripts must reach
        # tools only through the generated RPC stubs, which enforce the
        # execute_code allow-list, tool-call limit, and terminal restrictions.
        _hermes_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        child_env["PYTHONPATH"] = _build_child_pythonpath(
            tmpdir,
            _existing_pp,
            _hermes_root,
        )
        ...
        #   - strict : default behavior (sys.executable + tmpdir CWD).
        #   - project: opt-in compatibility mode using the user's venv python
        #              and session working directory.
        ...
        # The environment filter blocks direct os.environ access to common secret
        # names, but scripts may still read sensitive files from disk. This
        # ensures leaked secrets never enter the model context.
        from agent.redact import redact_sensitive_text
        stdout_text = redact_sensitive_text(stdout_text)
...
def _kill_process_group(proc, escalate: bool = False):
    ...
    from utils import kill_posix_process_tree

    if _IS_WINDOWS:
        try:
            proc.terminate()
        except Exception as exc:
            logger.debug("Could not terminate process: %s", exc, exc_info=True)
            try:
                proc.kill()
    ...
    kill_posix_process_tree(proc.pid, escalate=escalate)
    if escalate:
        try:
            proc.wait(timeout=5)
        except subprocess.TimeoutExpired:
            try:
                ...
                logger.debug("Could not kill process after group kill: %s", exc, exc_info=True)
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      - ``strict`` (default): scripts run in a temporary staging directory with
        ...
        won't resolve.
      - ``project``: opt-in compatibility mode where scripts run in the
        session's working directory with the active virtual environment's
        python, so project dependencies and files resolve naturally.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    ``mode`` controls the working-directory sentence in the description:
      ...
      - ``'project'``: opt-in compatibility mode that runs scripts in the
        session's CWD with the active venv's python
    ...
    # Mode-specific CWD guidance. Strict mode is the default and keeps
    # generated scripts out of the project/session CWD. Project mode remains
    # available as an explicit compatibility setting.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "to non-sensitive user files, or terminal()/read_file() for inspecting project files."
        )
    else:
        cwd_note = (
