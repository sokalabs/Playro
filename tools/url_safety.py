...
def _canonical_ip_for_policy(
    ip: ipaddress.IPv4Address | ipaddress.IPv6Address,
) -> ipaddress.IPv4Address | ipaddress.IPv6Address:
    """Return the address form used for SSRF policy comparisons.

    IPv4-mapped IPv6 literals (for example ``::ffff:169.254.169.254``)
    must be evaluated as their embedded IPv4 address so exact IPv4 sentinels
    and IPv4 networks cannot be bypassed by changing the textual address
    family.
    """
...
def _is_always_blocked_ip(
    ...
    """Return True if an IP is in the non-bypassable metadata floor."""
    policy_ip = _canonical_ip_for_policy(ip)
    return policy_ip in _ALWAYS_BLOCKED_IPS or any(
        policy_ip in net for net in _ALWAYS_BLOCKED_NETWORKS
    )
...
def _is_blocked_ip(ip: ipaddress.IPv4Address | ipaddress.IPv6Address) -> bool:
    ...
    policy_ip = _canonical_ip_for_policy(ip)
    if (
        policy_ip.is_private
        or policy_ip.is_loopback
        or policy_ip.is_link_local
        or policy_ip.is_reserved
    ):
        return True
    if policy_ip.is_multicast or policy_ip.is_unspecified:
        return True
    # CGNAT range not covered by is_private
    if policy_ip in _CGNAT_NETWORK:
        return True
    return False
