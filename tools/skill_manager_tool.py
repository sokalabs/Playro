...
# agent-created skills are scanned by default and can only be disabled explicitly.
try:
...
def _require_skill_mutation_approval(action: str, name: str, file_path: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """Require explicit user approval before mutating an existing skill.

    Skill files are persistent procedural instructions loaded in future
    sessions. Tool output, web pages, and project files can be
    attacker-controlled, so an autonomous model decision is not sufficient
    authorization to patch/edit a skill.
    """
    try:
        from tools.skill_provenance import get_current_write_origin, is_background_review
        if is_background_review():
            return {
                "success": False,
                "error": (
                    "BLOCKED: Background skill review cannot mutate persistent "
                    "skills without explicit user approval. Summarize the "
                    "proposed change for the user instead."
                ),
            }
        # Direct library/CLI callers that are not executing inside an agent
        # tool loop keep the historical behavior. AIAgent binds this context
        # to "assistant_tool" before model-requested tool calls, which is
        # the trust boundary this approval gate protects.
        if get_current_write_origin() == "foreground":
            return None
    except Exception:
        pass

    target = "SKILL.md" if not file_path else file_path
    command = f"skill_manage action={action} name={name} target={target}"
    description = (
        "Persistent skill mutation requires explicit user approval because "
        "skills are loaded as future procedural instructions"
    )
    try:
        ...
        logger.warning("Skill mutation approval check failed: %s", exc, exc_info=True)
        return {
            "success": False,
            "error": "BLOCKED: Could not obtain explicit user approval for skill mutation.",
        }

    if approval.get("approved") is True:
        return None

    error = approval.get("message") or "BLOCKED: Skill mutation requires explicit user approval."
    result = {"success": False, "error": error}
    for key in ("status", "command", "description", "pattern_key", "pattern_keys"):
        if key in approval:
...
def _guard_agent_created_enabled() -> bool:
    ...
    Agent-created skills are persisted and may be loaded in future sessions,
    so scan them by default and block dangerous findings before they become
    durable prompt context. Users may explicitly opt out with
...
def _background_review_skill_writes_enabled() -> bool:
    """Return True only when config explicitly allows review-fork skill writes.

    Background review runs automatically over conversation history, which may
    contain prompt-injected tool output or hostile user text. Keep persistent
    skill mutation off by default; foreground/user-directed skill_manage calls
    are unaffected.
    """
    try:
        from hermes_cli.config import load_config
        cfg = load_config()
        return is_truthy_value(
            cfg_get(cfg, "skills", "allow_background_review_writes"),
            default=False,
        )
    except Exception:
        return False
...
def _background_review_write_blocked() -> bool:
    """True when the current tool call is an automatic review-fork write."""
    try:
        from tools.skill_provenance import is_background_review
        return is_background_review() and not _background_review_skill_writes_enabled()
    except Exception:
        return False
...
def _archive_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    absorbed_err = _validate_absorbed_into(name, absorbed_into)
    if absorbed_err:
        return {"success": False, "error": absorbed_err}
    ...
    if ok and absorbed_into is not None and isinstance(absorbed_into, str) and absorbed_into.strip():
        result["absorbed_into"] = absorbed_into.strip()
    return result
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    absorbed_err = _validate_absorbed_into(name, absorbed_into, verb="deleted")
    if absorbed_err:
        return {"success": False, "error": absorbed_err}
...
def skill_manage(
    ...
    """
    if _background_review_write_blocked():
        return tool_error(
            "Background review cannot mutate skills by default. "
            "Review-fork skill writes run over untrusted conversation history; "
            "ask the foreground user to confirm the skill change, or explicitly "
            "enable skills.allow_background_review_writes if you accept that risk.",
            success=False,
        )
    ...
    if action == "create":
        ...
        approval_error = _require_skill_mutation_approval(action, name)
        if approval_error:
        ...
        approval_error = _require_skill_mutation_approval(action, name, file_path)
        if approval_error:
            return json.dumps(approval_error, ensure_ascii=False)
    ...
    if result.get("success"):
        ...
        try:
            ...
            if action == "create":
                ...
                # archive_skill updates lifecycle state; keep the usage record so
                # restore/list-archived can report the archived skill.
                pass
            elif action == "delete":
                forget(name)
        except Exception:
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "archive (recoverable move to .archive/), delete, write_file, remove_file.\n\n"
        "On archive/delete, pass `absorbed_into=<umbrella>` when you're merging this "
        ...
        "exist — create/patch the umbrella first, then archive/delete.\n\n"
        "Create when: complex task succeeded (5+ calls), errors overcome, "
        "user-corrected approach worked, non-trivial workflow discovered, "
        "or user asks you to remember a procedure.\n"
        "Update when: instructions stale/wrong, OS-specific failures, "
        "missing steps or pitfalls found during use. "
        "If you used a skill and hit issues not covered by it, propose a patch and request explicit user approval before calling skill_manage.\n\n"
        "After difficult/iterative tasks, offer to save as a skill. "
        "Skip for simple one-offs. Confirm with user before creating/updating/archiving/deleting.\n\n"
        "Good skills: trigger conditions, numbered steps with exact commands, "
        "pitfalls section, verification steps. Use skill_view() to see format examples.\n\n"
        "Pinned skills are protected from archival/deletion — skill_manage(action='archive') or skill_manage(action='delete') "
        "will refuse with a message pointing the user to `hermes curator unpin <name>`. "
        "Patches and edits go through on pinned skills so you can still improve them as "
        "pitfalls come up; pin guards against removal, not content evolution."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "absorbed_into": {
                "type": "string",
                "description": (
                    "For 'archive' or 'delete' — declares intent so the curator can "
