...
_WINDOWS_CMD_META_CHARS = set('^&|<>()%!')


def _escape_windows_cmd_placeholder(value: str) -> str:
    """Escape a placeholder value for cmd.exe shell parsing."""
    escaped = []
    for char in value:
        if char == '"':
            escaped.append('^"')
        elif char in _WINDOWS_CMD_META_CHARS:
...
def _quote_command_tts_placeholder(value: str, quote_context: Optional[str]) -> str:
    ...
    if os.name == "nt":
        escaped = _escape_windows_cmd_placeholder(value)
        if quote_context == '"':
            return escaped
        return f'"{escaped}"'
    if quote_context == "'":
...
def _safe_tts_output_path(output_path: str) -> Path:
    """Restrict model-controlled TTS output paths to the voice-memos directory."""
    requested = Path(output_path).expanduser()
    out_root = Path(DEFAULT_OUTPUT_DIR).expanduser().resolve()
    resolved = requested.resolve() if requested.is_absolute() else (out_root / requested).resolve()
    try:
        resolved.relative_to(out_root)
    except ValueError as exc:
        raise ValueError(
            "output_path must stay inside the Playro/Hermes voice-memos directory"
        ) from exc
    return resolved
...
def text_to_speech_tool(
    ...
    if output_path:
        try:
            file_path = _safe_tts_output_path(output_path)
        except ValueError as exc:
