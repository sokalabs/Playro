...
@contextmanager
def isolated_env_passthrough() -> Iterator[None]:
    """Run a gateway turn with a fresh env passthrough registry.

    Executor worker threads may be reused across gateway sessions.  Replacing
    the ContextVar value at both entry and exit prevents skill registrations
    from a previous job in that worker context from leaking into this turn or
    being restored for a later turn.
    """
    _allowed_env_vars_var.set(set())
    try:
        yield
