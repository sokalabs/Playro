...
Spawns child AIAgent instances with isolated context and restricted toolsets.
Children keep their own task IDs for tracking while terminal/file operations
share the parent task's sandbox. Supports single-task and batch (parallel)
modes. The parent blocks until all children complete.

Each child gets:
  - A fresh conversation (no parent history)
  - Its own task_id for tracking, routed to the parent's terminal sandbox
...
from tools.terminal_tool import (
    register_subagent_container_parent as _register_subagent_container_parent,
    set_approval_callback as _set_subagent_approval_cb,
    unregister_subagent_container_parent as _unregister_subagent_container_parent,
)
...
# Toolsets whose resolved tools include child-forbidden capabilities.  These
# are passed as disabled_toolsets to AIAgent so aggregate toolsets such as
# hermes-cli cannot reintroduce blocked individual tools during resolution.
...
def _strip_blocked_tools(toolsets: List[str]) -> List[str]:
    """Remove explicitly blocked child toolsets by name."""
    return [t for t in toolsets if t not in DELEGATE_BLOCKED_TOOLSETS]


def _delegate_disabled_toolsets_for_role(role: str) -> List[str]:
    """Return toolsets that must be subtracted after child toolset expansion.

    Name-level stripping is not enough for aggregate toolsets (for example,
    ``hermes-cli`` expands to ``memory``, ``execute_code``, and
    ``delegate_task``).  AIAgent applies ``disabled_toolsets`` after resolving
    enabled toolsets, which gives delegated children a hard post-expansion
    denylist.  Orchestrator children are the only exception for delegation:
    role grants them ``delegate_task`` intentionally, while every other
    blocked child capability remains denied.
    """
    blocked = set(DELEGATE_BLOCKED_TOOLSETS)
    if role == "orchestrator":
        blocked.discard("delegation")
...
def _build_child_agent(
    ...
    if toolsets is not None:
        ...
        # An explicit empty list means the child receives no tools. This is
        # important for prompt-injection-sensitive review tasks that pass
        # untrusted code or diff text to a subagent.
    ...
    child_disabled_toolsets = sorted(
        set(parent_disabled_toolsets)
        | set(_delegate_disabled_toolsets_for_role(effective_role))
    )
...
def _run_single_child(
    ...
    **_kwargs,
    ...
    try:
        ...
        _register_subagent_container_parent(child_task_id, parent_task_id)
        ...
        # Preserve ContextVar-based approval/session routing (for example ACP's
        # interactive approval flag) when the child runs in a fresh executor
        # thread. ThreadPoolExecutor workers do not inherit contextvars by
        # default, even though they do receive the TLS approval callback below.
        ...
        try:
            _unregister_subagent_container_parent(child_task_id)
...
def delegate_task(
    ...
    try:
        for i, t in enumerate(task_list):
            # ACP transport command/args are intentionally resolved only from
            # trusted runtime/provider configuration. Do not honor top-level or
            # per-task tool-call values here: delegate_task arguments are
            # model-controllable, and ACP transports spawn local subprocesses.
            #
            # Keep the deprecated acp_command/acp_args function parameters for
            # compatibility, but ignore them as untrusted input.
            ...
            child = _build_child_agent(
                ...
                toolsets=t.get("toolsets") if "toolsets" in t else toolsets,
                model=creds["model"],
                ...
                # ACP subprocess commands are operator-controlled runtime
                # configuration only. Do not honor model/tool-supplied
                # acp_command/acp_args values from delegate_task arguments.
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if configured_base_url:
        ...
        if not api_key:
            parent_base = getattr(parent_agent, "base_url", None) or ""
            if base_url_hostname(configured_base_url) != base_url_hostname(parent_base):
                raise ValueError(
                    "delegation.base_url points to a different host than the parent agent; "
                    "set delegation.api_key explicitly instead of inheriting the parent's key."
                )
        ...
        "properties": {
            ...
            "toolsets": {
                ...
                "description": (
                    ...
                    "Default: inherits your enabled toolsets; pass [] for no tools. "
...
registry.register(
    ...
    handler=lambda args, **kw: delegate_task(
        ...
        # ACP commands are intentionally not read from tool arguments;
        # subprocess-backed transports must come from operator config.
        role=args.get("role"),
