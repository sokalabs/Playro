...
def _validate_skill_name_segment(value: str, *, field: str) -> Optional[str]:
    """Reject path traversal or unsafe characters in skill name components."""
    if not value or value in {".", ".."}:
        return f"Invalid {field}: empty or reserved name"
    if "/" in value or "\\" in value or ".." in value:
        return f"Invalid {field}: path separators are not allowed"
    if any(ord(ch) < 32 or ord(ch) == 127 for ch in value):
        return f"Invalid {field}: control characters are not allowed"
    return None


def _validate_skill_name_path(value: str, *, field: str) -> Optional[str]:
    """Validate slash-separated skill names such as ``mlops/axolotl``."""
    if not value:
        return f"Invalid {field}: empty or reserved name"
    rel = Path(value)
    if rel.is_absolute() or value.startswith(("/", "\\")):
        return f"Invalid {field}: absolute paths are not allowed"
    if "\\" in value:
        return f"Invalid {field}: backslash path separators are not allowed"
    parts = value.split("/")
    if any(part == "" for part in parts):
        return f"Invalid {field}: empty path segments are not allowed"
    for part in parts:
        error = _validate_skill_name_segment(part, field=field)
        if error:
            return error
    return None


def _resolve_skill_under_root(search_dir: Path, relative: str) -> Optional[Path]:
    """Resolve *relative* under *search_dir* or return None when it escapes."""
    rel = Path(relative)
...
def skill_view(
    ...
    try:
        ...
        if ":" in name:
            ...
            if bare:
                segment_error = _validate_skill_name_segment(bare, field="skill name")
                if segment_error:
                    return json.dumps({"success": False, "error": segment_error}, ensure_ascii=False)
                namespace_error = _validate_skill_name_segment(namespace, field="skill namespace")
                if namespace_error:
                    return json.dumps({"success": False, "error": namespace_error}, ensure_ascii=False)
        ...
        name_error = _validate_skill_name_path(name, field="skill name")
        if name_error and ":" not in name:
            return json.dumps({"success": False, "error": name_error}, ensure_ascii=False)

        # Search all dirs: local first, then external (first match wins)
        for search_dir in all_dirs:
            # Try direct path first (e.g., "mlops/axolotl")
            direct_path = _resolve_skill_under_root(search_dir, name)
            if direct_path is not None:
                if direct_path.is_dir() and (direct_path / "SKILL.md").exists():
                    skill_dir = direct_path
                    skill_md = direct_path / "SKILL.md"
                    break
                skill_md_candidate = direct_path.with_suffix(".md")
                if skill_md_candidate.exists():
                    skill_md = skill_md_candidate
                    break
            if local_category_name:
                categorized_path = _resolve_skill_under_root(search_dir, local_category_name)
                if categorized_path is not None:
                    if categorized_path.is_dir() and (categorized_path / "SKILL.md").exists():
                        ...
                        break
                    skill_md_candidate = categorized_path.with_suffix(".md")
                    if skill_md_candidate.exists():
                        skill_md = skill_md_candidate
                        break
            ...
            return json.dumps(
                {
                    "success": False,
                    "error": (
                        f"Skill '{name}' resolved outside trusted skills directories: {skill_md}"
                    ),
                },
                ensure_ascii=False,
            )
        ...
        if _injection_detected:
            logging.getLogger(__name__).warning(
                "Skill security warning for '%s': prompt injection patterns detected",
                name,
            )
