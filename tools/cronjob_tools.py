...
_GITHUB_API_CURL_RE = re.compile(
    rf"""curl\s+(?:[^\n]*\s+)?"""
    rf"""(?:-H|--header)\s+["']Authorization:\s*token\s+{_CRON_SECRET_VAR_RE}["']\s+"""
    rf"""["']?https://api\.github\.com(?:/|$)""",
    re.IGNORECASE,
)


def _is_allowed_github_api_curl(prompt: str) -> bool:
    """True only for a tight bundled-GitHub curl shape to api.github.com."""
    match = _GITHUB_API_CURL_RE.search(prompt)
    if not match:
        return False
    url_match = re.search(
...
def _scan_cron_prompt(prompt: str) -> str:
    ...
    if not _is_allowed_github_api_curl(prompt):
...
def _origin_scope(origin: Optional[Dict[str, Any]]) -> Optional[tuple]:
    """Return the security scope used for cron job ownership checks."""
...
def _same_origin_scope(left: Optional[Dict[str, Any]], right: Optional[Dict[str, Any]]) -> bool:
    """True when two cron jobs are in the same delivery/security boundary."""
    return _origin_scope(left) == _origin_scope(right)


def _job_accessible_from_origin(job: Dict[str, Any], current_origin: Optional[Dict[str, Any]]) -> bool:
    """Interactive/local callers can manage all jobs; gateway callers only their origin."""
    if _origin_scope(current_origin) is None:
        return True
    return _same_origin_scope(job.get("origin"), current_origin)


def _validate_context_from_refs(
    context_from: Optional[Union[str, List[str]]],
    current_origin: Optional[Dict[str, Any]],
) -> Optional[str]:
    """Validate context_from references exist and stay within the current origin."""
    if not context_from:
        return None
    ...
    for ref_id in refs:
        ...
        if not _job_accessible_from_origin(source_job, current_origin):
            return (
                f"context_from job '{ref_id}' is not accessible from this origin. "
                "Only jobs created from the same platform/chat/thread can be chained."
            )
    return None
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    # Reject explicit traversal even if it would normalize back inside.
    ...
    try:
        ...
        if normalized == "create":
            ...
            if normalized_base_url:
                return tool_error(
                    "Per-job base_url overrides are not allowed for cron jobs; "
                    "configure provider endpoints in the user's provider config instead.",
                    success=False,
                )
            ...
            # Validate context_from references existing jobs within the caller's origin.
            context_error = _validate_context_from_refs(context_from, current_origin)
            if context_error:
                return tool_error(context_error, success=False)
        ...
        if normalized == "list":
            jobs = [
                ...
                if _job_accessible_from_origin(job, current_origin)
            ]
        ...
        job = get_job(job_id)
        if not job or not _job_accessible_from_origin(job, current_origin):
            return json.dumps(
        ...
        if normalized == "update":
            ...
            if context_from is not None:
                ...
                # each referenced job exists within the caller's origin. Normalized
                # to a list (or None) to match the shape stored by create_job().
                if isinstance(context_from, str):
                    refs = [context_from.strip()] if context_from.strip() else []
                else:
                    refs = [str(j).strip() for j in context_from if str(j).strip()]
                context_error = _validate_context_from_refs(refs, current_origin)
                if context_error:
                    return tool_error(context_error, success=False)
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional relative path to a script under {display_hermes_home()}/scripts/ that runs each tick. Absolute paths, ~ paths, traversal, and scheduler-time symlinks are blocked. In the default mode stdout is injected into the agent's prompt as context (data-collection / change-detection pattern). With no_agent=True, the script IS the job and stdout is delivered verbatim (classic watchdog pattern). ``.sh``/``.bash`` extensions run via bash, everything else via Python. On update, pass empty string to clear."
            },
            "no_agent": {
                "type": "boolean",
                "default": False,
            ...
            "context_from": {
                ...
                "description": (
                    ...
                    "Gateway/messaging callers may only chain jobs created from the same "
                    "platform/chat/thread origin; local interactive callers can manage local jobs. "
