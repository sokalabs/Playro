...
2. User config allowlist at ``discord.server_actions``. If the user
   ...
   non-destructive actions are exposed; destructive actions require an
   explicit allowlist opt-in.
...
_SNOWFLAKE_PATTERN = r"^[0-9]{1,20}$"
_SNOWFLAKE_RE = re.compile(_SNOWFLAKE_PATTERN)
...
def _invalid_snowflake_params(params: Dict[str, str]) -> List[str]:
    """Return names of provided Discord snowflake IDs with invalid syntax."""
    return [
        name for name, value in params.items()
        if value and not _SNOWFLAKE_RE.fullmatch(str(value))
    ]
...
# Destructive actions that are hidden and blocked unless the operator
# explicitly opts in via discord.server_actions. This prevents newly added
# irreversible operations from becoming model-callable under the legacy
# default of an empty allowlist.
_EXPLICIT_OPT_IN_ACTIONS = frozenset({"delete_message"})
...
def _load_allowed_actions_config() -> Optional[List[str]]:
    ...
    hasn't restricted the set (default: all non-destructive actions allowed).
...
def _is_action_allowed_by_config(action: str, allowlist: Optional[List[str]]) -> bool:
    """Return whether ``action`` is permitted by operator configuration."""
    if allowlist is not None:
        return action in allowlist
    return action not in _EXPLICIT_OPT_IN_ACTIONS
...
def _available_actions(
    ...
    for name in _ACTIONS:
        ...
        if not caps.get("has_members_intent", True) and name in _INTENT_GATED_MEMBERS:
            continue
        # Config allowlist filter. Destructive actions require explicit opt-in.
        if not _is_action_allowed_by_config(name, allowlist):
            continue
        actions.append(name)
    return actions
...
def _run_discord_action(
    ...
    # actions through).
    allowlist = _load_allowed_actions_config()
    if not _is_action_allowed_by_config(action, allowlist):
        allowed = allowlist if allowlist is not None else [
            name for name in _ACTIONS if name not in _EXPLICIT_OPT_IN_ACTIONS
        ]
        return json.dumps({
            "error": (
    ...
    invalid_snowflakes = _invalid_snowflake_params({
        "guild_id": guild_id,
        "channel_id": channel_id,
        "user_id": user_id,
        "role_id": role_id,
    ...
    if invalid_snowflakes:
        return json.dumps({
            "error": (
                "Invalid Discord snowflake ID for parameter(s): "
                f"{', '.join(invalid_snowflakes)}. IDs must contain only 1-20 digits."
            ),
        })
...
_STATIC_ADMIN_SCHEMA = _build_schema(
    [name for name in _ADMIN_ACTIONS if name not in _EXPLICIT_OPT_IN_ACTIONS],
    caps={"detected": False},
