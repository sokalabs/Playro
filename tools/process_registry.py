...
class ProcessRegistry:
    ...
    def _terminate_host_pid(pid: int) -> None:
        ...
        kill_posix_process_tree(pid, escalate=False)
    ...
    def _move_to_finished(self, session: ProcessSession):
        ...
        if was_running and session.notify_on_complete:
            # Do not copy process output into the completion queue. Background
            # output is untrusted and completion notifications are converted
            # into synthetic agent input by CLI/gateway consumers. Keeping the
            # notification to trusted status metadata prevents command output
            # from becoming an autonomous prompt-injection channel; callers can
            # explicitly inspect logs with poll/read_log/wait when needed.
            self.completion_queue.put({
                "type": "completion",
                "session_id": session.id,
                "command": session.command,
            ...
            if session._pty:
                ...
                try:
                    if _IS_WINDOWS:
                        ...
                        kill_posix_process_tree(session.process.pid, escalate=False)
