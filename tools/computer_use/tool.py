...
def _get_approval_callback():
    """Return the approval callback bound to the current thread, if any."""
...
# Actions that do not expose screen contents or mutate user-visible state.
_SAFE_ACTIONS = frozenset({"wait", "list_apps"})

# Actions that can expose sensitive screen contents. Go through approval.
_CAPTURE_ACTIONS = frozenset({"capture"})
...
_APPROVAL_REQUIRED_ACTIONS = _CAPTURE_ACTIONS | _DESTRUCTIVE_ACTIONS
...
def _canon_key_combo(keys: str) -> frozenset:
    # cua-driver accepts both '+' and '-' separators; normalize before blocking.
    normalized = re.sub(r"\s*-\s*", "+", keys)
    parts = [p.strip().lower() for p in re.split(r"\s*\+\s*", normalized) if p.strip()]
...
def reset_backend_for_tests() -> None:  # pragma: no cover
    ...
    global _backend, _always_allow, _approval_callback
    with _backend_lock:
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    # Approval gate for screenshots and mutating actions.
...
def _request_approval(action: str, args: Dict[str, Any]) -> Optional[str]:
    ...
    if cb is None:
        logger.warning("computer_use %s denied: no approval callback installed", action)
        return json.dumps({
            "error": "approval required for computer_use action",
            "action": action,
            "hint": "Install an interactive approval callback before using screen capture or desktop-control actions.",
        })
    ...
    if verdict == "approve_session" or verdict == "always_approve":
        # Scope persistent approvals to the action that was explicitly approved.
        # For example, always allowing screenshots must not silently unlock
        # later desktop-control actions such as typing, clicking, or key presses.
...
def _summarize_action(action: str, args: Dict[str, Any]) -> str:
    if action == "capture":
        ...
        app = args.get("app")
        scope = f" app={app!r}" if app else " frontmost app or whole screen"
        return f"capture {mode!r}{scope}"
