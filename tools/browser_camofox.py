...
def is_camofox_mode() -> bool:
    """True when the Camofox backend is configured.

    Unsafe user-supplied live Chrome CDP overrides are ignored elsewhere, so a
    leftover ``BROWSER_CDP_URL`` value must not suppress this isolated backend.
    """
...
def _ensure_tab(task_id: Optional[str], url: str = "about:blank") -> Dict[str, Any]:
    ...
    session["_last_tab_response"] = data
    return session
...
def _blank_tab(session: Dict[str, Any]) -> None:
    """Navigate the current Camofox tab away from sensitive content."""
    ...
    try:
        ...
        logger.warning("Failed to blank unsafe Camofox tab %s: %s", tab_id, exc)


def _blocked_final_url_error(requested_url: str, final_url: Optional[str]) -> Optional[str]:
    """Return a Camofox blocking error for unsafe loaded URLs."""
    if not final_url:
        return None
    if is_always_blocked_url(final_url):
        return "Blocked: URL targets a cloud metadata endpoint"
    if final_url != requested_url and not is_safe_url(final_url):
...
def camofox_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    from tools import browser_tool as browser_tool_module

    try:
        session = _get_session(task_id)
        if not session["tab_id"]:
            # Create a blank tab first so every requested URL flows through the
            # navigate endpoint and returns a final URL for redirect checks.
        ...
        blocked_error = _blocked_final_url_error(url, final_url)
        if blocked_error:
            _blank_tab(session)
            return json.dumps({"success": False, "error": blocked_error})
