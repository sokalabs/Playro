...
def _cache_mcp_image_block(block) -> str:
    ...
    try:
        from gateway.platforms.security import mcp_image_block_within_limits

        if not mcp_image_block_within_limits(data):
            logger.warning(
                "MCP image block exceeds size limits (encoded=%d bytes)",
                len(data),
            )
            return ""
...
def _mcp_tool_call_is_idempotent(tool_name: str) -> bool:
    """Return True when replaying a failed MCP call cannot duplicate writes."""
    normalized = (tool_name or "").strip().lower()
    if not normalized:
        return False
    if normalized.startswith(("list_", "get_", "read_", "search_", "describe_", "fetch_")):
        return True
    return any(normalized.endswith(suffix) for suffix in _MCP_IDEMPOTENT_TOOL_SUFFIXES)
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        try:
            ...
            if _mcp_tool_call_is_idempotent(tool_name):
