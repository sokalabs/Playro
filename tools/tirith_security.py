...
The download verifies both cosign provenance (GitHub Actions workflow
signature) and SHA-256 checksums before installing.  If cosign is not installed
or provenance cannot be verified, auto-install is aborted.  Installation runs
in a background thread so startup never blocks.
"""
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    ...
    try:
        ...
        if not shutil.which("cosign"):
            log("tirith install aborted: cosign is required for provenance verification")
        ...
        try:
            ...
            return None, "cosign_artifacts_missing"

        cosign_result = _verify_cosign(checksums_path, sig_path, cert_path)
        if cosign_result is False:
            # Verification explicitly rejected — abort, the release may have
            # been tampered with.
        ...
        if cosign_result is None:
            log("tirith install aborted: cosign provenance verification could not run")
