...
def _get_cdp_override() -> str:
    ...
    Live Chrome CDP overrides are intentionally disabled. Routing browser
    tools into a user's authenticated desktop browser lets untrusted page
    content returned by ``browser_snapshot`` influence later clicks, typing,
    and navigation in that real browser session. Keep browser automation on
    isolated local/headless or managed cloud sessions instead.
    """
    return ""
...
def _ensure_cdp_supervisor(task_id: str) -> None:
    ...
    ``browser_navigate`` without worrying about double-attach.

    Resolves CDP only from ``_active_sessions[task_id]["cdp_url"]`` (Browserbase
    or another managed cloud provider whose ``create_session`` returns a raw CDP
    URL). User-supplied live Chrome overrides are intentionally ignored.
...
_allow_cloud_local_fallback_resolved = False
_cached_allow_cloud_local_fallback: bool = False
...
def _is_local_backend() -> bool:
    """Return True when the configured browser backend is local.

    This helper is for backend selection and diagnostics only.  It must not be
    used to bypass URL safety checks: browser toolsets can be exposed without
    terminal-equivalent access, and a user-supplied CDP endpoint may point at a
    remote browser even when no cloud provider is configured.
    """
...
def _run_chrome_fallback_command(
    ...
    if _get_cloud_provider() and not _allow_private_urls():
        if _is_always_blocked_url(current_url) or not _is_safe_url(current_url):
            return {
                "success": False,
                "error": (
                    "Chrome fallback blocked: Lightpanda session URL targets a "
                    "private or internal address in cloud browser mode"
                ),
            }
    ...
    try:
        cmd_prefix = _agent_browser_cmd_prefix(browser_cmd)
    except FileNotFoundError as e:
        return {"success": False, "error": str(e)}
...
def _navigation_session_key(task_id: str, url: str) -> str:
    ...
    Returns the bare task_id unless ALL of these are true:
      ...
      4. Camofox mode is not active (Camofox is already local-only).
...
def _allow_cloud_local_fallback() -> bool:
    """Return whether cloud provider failures may fall back to local Chromium.

    Reads ``browser.allow_local_fallback`` once and caches the result for the
    process lifetime.  Defaults to ``False`` so a configured cloud provider
    fails closed instead of silently crossing the cloud/local isolation boundary.
    """
    global _allow_cloud_local_fallback_resolved, _cached_allow_cloud_local_fallback
    if _allow_cloud_local_fallback_resolved:
        return _cached_allow_cloud_local_fallback

    _allow_cloud_local_fallback_resolved = True
    _cached_allow_cloud_local_fallback = False
    try:
        from hermes_cli.config import read_raw_config
        cfg = read_raw_config()
        browser_cfg = cfg.get("browser", {})
        if isinstance(browser_cfg, dict):
            _cached_allow_cloud_local_fallback = is_truthy_value(
                browser_cfg.get("allow_local_fallback"), default=False
            )
    except Exception as e:
        logger.debug("Could not read allow_local_fallback from config: %s", e)
    return _cached_allow_cloud_local_fallback
...
def _get_session_info(task_id: Optional[str] = None) -> Dict[str, str]:
    ...
    # Create session outside the lock (network call in cloud mode). User-supplied
    # CDP overrides are disabled, so sessions are either isolated local browsers
    # or managed cloud-provider sessions.
    if force_local:
        ...
        if provider is None:
            ...
            try:
                ...
                if not _allow_cloud_local_fallback():
                    logger.warning(
                        "Cloud provider %s failed (%s); local Chromium fallback "
                        "is disabled for task %s",
                        provider_name, e, task_id,
                        exc_info=True,
                    )
                    raise RuntimeError(
                        f"Cloud provider {provider_name} failed ({e}); local "
                        "Chromium fallback is disabled. Set "
                        "browser.allow_local_fallback: true to opt in."
                    ) from e

                logger.warning(
                    "Cloud provider %s failed (%s); attempting configured "
                    "fallback to local Chromium for task %s",
                    provider_name, e, task_id,
                    exc_info=True,
                )
                try:
                ...
                # Mark explicitly configured fallback sessions for observability.
...
def _find_agent_browser() -> str:
    ...
    # consults PATHEXT, so Windows may select the `.cmd` shim here; the command
    # builder below rewrites batch shims to `node <agent-browser.js>` before
    # appending prompt-controlled browser arguments.
...
def _is_windows_batch_file(command: str) -> bool:
    """Return True for Windows batch shims that require cmd.exe parsing."""
    return os.name == "nt" and Path(command).suffix.lower() in {".cmd", ".bat"}


def _node_executable_for_shim(shim_path: Optional[Path] = None) -> str:
    """Resolve node.exe without going through an npm batch shim."""
...
def _agent_browser_js_candidates(shim_path: Path) -> List[Path]:
    """Return likely npm package JS entrypoints for an agent-browser shim."""
    package_name = shim_path.stem
    return [
        # Local install: node_modules/.bin/agent-browser.cmd ->
        # node_modules/agent-browser/bin/agent-browser.js
        shim_path.parent.parent / package_name / "bin" / f"{package_name}.js",
        # Global npm prefix: <prefix>/agent-browser.cmd ->
        # <prefix>/node_modules/agent-browser/bin/agent-browser.js
        shim_path.parent / "node_modules" / package_name / "bin" / f"{package_name}.js",
        # Alternate global layout occasionally used by Node distributions.
        shim_path.parent.parent / "node_modules" / package_name / "bin" / f"{package_name}.js",
    ]


def _agent_browser_cmd_prefix(browser_cmd: str) -> List[str]:
    """Build a subprocess argv prefix without invoking unsafe batch shims.

    On Windows, Python's ``subprocess`` ultimately runs ``.cmd``/``.bat`` files
    through ``cmd.exe`` even with ``shell=False``.  ``cmd.exe`` reparses the
    command line and treats metacharacters such as ``&`` and ``>`` specially,
    so prompt-controlled browser arguments must never be appended to npm batch
    shims.  When a shim is selected, run Node directly with the underlying JS
    entrypoint instead.
    """
    if browser_cmd == "npx agent-browser":
        ...
        if not npx_bin:
            if os.name == "nt":
                raise FileNotFoundError(
                    "Refusing to execute bare Windows npx fallback with browser "
                    "arguments; install agent-browser locally/globally."
                )
            return ["npx", "agent-browser"]
        if not _is_windows_batch_file(npx_bin):
        ...
        for candidate in npx_candidates:
            if candidate.exists():
                return [
                    _node_executable_for_shim(npx_path),
                    str(candidate),
                    "agent-browser",
                ]
        raise FileNotFoundError(
            "Refusing to execute Windows npx batch shim with browser arguments; "
            "install agent-browser locally/globally or ensure node and npm's "
            "npx-cli.js are available."
        )

    if _is_windows_batch_file(browser_cmd):
        shim_path = Path(browser_cmd)
        for candidate in _agent_browser_js_candidates(shim_path):
            if candidate.exists():
                return [_node_executable_for_shim(shim_path), str(candidate)]
        raise FileNotFoundError(
            "Refusing to execute Windows agent-browser batch shim with browser "
            "arguments because the underlying JS entrypoint could not be found."
        )
...
def _run_browser_command(
    ...
    try:
        ...
        logger.warning("agent-browser CLI is not safely executable: %s", e)
        return {"success": False, "error": str(e)}
...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    # This applies to both cloud and non-cloud browser backends because the
    # browser toolset can be exposed without terminal-equivalent local network
    # access.  Hybrid routing may still auto-spawn a local Chromium sidecar for
    # ordinary private URLs; cloud metadata endpoints remain blocked below.
    # Users can explicitly opt out via ``browser.allow_private_urls`` in config.
    ...
    if result.get("success"):
        ...
        # internal content via subsequent browser_snapshot calls.  This applies
        # to both cloud and non-cloud browser backends.  Hybrid local sidecar
        # routing can still allow ordinary private destinations by design, but
...
def browser_console(clear: bool = False, expression: Optional[str] = None, task_id: Optional[str] = None) -> str:
    ...
    Args:
        ...
        expression: Deprecated and blocked. Browser-console JavaScript
            evaluation is disabled because arbitrary page-context JS can
            bypass navigation safety checks and expose browser secrets.
        task_id: Task identifier for session isolation

    Returns:
        JSON string with console messages/errors, or a safety error when
        JavaScript evaluation is requested.
    """
    if expression is not None:
        return json.dumps({
            "success": False,
            "error": (
                "Blocked: browser_console JavaScript evaluation is disabled. "
                "Use browser_snapshot, browser_vision, or purpose-built browser "
                "tools instead."
            ),
        })
...
def check_browser_requirements() -> bool:
    ...
    # User-supplied CDP override mode is disabled; do not treat a leftover
    # BROWSER_CDP_URL as satisfying browser requirements.
