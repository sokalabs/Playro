...
_DOCUMENT_EXTS = {
    ".epub", ".pdf", ".zip", ".rar", ".7z", ".doc", ".docx",
    ".xls", ".xlsx", ".ppt", ".pptx", ".txt", ".csv", ".apk", ".ipa",
}
_SEND_MESSAGE_MEDIA_EXTS = _IMAGE_EXTS | _VIDEO_EXTS | _AUDIO_EXTS | _DOCUMENT_EXTS
_DEFAULT_MEDIA_ROOT_SUBDIRS = (
    "cache/images",
    "cache/audio",
    "cache/screenshots",
    "cache/documents",
...
_MATRIX_MEDIA_MAX_BYTES = int(
    os.getenv("HERMES_MATRIX_MEDIA_MAX_BYTES", str(25 * 1024 * 1024))
)


def _iter_matrix_media_roots():
    """Yield directories from which Matrix MEDIA attachments may be uploaded.

    Matrix uploads read local files in-process, so MEDIA tags must not be able
    to point at arbitrary host paths such as /etc/passwd.  The default roots
    cover Hermes-generated artifacts; deployments that intentionally need
    another local export directory can opt in with HERMES_MATRIX_MEDIA_ROOTS
    using os.pathsep-separated paths.
    """
    env_roots = os.getenv("HERMES_MATRIX_MEDIA_ROOTS", "")
    ...
    try:
        ...
        yield get_hermes_home() / "cache"
    except Exception:
        pass

    # Some older/generated artifacts live directly in process temp dirs.  Keep
    # this scoped to Hermes-owned prefixes instead of allowing all of /tmp.
    tmp_root = Path(tempfile.gettempdir())
    for name in ("hermes-results",):
        yield tmp_root / name
...
def _validate_matrix_media_path(media_path: str) -> tuple[Path | None, str | None]:
    """Validate a Matrix MEDIA path before handing it to the adapter."""
    try:
        candidate = Path(media_path).expanduser().resolve(strict=True)
    except FileNotFoundError:
        return None, f"Media file not found: {media_path}"
    except OSError as exc:
        return None, f"Media file is not accessible: {media_path} ({exc})"
    ...
    if not candidate.is_file():
        return None, f"Media path is not a regular file: {media_path}"
    if stat.st_size > _MATRIX_MEDIA_MAX_BYTES:
        return None, (
            f"Media file is too large for Matrix upload: {media_path} "
            f"({stat.st_size} bytes > {_MATRIX_MEDIA_MAX_BYTES} bytes)"
        )

    roots = []
    for root in _iter_matrix_media_roots():
        try:
            roots.append(root.resolve(strict=False))
        except OSError:
            continue

    if not any(_path_is_relative_to(candidate, root) for root in roots):
        return None, (
            "Matrix MEDIA attachments must be generated under Hermes cache "
            "directories or an explicit HERMES_MATRIX_MEDIA_ROOTS allowlist"
        )
...
_SIGNAL_MAX_MEDIA_BYTES = 25 * 1024 * 1024
_SIGNAL_MEDIA_MAGIC = {
    ".png": (b"\x89PNG\r\n\x1a\n",),
    ".jpg": (b"\xff\xd8\xff",),
    ".jpeg": (b"\xff\xd8\xff",),
...
def _yuanbao_media_allowed_roots() -> list[Path]:
    """Return Hermes-owned media cache directories safe for Yuanbao uploads.

    Yuanbao send_message delivery runs from the generic tool path where MEDIA
    tags can be model-controlled.  Keep Yuanbao attachments constrained to
    files previously materialized by Hermes media cache helpers so arbitrary
    local paths cannot be uploaded through an active Yuanbao adapter.
    """
    from hermes_constants import get_hermes_home
    ...
    return [
        ...
        # Legacy cache locations kept for existing installs.
        home / "image_cache",
        home / "audio_cache",
        home / "video_cache",
        home / "document_cache",
...
def _validate_yuanbao_media_files(media_files):
    """Allow Yuanbao MEDIA uploads only from Hermes-owned media caches."""
    if not media_files:
        return [], None

    allowed_roots = _yuanbao_media_allowed_roots()
    safe_media_files = []
    for media_path, is_voice in media_files:
        candidate = Path(os.path.expanduser(str(media_path)))
        if not any(_is_path_under(candidate, root) for root in allowed_roots):
            return [], _error(
                "Yuanbao MEDIA attachments must come from Hermes media cache directories; "
                "refusing to upload an arbitrary local path."
            )
        safe_media_files.append((str(candidate), is_voice))
    return safe_media_files, None
...
def _send_message_media_roots() -> list[str]:
    """Return directories trusted for MEDIA attachments from send_message.

    MEDIA tags are model/caller-controlled text, so native upload platforms must
    only read files from Hermes-owned media caches or explicitly configured
    extra roots.  ``HERMES_SEND_MESSAGE_MEDIA_ROOTS`` may add project-specific
    cache directories using the platform path separator.
    """
    roots = []
    try:
        from hermes_constants import get_hermes_home
        ...
        roots.extend(
            str(hermes_home / subdir)
            for subdir in _DEFAULT_MEDIA_ROOT_SUBDIRS
        )
    except Exception:
        pass
    ...
    extra_roots = os.getenv("HERMES_SEND_MESSAGE_MEDIA_ROOTS", "")
...
def _validate_send_message_media_files(
    media_files,
    *,
    platform_name: str = "send_message",
) -> Optional[str]:
    """Validate MEDIA attachment paths before a native upload opens them."""
    if not media_files:
        return None

    allowed_roots = _send_message_media_roots()
    for media_path, _is_voice in media_files:
        ext = os.path.splitext(str(media_path))[1].lower()
        if ext not in _SEND_MESSAGE_MEDIA_EXTS:
            return f"{platform_name} MEDIA attachment rejected: unsupported file extension"

        try:
            raw_path = os.path.abspath(os.path.expanduser(str(media_path)))
            resolved_path = os.path.realpath(raw_path)
        except (TypeError, ValueError, OSError):
            return f"{platform_name} MEDIA attachment rejected: invalid file path"

        if not os.path.isfile(resolved_path):
            return f"{platform_name} MEDIA attachment rejected: file not found or not a regular file"

        if not any(
            os.path.commonpath([resolved_path, root]) == root
            for root in allowed_roots
        ):
            return (
                f"{platform_name} MEDIA attachment rejected: path must be inside a Hermes media cache "
                "or HERMES_SEND_MESSAGE_MEDIA_ROOTS"
            )
...
def _handle_send(args):
    ...
    # WhatsApp targets must be E.164 phone numbers or explicit IDs — never
    # channel-directory aliases that can misroute to another platform's home.
    if platform_name == "whatsapp" and target_ref and not is_explicit:
        return json.dumps({
            "error": (
                f"WhatsApp target '{target_ref}' must be an E.164 phone number "
                "(for example whatsapp:+15551234567) or use bare 'whatsapp' "
                "to send to the configured home channel."
            ),
        })
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False):
    ...
    if platform == Platform.YUANBAO and media_files:
        media_files, yuanbao_media_error = _validate_yuanbao_media_files(media_files)
        if yuanbao_media_error:
            return yuanbao_media_error

    if platform == Platform.FEISHU:
        validation_error = _validate_send_message_media_files(
            media_files,
...
async def _send_discord(token, chat_id, message, thread_id=None, media_files=None):
    ...
    try:
        from gateway.platforms.base import (
            ...
            validate_media_attachment_path,
        )
        ...
        if thread_id:
            ...
            if is_forum:
                ...
                # Filter to safe, generated media files up front so prompt-
                # controlled MEDIA tags cannot upload arbitrary local files or
                # block on special files before opening a session.
                valid_media = []
                for media_path, _is_voice in media_files:
                    safe_path, warning = validate_media_attachment_path(media_path)
                    if warning:
                        logger.warning(warning)
                        warnings.append(warning)
                        continue
                    valid_media.append(safe_path)
...
def _signal_media_allowed_roots(extra) -> list[Path]:
    """Return explicit roots from which Signal attachments may be sent."""
    from hermes_constants import get_hermes_dir, get_hermes_home

    configured = []
    if isinstance(extra, dict):
        configured = extra.get("media_roots") or extra.get("allowed_media_roots") or []
    if isinstance(configured, str):
        configured = [configured]

    roots = [
        get_hermes_dir("cache", "cache"),
        get_hermes_dir("cache/images", "image_cache"),
        get_hermes_dir("cache/vision", "temp_vision_images"),
        get_hermes_dir("cache/video", "temp_video_files"),
        get_hermes_home() / "attachments",
        Path("/tmp/hermes"),
    ]
    roots.extend(Path(os.path.expanduser(str(root))) for root in configured if str(root).strip())
...
def _path_is_within_any_root(path: Path, roots: list[Path]) -> bool:
    for root in roots:
        try:
            path.relative_to(root)
            return True
...
def _signal_media_type_is_allowed(path: Path) -> bool:
    suffix = path.suffix.lower()
    if suffix not in _SIGNAL_MEDIA_EXTS:
        return False

    magic_prefixes = _SIGNAL_MEDIA_MAGIC.get(suffix)
    ...
    try:
        ...
        return False
    return any(header.startswith(prefix) for prefix in magic_prefixes)


def _validate_signal_media_path(media_path: str, extra) -> tuple[str | None, str | None]:
    """Validate a model-supplied Signal media path before JSON-RPC upload.

    Returns ``(resolved_path, warning)``.  ``resolved_path`` is only set when
    the file is inside an explicit media root, has an allowed media extension,
    is a regular readable file, and is below the size cap.
    """
    try:
        raw_path = Path(os.path.expanduser(str(media_path)))
        resolved = raw_path.resolve(strict=True)
    except (OSError, RuntimeError) as exc:
        return None, f"Signal media file not found, skipping: {media_path} ({exc})"

    if not _path_is_within_any_root(resolved, _signal_media_allowed_roots(extra)):
        return None, f"Signal media file outside allowed media roots, skipping: {media_path}"

    if not resolved.is_file():
        return None, f"Signal media path is not a regular file, skipping: {media_path}"

    try:
        stat_result = resolved.stat()
    except OSError as exc:
        return None, f"Signal media file is not readable, skipping: {media_path} ({exc})"
    if not os.access(resolved, os.R_OK):
        return None, f"Signal media file is not readable, skipping: {media_path}"

    if stat_result.st_size > _SIGNAL_MAX_MEDIA_BYTES:
        return None, f"Signal media file exceeds 25 MiB limit, skipping: {media_path}"

    if not _signal_media_type_is_allowed(resolved):
        return None, f"Signal media file type is not allowed, skipping: {media_path}"
...
async def _send_signal(extra, chat_id, message, media_files=None):
    ...
    try:
        ...
        attachment_paths = []
        skipped_media_warnings = []
        for media_path, _is_voice in valid_media:
            attachment_path, warning = _validate_signal_media_path(media_path, extra)
            if attachment_path:
                attachment_paths.append(attachment_path)
            elif warning:
                logger.warning(warning)
                skipped_media_warnings.append(warning)
        ...
        warnings = []
        if skipped_media_warnings:
            warnings.append("Some media files were skipped (failed Signal media safety checks)")
        if failed_batches:
            warnings.append(
...
async def _send_matrix_via_adapter(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    try:
        ...
        for media_path, is_voice in media_files:
            safe_media_path, validation_error = _validate_matrix_media_path(media_path)
            if validation_error:
                return _error(validation_error)
            media_path = str(safe_media_path)
