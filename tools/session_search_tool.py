...
def _list_recent_sessions(
    ...
    """Return metadata for the most recent in-scope sessions (no LLM calls)."""
    try:
        list_kwargs = {
            "limit": limit + 5,
...
def session_search(
    ...
    # Restrict prompt-callable recall to the caller's current interface/source.
    # SessionDB.search_messages(None) intentionally means "all sources" for
    # internal/admin callers, so the tool entrypoint must supply the user's
    # source scope before loading transcripts for summarization.
    source_scope = current_source.strip() if isinstance(current_source, str) else None
    if not source_scope and current_session_id:
        try:
            current_session = db.get_session(current_session_id)
            if current_session:
                source = current_session.get("source")
                source_scope = source.strip() if isinstance(source, str) else source
        except Exception:
            logging.debug(
                "Unable to resolve session_search source scope from current session",
                exc_info=True,
            )
    ...
    if not query or not query.strip():
        return _list_recent_sessions(db, limit, current_session_id, source_scope)
    ...
    try:
        ...
        raw_results = db.search_messages(
            query=query,
            source_filter=[source_scope] if source_scope else None,
...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "past sessions from your current interface are searchable, and this tool summarizes what happened.\n\n"
        "TWO MODES:\n"
        "1. Recent sessions (no query): Call with no arguments to see what was worked on recently. "
        "Returns titles, previews, and timestamps. Zero LLM cost, instant. "
        "Start here when the user asks what were we working on or what did we do recently.\n"
        "2. Keyword search (with query): Search for specific topics across in-scope past sessions. "
...
registry.register(
    ...
    handler=lambda args, **kw: session_search(
        ...
        current_source=kw.get("current_source") or kw.get("platform")),
