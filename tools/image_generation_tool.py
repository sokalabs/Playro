...
_DEFAULT_IMAGE_URL_HOST_SUFFIXES = (
    "fal.media",
    "fal.ai",
    "fal.run",
...
def _allowed_image_url_host_suffixes() -> tuple[str, ...]:
    """Return the HTTPS host suffixes accepted for generated image URLs."""
    raw = os.getenv("HERMES_IMAGE_URL_ALLOWED_HOSTS", "")
    configured = tuple(
        host.strip().lower().lstrip(".")
        for host in raw.split(",")
        if host.strip()
    )
    return configured or _DEFAULT_IMAGE_URL_HOST_SUFFIXES
...
def _validate_generated_image_url(url: Any) -> str:
    """Validate a provider-returned image URL before exposing it to agents.

    Comic and image workflows often download this value as a follow-up step.
    Keep the value constrained to HTTPS URLs from known image-delivery hosts so
    provider/gateway-controlled responses cannot become ``file://`` reads,
    localhost/internal-network fetches, or shell-metacharacter surprises.
    """
    ...
    if stripped != url or any(ord(ch) < 32 or ord(ch) == 127 for ch in stripped):
        raise ValueError(
            "Generated image URL contains unsafe whitespace/control characters"
        )
    ...
    if parsed.username or parsed.password:
        raise ValueError("Generated image URL must not include credentials")

    host = (parsed.hostname or "").lower().rstrip(".")
    if not host:
        raise ValueError("Generated image URL must include a host")
    ...
    try:
        ...
        allowed_suffixes = _allowed_image_url_host_suffixes()
        if not any(_host_matches_suffix(host, suffix) for suffix in allowed_suffixes):
            raise ValueError(
                "Generated image URL host is not allowed; set "
                "HERMES_IMAGE_URL_ALLOWED_HOSTS to add trusted HTTPS hosts"
            )
    else:
        raise ValueError("Generated image URL must use a hostname, not an IP literal")
...
def _upscale_image(image_url: str, original_prompt: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if result and "image" in result:
            ...
            safe_upscaled_url = _validate_generated_image_url(upscaled_image.get("url"))
            return {
                "url": safe_upscaled_url,
    ...
    try:
        ...
        for img in images:
            if not (isinstance(img, dict) and "url" in img):
                continue
            safe_url = _validate_generated_image_url(img["url"])
            original_image = {
                "url": safe_url,
            ...
            if should_upscale:
                upscaled_image = _upscale_image(safe_url, prompt.strip())
                if upscaled_image:
