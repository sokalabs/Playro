...
def _is_write_denied(path: str, cwd: Optional[str] = None, home: Optional[str] = None) -> bool:
    """Return True if path is on the write deny list.

    When checking a relative path that will be executed by a terminal
    backend, pass the backend's effective cwd so authorization and the
    subsequent shell command resolve the target identically.

    ``home`` may be supplied by backend-aware callers (for example SSH) so
    home-relative deny rules are evaluated against the filesystem where the
    write will occur, not just the local Hermes process home.
    """
    return _shared_is_write_denied(path, cwd=cwd, home=home)
...
class ShellFileOperations(FileOperations):
    ...
    def __init__(self, terminal_env, cwd: str = None):
        ...
        # Cache for command availability and backend home directory checks.
    ...
    def _effective_cwd(self, cwd: str = None) -> str:
        """Return the cwd that _exec() will pass to the terminal backend."""
    ...
    def _get_home_directory(self) -> Optional[str]:
        """Return the backend user's home directory, if it can be determined."""
    ...
    def _is_write_denied_for_backend(self, path: str) -> bool:
        """Check write protection using the backend cwd and home directory."""
    ...
    def delete_file(self, path: str) -> WriteResult:
        ...
        path = self._expand_path(path)
        if self._is_write_denied_for_backend(path):
    ...
    def move_file(self, src: str, dst: str) -> WriteResult:
        ...
        for p in (src, dst):
            if self._is_write_denied_for_backend(p):
