...
# Explicit child->parent container routing for delegate_task subagents.
# Normal top-level task/session IDs must remain isolated; only subagent IDs
# registered here may share their parent task's sandbox.
_subagent_container_parents: Dict[str, Optional[str]] = {}
_subagent_container_parents_lock = threading.Lock()


def register_subagent_container_parent(child_task_id: str, parent_task_id: Optional[str]) -> None:
    """Route a delegate_task child to its parent's container sandbox.

    This preserves task_id/session_id isolation for unrelated top-level agents
    while still allowing known subagents to share the parent's working
    directory, installed packages, and terminal state.
    """
    if not child_task_id:
        return
    with _subagent_container_parents_lock:
        _subagent_container_parents[child_task_id] = parent_task_id


def unregister_subagent_container_parent(child_task_id: str) -> None:
    """Remove a delegate_task child container mapping after it finishes."""
    if not child_task_id:
        return
    with _subagent_container_parents_lock:
        _subagent_container_parents.pop(child_task_id, None)
...
def _finalize_sandbox_task_id(task_id: str) -> str:
    """Return a filesystem-safe sandbox key for container backends."""
    from utils import sanitize_sandbox_task_id

    if task_id in _task_env_overrides:
        return task_id
    return sanitize_sandbox_task_id(task_id)
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Missing task IDs land on ``"default"`` for backwards-compatible direct
    tool calls. Explicit top-level task/session IDs remain isolated and resolve
    to themselves. Only delegate_task children registered via
    ``register_subagent_container_parent`` are routed to their parent's
    container key so subagents can share the parent's long-lived sandbox without
    collapsing unrelated users, gateway sessions, or batch tasks together.

    RL / benchmark environments (TerminalBench2, HermesSweEnv, ...) call
    ``register_task_env_overrides(task_id, {...})`` to request a per-task
    Docker/Modal image. Registered overrides always keep their own task ID.
    """
    if not task_id:
        return "default"
    if task_id in _task_env_overrides:
        return _finalize_sandbox_task_id(task_id)
    ...
    while True:
        with _subagent_container_parents_lock:
            parent_task_id = _subagent_container_parents.get(current, sentinel)
        if parent_task_id is sentinel:
            return _finalize_sandbox_task_id(current)
        if not parent_task_id:
            return "default"
        if parent_task_id in _task_env_overrides:
            return _finalize_sandbox_task_id(parent_task_id)
        if parent_task_id in seen:
            logger.warning(
                "Cycle detected in subagent container mapping for task_id=%r",
                task_id,
            )
            return _finalize_sandbox_task_id(task_id)
...
def _load_docker_env_config() -> dict:
    """Load docker_env from the private runtime store (not host env)."""
    from tools.docker_env_store import clear_legacy_docker_env_envvar, read_docker_env

    clear_legacy_docker_env_envvar()
    return read_docker_env()
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "docker_env": _load_docker_env_config(),
...
def terminal_tool(
    ...
    try:
        ...
        # Use task_id for environment isolation. Registered delegate_task
        # children share their parent's container; unrelated task/session IDs
        # keep separate sandboxes unless they intentionally use the same ID.
