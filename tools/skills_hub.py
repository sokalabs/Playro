...
def _guarded_httpx_get(
    ...
    *,
    ...
    """Fetch a URL with SSRF checks, redirect validation, and optional headers."""
    current_url = url
...
def _guarded_http_get(url: str, *, timeout: int = 20) -> Optional[httpx.Response]:
    ...
    return _guarded_httpx_get(url, timeout=timeout)
...
class GitHubSource(SkillSource):
    ...
    def _list_skills_in_repo(self, repo: str, path: str) -> List[SkillMeta]:
        ...
        try:
            resp = _guarded_httpx_get(url, headers=self.auth.get_headers(), timeout=15)
    ...
    def _check_rate_limit_response(self, resp: Optional["httpx.Response"]) -> None:
...
class SkillsShSource(SkillSource):
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        for candidate in self._candidate_identifiers(canonical):
            ...
            if bundle:
                return self._finalize_fetch_bundle(bundle, canonical, detail, candidate)

        resolved = self._discover_identifier(canonical, detail=detail)
        if resolved:
            bundle = self.github.fetch(resolved)
            if bundle:
                return self._finalize_fetch_bundle(bundle, canonical, detail, resolved)
        return None

    def _finalize_fetch_bundle(
        self,
        bundle: SkillBundle,
        canonical: str,
        ...
        bundle.metadata["resolved_identifier"] = resolved_identifier
        return bundle
...
def _load_hermes_index() -> Optional[dict]:
    ...
    try:
        resp = _guarded_http_get(HERMES_INDEX_URL, timeout=15)
        if resp is None or resp.status_code != 200:
            if resp is None:
                logger.debug("Hermes index fetch returned no response")
            else:
...
class HermesIndexSource(SkillSource):
    ...
    def trust_level_for(self, identifier: str) -> str:
        ...
        if not entry:
            return "community"
        origin = self._entry_github_identifier(entry)
        return self._trust_level_for_github_origin(origin)
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # The index is an unsigned catalog, not a trust authority.  Use it only
        # to resolve the concrete GitHub content path, then preserve the
        # GitHubSource bundle provenance so scans, install policy, audit logs,
        # and user-facing warnings are based on the actual downloaded origin.
        github_id = self._entry_github_identifier(entry)
        if not github_id:
            return None

        bundle = self._get_github().fetch(github_id)
        if bundle:
            bundle.metadata = dict(getattr(bundle, "metadata", {}) or {})
            bundle.metadata.setdefault("index_identifier", entry.get("identifier", identifier))
            bundle.metadata.setdefault("index_source", entry.get("source", "hermes-index"))
            return bundle
    ...
    @classmethod
    def _entry_github_identifier(cls, entry: dict) -> Optional[str]:
        """Return the concrete GitHub path supplied by an index entry.

        The docs-hosted index may be stale or compromised, so callers must not
        treat its display identifier, source, or trust labels as authoritative.
        This helper extracts only a plausible GitHub content origin.
        """
        ...
        if isinstance(repo, str) and isinstance(path, str):
            ...
            if len(repo.split("/")) == 2 and path:
                return f"{repo}/{path}"

        return None

    @staticmethod
    def _trust_level_for_github_origin(github_id: Optional[str]) -> str:
        if not github_id:
            return "community"
    ...
    def _to_meta(cls, entry: dict) -> SkillMeta:
        origin = cls._entry_github_identifier(entry)
        extra = dict(entry.get("extra", {}) or {})
        if entry.get("identifier"):
            extra.setdefault("index_identifier", entry.get("identifier"))
        if entry.get("source"):
            extra.setdefault("index_source", entry.get("source"))
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source="github" if origin else "hermes-index",
            identifier=entry.get("identifier", ""),
            trust_level=cls._trust_level_for_github_origin(origin),
            repo=entry.get("repo"),
            path=entry.get("path"),
            tags=entry.get("tags", []),
            extra=extra,
