...
class FileSyncManager:
    ...
    def __init__(
        ...
        allow_inferred_sync_back: bool = True,
    ):
        ...
        # Some cloud backends sync from attacker-influenced sandboxes.
        # For those, sync_back must only apply files with explicit host
        # mappings and must not infer new sibling paths from remote tar data.
        self._allow_inferred_sync_back = allow_inferred_sync_back
            ...
            with tempfile.TemporaryDirectory(prefix="hermes-sync-back-") as staging:
                ...
                for dirpath, _dirnames, filenames in os.walk(staging):
                    for fname in filenames:
                        ...
                        # Only apply changes to files that were explicitly
                        # pushed by this manager.  Do not infer destinations
                        # for attacker-created remote files: mappings can
                        # include sensitive root-level Hermes files, and a
                        # prefix match would let a sandbox write arbitrary
                        # siblings such as local config or skill files.
                        ...
                        # Resolve host path from cached mapping. If a path was
                        # pushed but is no longer in the current allowlist, do
                        # not write it back to the host.
                        host_path = self._resolve_host_path(remote_path, file_mapping)
                        if host_path is None and self._allow_inferred_sync_back:
                            host_path = self._infer_host_path(remote_path, file_mapping)
                        if host_path is None:
                            logger.debug(
                                "sync_back: skipping %s (no current host mapping)",
                                remote_path,
                            )
                            continue
