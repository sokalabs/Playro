...
def _read_terminal_shell_init_config() -> tuple[list[str], bool]:
    ...
    Best-effort — returns secure defaults on any failure so terminal
    execution never sources user startup files implicitly.
    """
    try:
        ...
        if not isinstance(files, list):
            files = []
        auto_bashrc = bool(terminal_cfg.get("auto_source_bashrc", False))
...
def _resolve_shell_init_files() -> list[str]:
    ...
    The opt-in ``auto_source_bashrc`` path runs only when the user hasn't
    supplied an explicit list — once they have, Hermes trusts them.
    """
