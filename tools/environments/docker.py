...
def _redact_docker_run_args_for_log(run_args: list[str]) -> list[str]:
    """Return docker run args safe for INFO logs (hide ``-e`` values)."""
    redacted: list[str] = []
    index = 0
    while index < len(run_args):
        token = run_args[index]
        if token == "-e" and index + 1 < len(run_args):
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        if self._persistent:
            ...
            safe_task_id = sanitize_sandbox_task_id(task_id)
            sandbox = get_sandbox_dir() / "docker" / safe_task_id
        ...
        try:
            ...
            # Mount only request-scoped cache files. Gateway caches are shared
            # across users/sessions, so whole cache directories must never be
            # exposed to remote sandboxes.
            for cache_file in iter_cache_files():
                volume_args.extend([
                    "-v",
                    f"{cache_file['host_path']}:{cache_file['container_path']}:ro",
                ])
                logger.info(
                    "Docker: mounting cache file %s -> %s",
                    cache_file["host_path"],
                    cache_file["container_path"],
                )
        ...
        logger.info("Docker run_args: %s", _redact_docker_run_args_for_log(all_run_args))
        ...
        if self._container_id:
            ...
            def _stop_or_remove() -> None:
                try:
                    ...
                    logger.debug("Docker stop failed; forcing remove", exc_info=True)
                    try:
                        subprocess.run(
                            [docker_exe, "rm", "-f", container_id],
                            stdout=subprocess.DEVNULL,
                        ...
                        logger.debug("Docker forced remove failed", exc_info=True)

            threading.Thread(target=_stop_or_remove, daemon=True).start()
            ...
            if not self._persistent:
                ...
                def _deferred_remove() -> None:
                    try:
                        ...
                        logger.debug("Docker deferred remove failed", exc_info=True)
