...
Use only pinned, verified blogwatcher-cli artifacts. Do not install from `latest` URLs, do not pipe network downloads directly into `tar`, and do not run container tags without an immutable digest.

Pick one method:

- **Go:** install a specific reviewed version, for example `go install github.com/JulienTant/blogwatcher-cli/cmd/blogwatcher-cli@vX.Y.Z`.
- **Docker:** run an immutable image reference, for example `ghcr.io/julientant/blogwatcher-cli@sha256:<reviewed-image-digest>`.
- **Binary archive:** download the archive for a specific release version, verify its SHA256 checksum or signature against a trusted release manifest, extract it into a temporary directory, inspect permissions, then move only the expected `blogwatcher-cli` executable onto `PATH`.

All releases: https://github.com/JulienTant/blogwatcher-cli/releases

Safe binary installation checklist:

1. Choose and record the exact release version and platform archive name.
2. Download the release archive and its checksum or signature files as separate files.
3. Verify the archive before extraction, for example with `sha256sum -c` or the project's documented signature-verification command.
4. Extract into a new temporary directory rather than directly into `/usr/local/bin`.
5. Install only the verified `blogwatcher-cli` executable.

### Docker with persistent storage

By default the database lives at `~/.blogwatcher-cli/blogwatcher-cli.db`. In Docker this is lost on container restart. Use `BLOGWATCHER_DB` or a volume mount to persist it, and keep the image pinned by digest:

```bash
BLOGWATCHER_IMAGE='ghcr.io/julientant/blogwatcher-cli@sha256:<reviewed-image-digest>'

# Named volume (simplest)
docker run --rm -v blogwatcher-cli:/data -e BLOGWATCHER_DB=/data/blogwatcher-cli.db "$BLOGWATCHER_IMAGE" scan

# Host bind mount; grant write access only to a dedicated blogwatcher data directory.
docker run --rm -v /path/to/dedicated/blogwatcher-data:/data -e BLOGWATCHER_DB=/data/blogwatcher-cli.db "$BLOGWATCHER_IMAGE" scan
```
