...
**Print mode avoids interactive dialogs** — use it with explicit `--allowedTools`, `--disallowedTools`, `--max-turns`, and budget limits so automation stays constrained.
...
Claude Code may present a workspace trust dialog on first launch. You may handle this via tmux only when the target directory is the expected project workspace:
...
❯ 1. Yes, I trust this folder    ← DEFAULT (just press Enter)
  2. No, exit
```
**Handling:** `tmux send-keys -t <session> Enter` — only after confirming the tmux session is in the intended trusted workspace.

### Permission Bypass Warning

Do **not** launch Claude Code with `--dangerously-skip-permissions`, `--permission-mode bypassPermissions`, or equivalent bypass modes from Hermes automation. Do **not** send keystrokes that accept a permission-bypass warning. Permission prompts must remain visible to the user or be avoided by using safer, explicit tool allowlists.

### Robust Dialog Handling Pattern
```
# Launch without permission bypass and constrain tools to the task
terminal(command="tmux send-keys -t claude-work 'cd /path/to/project && claude --allowedTools \"Read,Edit\"' Enter")

# Handle trust dialog only after confirming the project directory is expected
terminal(command="sleep 4 && tmux send-keys -t claude-work Enter")

# Monitor for progress or permission prompts; do not auto-accept permission bypass prompts
terminal(command="sleep 15 && tmux capture-pane -t claude-work -p -S -60")
```

**Note:** After the first trust acceptance for a directory, the trust dialog won't appear again. Permission bypass dialogs must not be automated.
...
| Flag | Effect |
|------|--------|
| `--dangerously-skip-permissions` | Auto-approves ALL tool use; do not use or automate from Hermes |
| `--allow-dangerously-skip-permissions` | Enables bypass as an option; avoid in Hermes automation |
| `--permission-mode <mode>` | `default`, `acceptEdits`, `plan`, `auto`, `dontAsk`; avoid `bypassPermissions` in Hermes automation |
...
- `⏵⏵ bypass permissions on` = status bar showing an unsafe bypass mode; stop and ask the user to restart without bypass
...
2. **`--dangerously-skip-permissions` bypasses safety controls** — do not launch it from Hermes automation or send keystrokes to accept its warning. Use explicit `--allowedTools` instead.
