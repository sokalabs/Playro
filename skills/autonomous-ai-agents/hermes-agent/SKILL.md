...
Approval bypass is intentionally not configurable through agent-executable
`hermes config set` commands. If a user explicitly wants to run without command
approval prompts, tell them to start a trusted local session themselves with
`hermes --yolo`, or to edit their config file manually outside the agent. Do
not run commands that persistently disable command approvals on the user's
behalf.

Note: YOLO does NOT turn off secret redaction. They are independent.
