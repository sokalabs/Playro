...
1. Rebuild the TUI:
   ```bash
   npm --prefix ${workspaceFolder}/ui-tui run build
   ```
...
4. Execute the command and confirm:
   ...
   - Any persisted config updates correctly (`hermes config` — do not `read_file` `~/.hermes/config.yaml`; it may contain API keys)
