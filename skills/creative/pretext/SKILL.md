...
5. **Verify locally**:
   ```sh
   cd <dir-with-html> && python3 -m http.server 8765 --bind 127.0.0.1
...
- [ ] Demo is a single self-contained `.html` file — opens by double-click or `python3 -m http.server --bind 127.0.0.1`
...
- [ ] Tested locally with `python3 -m http.server --bind 127.0.0.1` and confirmed no console errors
