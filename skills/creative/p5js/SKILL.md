...
| **High-res PNG** | Sandboxed Puppeteer capture over localhost | `node scripts/export-frames.js sketch.html --width 3840 --height 2160 --frames 1` |
...
| **MP4** | Localhost Puppeteer frame capture + ffmpeg | `bash scripts/render.sh sketch.html output.mp4 --duration 30 --fps 30` |
...
Security note: the headless exporter serves the sketch directory from `127.0.0.1`, keeps Chromium web security enabled, and blocks external network requests by default. Use `--allow-origin https://trusted.example` only for trusted CDN/assets needed by a known-safe sketch.
...
3. **Local assets** (fonts, images) require a server: `python3 -m http.server 8080 --bind 127.0.0.1` in the project directory, then open `http://127.0.0.1:8080/sketch.html`
