 ...
 *   --allow-origin <origin>  Trusted external origin to allow during export (repeatable)
 *
 * Examples:
...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i].startsWith('--')) {
      ...
      const val = args[i + 1];
      if ((key === 'allow-origin' || key === 'allowOrigin') && val !== undefined) {
        opts.allowOrigin.push(new URL(val).origin);
        i++;
      } else if (key in opts && val !== undefined) {
...
function serveSketchDirectory(rootDir) {
...
async function restrictRequests(page, allowedOrigins) {
...
async function main() {
  ...
  const { server, origin } = await serveSketchDirectory(rootDir);
  const sketchUrl = `${origin}/${encodeURIComponent(path.basename(inputPath))}`;
  let browser;
  ...
  try {
    ...
    await restrictRequests(page, new Set([origin, ...opts.allowOrigin]));
    ...
    // Serve the sketch from a localhost root instead of file:// so sketch
    // JavaScript cannot read arbitrary local files via browser file APIs.
