...
def redact_secrets_for_log(text: str) -> str:
    """Redact API keys and token query params before writing to logs."""
    redacted = re.sub(
        r"([?&]token=)[^&\s\"']+",
        r"\1<redacted>",
        text,
    ...
    redacted = re.sub(
        r"((?:--api-key|api_key|api-key|Authorization)\s*[=:]\s*)(\S+)",
        r"\1<redacted>",
        redacted,
        flags=re.IGNORECASE,
    )
...
def log(msg: str) -> None:
    ...
    print(f"[comfyui-skill] {redact_secrets_for_log(msg)}", file=sys.stderr)
