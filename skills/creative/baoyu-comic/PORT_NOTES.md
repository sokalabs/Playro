...
- **Download step** — after every `image_generate` call, the returned URL is fetched with `python -m tools.safe_image_download` (no shell-interpolated `curl`/`wget`) and verified before the workflow advances.
