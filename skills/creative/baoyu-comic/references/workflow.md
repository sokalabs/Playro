...
2. Write the URL to a temporary text file and download it with the project-local safe downloader, e.g. `python -m tools.safe_image_download --url-file /abs/path/to/url.txt --output /abs/path/to/comic/{slug}/<target>.png`.
3. Do **not** paste raw provider-returned URLs into shell `curl`/`wget` commands. Double quotes do not stop shell command substitution, and non-HTTPS/internal URLs must be rejected.
4. Verify the file is non-empty (`test -s /abs/path/to/comic/{slug}/<target>.png`); on failure, retry the generation once.
...
4. Download the returned URL with `python -m tools.safe_image_download` → save to `characters/characters.png`
...
4. Download the returned URL with `python -m tools.safe_image_download` → save to `NN-{cover|page}-[slug].png`
