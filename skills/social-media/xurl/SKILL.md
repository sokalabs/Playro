...
Installation must be performed by the user manually, outside the agent session. Do not execute installer commands for the user, and do not recommend curl-piped-to-shell installers or mutable `latest` / branch-based installs.

Recommended safe flow:

1. Open the official xurl repository or package registry in a browser.
2. Select a specific released version.
3. Review the release notes and installation artifact or package metadata.
4. Verify any checksum or signature published for that release.
5. Install that exact reviewed version manually on the user's machine.

After the user installs `xurl`, the agent may verify only the installed binary and auth status:
