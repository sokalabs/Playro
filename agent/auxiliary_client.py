...
def _is_azure_endpoint_url(base_url: str) -> bool:
    """True when *base_url* targets Azure AI (hostname match, not substring)."""
    return (
        base_url_host_matches(base_url, "azure.com")
...
def _recoverable_pool_provider(resolved_provider: str, client: Any) -> Optional[str]:
    ...
    if base_url_host_matches(base, "api.githubcopilot.com"):
        return "copilot"
    if base_url_host_matches(base, "api.moonshot.cn"):
        return "kimi-coding-cn"
    if base_url_host_matches(base, "api.moonshot.ai"):
        return "kimi-coding"
...
def resolve_provider_client(
    ...
    if pconfig.auth_type == "api_key":
        if provider == "anthropic":
            ...
            if _eff_base and _is_azure_endpoint_url(_eff_base):
                _azure_key = (
                    (explicit_api_key or "").strip()
                ...
                if not _azure_key:
                    logger.warning(
                        "resolve_provider_client: Azure Anthropic endpoint configured "
                        "but no AZURE_ANTHROPIC_KEY / ANTHROPIC_API_KEY found"
                    )
                    return None, None
                final_model = _normalize_resolved_model(
                ...
                try:
                    ...
                    real_client = build_anthropic_client(_azure_key, _eff_base.rstrip("/"))
                except ImportError:
                    return None, None
                sync_anthropic = AnthropicAuxiliaryClient(
                    real_client, final_model, _azure_key, _eff_base, is_oauth=False,
                )
                if async_mode:
                    return AsyncAnthropicAuxiliaryClient(sync_anthropic), final_model
                return sync_anthropic, final_model
