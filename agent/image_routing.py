...
def build_native_content_parts(
    ...
    When images attach successfully, the text part notes how many images are
    present without embedding absolute local paths (which would leak usernames,
    ``HERMES_HOME`` layout, and project directory names to the model provider).
    Pixels are still sent via ``image_url`` data URLs for native vision.  For a
    closer look at a specific file, the model can use ``vision_analyze`` with a
    path the user supplied in the current turn.
    ...
    if attached_paths:
        ...
        count = len(attached_paths)
        hint = (
            f"[{count} image{'s' if count != 1 else ''} attached natively for vision]"
        )
