...
class ShellHookSpec:
    ...
    approved_argv: Optional[List[str]] = field(default=None, repr=False)
    ...
    *,
    ...
    for spec in specs:
        ...
        with _registered_lock:
            if key in _registered:
                continue
            spec.approved_argv = _resolve_argv_for_execution(spec.command)
...
def _spawn(spec: ShellHookSpec, stdin_json: str) -> Dict[str, Any]:
    ...
    try:
        argv = (
            list(spec.approved_argv)
            if spec.approved_argv
            else shlex.split(os.path.expanduser(spec.command))
        )
...
def _approval_identity(command: str) -> Dict[str, Optional[str]]:
    """Return the command identity that consent is bound to.

    The raw command string alone is not enough for relative hooks: the same
    ``scripts/hook.sh`` command can point at different files as Hermes moves
    between project directories.  Store and re-check the resolved script path
    plus content hash so approval follows the file the user reviewed, not an
    attacker-controlled path with the same spelling.
    """
    resolved_path = _resolved_script_path(command)
    return {
        "resolved_script_path_at_approval": resolved_path,
        "script_mtime_at_approval": _path_mtime_iso(resolved_path),
        "script_sha256_at_approval": _path_sha256(resolved_path),
    }


def _resolved_script_path(command: str) -> Optional[str]:
    ...
    if (
        ...
        or (os.altsep and os.altsep in expanded)
    ):
...
def _path_mtime_iso(path: Optional[str]) -> Optional[str]:
    if not path:
        return None
    try:
...
def _path_sha256(path: Optional[str]) -> Optional[str]:
...
def _resolve_argv_for_execution(command: str) -> Optional[List[str]]:
    try:
    ...
    for index, part in enumerate(argv):
        if (
            part == script_path
            or os.path.expanduser(part) == os.path.expanduser(script_path)
        ):
            resolved = _resolved_script_path(command)
            if resolved:
                argv[index] = resolved
            break
...
def _is_allowlisted(event: str, command: str) -> bool:
    data = load_allowlist()
    current = _approval_identity(command)
    current_path = current.get("resolved_script_path_at_approval")
    current_hash = current.get("script_sha256_at_approval")
    current_mtime = current.get("script_mtime_at_approval")
    ...
    for e in data.get("approvals", []):
        ...
        approved_path = e.get("resolved_script_path_at_approval")
        if not approved_path or approved_path != current_path:
            continue

        approved_hash = e.get("script_sha256_at_approval")
        if approved_hash is not None or current_hash is not None:
            if approved_hash != current_hash:
                continue
        elif e.get("script_mtime_at_approval") != current_mtime:
            continue
...
def script_mtime_iso(command: str) -> Optional[str]:
    ...
    script is missing."""
    return _path_mtime_iso(_resolved_script_path(command))
