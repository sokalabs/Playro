...
def _align_compressor_provider(base_url: str, provider: str) -> str:
    """Keep Kimi CN credentials on ``kimi-coding-cn`` when the base URL is CN."""
    if base_url_host_matches(base_url, "api.moonshot.cn"):
        return "kimi-coding-cn"
    if base_url_host_matches(base_url, "api.moonshot.ai"):
        return "kimi-coding"
    return provider
...
class ContextCompressor(ContextEngine):
    ...
    def update_model(
        ...
        self.provider = _align_compressor_provider(base_url, provider)
    ...
    def __init__(
        ...
        self.base_url = base_url
        self.api_key = api_key
        aligned_provider = _align_compressor_provider(base_url, provider)
        self.provider = aligned_provider
        self.api_mode = api_mode
        ...
        if not quiet_mode:
            logger.info(
                ...
                aligned_provider or "none", base_url or "none",
            )
