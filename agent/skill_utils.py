...
def resolve_skill_config_values(
    ...
    for var in config_vars:
        ...
        used_default = value is None or (isinstance(value, str) and not value.strip())
        if used_default:
            value = var.get("default", "")

        if isinstance(value, str):
            # Defaults are supplied by skill metadata and may come from
            # untrusted third-party skills.  Never expand environment
            # variables in defaults: doing so can turn a malicious default like
            # ``${OPENAI_API_KEY}`` into a secret that is later displayed or
            # injected into model-visible skill context.  User-authored values
            # from config.yaml keep the previous env-var expansion behavior.
            value = os.path.expanduser(value)
            if not used_default and "${" in value:
...
def extract_skill_description(frontmatter: Dict[str, Any]) -> str:
    """Extract a prompt-safe, truncated description from parsed frontmatter."""
    raw_desc = frontmatter.get("description", "")
    if not raw_desc:
        return ""
    desc = str(raw_desc).strip().strip("'\"")
    # Skill descriptions are later embedded in system prompts and reload
    # notes. Collapse all whitespace so YAML block scalars or instruction-like
    # multi-line text cannot break out of the intended single-line catalog
    # entry before the existing 60-character budget is applied.
