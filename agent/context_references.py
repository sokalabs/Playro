...
_SENSITIVE_PATH_PARTS = frozenset(_SENSITIVE_HOME_DIRS)
...
async def preprocess_context_references_async(
    ...
    *,
    ...
    # Bound file reads before loading content into memory. The rough token
    # estimator uses ~4 chars/token, so this keeps individual @file reads
    # from exceeding the hard context budget before final formatting overhead.
    max_file_chars = max(1, hard_limit * 4)
    ...
    try:
        if ref.kind == "file":
            return _expand_file_reference(
                ref, cwd, allowed_root=allowed_root, max_file_chars=max_file_chars
            )
        if ref.kind == "folder":
...
def _expand_file_reference(
    ...
    *,
    ...
    text = _read_file_reference_text(path, ref, max_chars=max_file_chars)
...
def _read_file_reference_text(
    path: Path, ref: ContextReference, *, max_chars: int | None = None
) -> str:
    ...
    if ref.line_start is None:
        ...
        if size > max_chars:
            raise ValueError(
                f"file is too large to attach safely ({size} bytes; limit {max_chars} bytes). "
                "Use a line range to attach a smaller section."
            )
        text = path.read_text(encoding="utf-8")
        if len(text) > max_chars:
            raise ValueError(
                f"file is too large to attach safely ({len(text)} characters; limit {max_chars}). "
                "Use a line range to attach a smaller section."
            )
        return text

    start_line = max(ref.line_start, 1)
    end_line = max(ref.line_end or ref.line_start, start_line)
    selected: list[str] = []
    char_count = 0
    with path.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            if line_number > end_line:
                break
            line = line.rstrip("\n")
            char_count += len(line) + (1 if selected else 0)
            if char_count > max_chars:
                raise ValueError(
                    f"selected line range is too large to attach safely (limit {max_chars} characters)."
                )
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    if (
        ...
        or path.name.lower() in _SENSITIVE_FILE_NAMES
