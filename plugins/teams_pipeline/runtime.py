...
from plugins.teams_pipeline.subscriptions import build_graph_client, expected_client_state
...
def _adapter_expected_client_state(adapter: Any) -> str | None:
    """Return the secret that the running webhook adapter enforces."""
    configured = getattr(adapter, "_client_state", None)
    if configured:
        return str(configured).strip()
    adapter_config = getattr(adapter, "config", None)
    extra = getattr(adapter_config, "extra", None) if adapter_config is not None else None
    if isinstance(extra, dict):
        return expected_client_state(extra.get("client_state"))
    return None


def _install_drop_scheduler(adapter: Any, reason: str) -> None:
    async def _drop(notification: dict[str, Any], event: Any) -> None:
        logger.debug(
            "Dropping Graph notification for Teams pipeline: %s id=%s subscription=%s resource=%s",
            reason,
            notification.get("id"),
...
def _managed_subscription_record(
    runtime: TeamsMeetingPipeline,
    notification: dict[str, Any],
) -> dict[str, Any] | None:
    subscription_id = str(notification.get("subscriptionId") or "").strip()
    if not subscription_id:
        return None
    record = runtime.store.get_subscription(subscription_id)
...
def bind_gateway_runtime(gateway: Any) -> bool:
    ...
    adapter_client_state = _adapter_expected_client_state(adapter)
    if not adapter_client_state:
        reason = (
            "msgraph_webhook client_state is required before binding "
            "Teams transcript processing"
        )
        gateway._teams_pipeline_runtime_error = reason
        logger.warning("Teams pipeline runtime not bound: %s", reason)
        _install_drop_scheduler(adapter, reason)
        return False
    ...
    try:
        ...
        _install_drop_scheduler(adapter, "runtime unavailable")
        return False

    async def _schedule(notification: dict[str, Any], event: Any) -> None:
        record = _managed_subscription_record(runtime, notification)
        if record is None:
            logger.warning(
                "Dropping unmanaged Teams pipeline Graph notification: "
                "id=%s subscription=%s resource=%s",
                notification.get("id"),
                notification.get("subscriptionId"),
                notification.get("resource"),
            )
