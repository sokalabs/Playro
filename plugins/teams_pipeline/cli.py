...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    sub_p.add_argument("--client-state", default=os.getenv("MSGRAPH_WEBHOOK_CLIENT_STATE", ""))
...
def _validate_configuration_snapshot(store: TeamsPipelineStore) -> dict[str, Any]:
    ...
    if not webhook_enabled:
        ...
        webhook_extra = dict(webhook_config.extra or {}) if webhook_config else {}
        if not expected_client_state(webhook_extra.get("client_state")):
            issues.append(
                "MSGRAPH_WEBHOOK_CLIENT_STATE is required for Teams pipeline "
...
def _cmd_subscribe(args) -> None:
    ...
    client_state = expected_client_state(str(getattr(args, "client_state", "") or "")) or ""
    ...
    if not client_state:
        raise ValueError(
            "--client-state or MSGRAPH_WEBHOOK_CLIENT_STATE is required "
            "for Teams pipeline subscriptions."
        )
    ...
    _sync_subscription_record(
        store,
        {**(result or {}), "clientState": client_state},
        status="active",
    )
