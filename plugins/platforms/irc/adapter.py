...
# IRC 512-byte line limit; PRIVMSG needs room for ``PRIVMSG <target> :<payload>\r\n``.
IRC_MAX_TARGET_BYTES = 300


def _irc_privmsg_max_content_bytes(target: str) -> int:
    overhead = len(f"PRIVMSG {target} :".encode("utf-8")) + 2
    return 510 - overhead


def _validate_irc_delivery_target(target: str) -> Optional[str]:
    """Return an error message when *target* cannot be used in PRIVMSG."""
    if any(ch in target for ch in ("\r", "\n", "\x00", " ")):
        return "IRC delivery target contains illegal IRC characters"
    if len(target.encode("utf-8")) > IRC_MAX_TARGET_BYTES:
        return "IRC delivery target exceeds maximum length"
    if _irc_privmsg_max_content_bytes(target) < 1:
        return "IRC delivery target too long for IRC PRIVMSG"
    return None
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def _split_message(self, content: str, target: str) -> List[str]:
        ...
        if max_bytes < 1:
            logger.warning("IRC target too long for PRIVMSG: %r", target)
            return []
...
async def _standalone_send(
    ...
    *,
    ...
    target_err = _validate_irc_delivery_target(raw_target)
    if target_err:
        return {"error": f"IRC standalone send: {target_err}"}
    target = raw_target
    ...
    try:
        ...
        max_bytes = 510 - overhead
        if max_bytes < 1:
            return {"error": "IRC standalone send: delivery target too long for IRC PRIVMSG"}
        sent_any = False
        for paragraph in plain.split("\n"):
            ...
            while paragraph:
                ...
                split_at = best
                if split_at < 1:
                    return {"error": "IRC standalone send: message chunking failed (target too long)"}
                space = paragraph.rfind(" ", 0, split_at)
                if space > split_at // 3:
                    split_at = space
