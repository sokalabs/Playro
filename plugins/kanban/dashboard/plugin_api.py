...
Security note
-------------
Kanban HTTP routes are mounted under ``/api/plugins/kanban/`` and rely on
the dashboard's global HTTP auth middleware (``web_server.auth_middleware``)
for the ephemeral session token, just like other sensitive ``/api/``
endpoints. The WebSocket also requires the session token as a ``?token=``
query parameter because browsers cannot set the ``Authorization`` header on
an upgrade request, matching the established pattern used by the in-browser
PTY bridge in ``hermes_cli/web_server.py``.
"""
...
# ---------------------------------------------------------------------------
# Auth helper for the WebSocket query-param token path. HTTP routes are
# protected by the dashboard's global auth middleware.
# ---------------------------------------------------------------------------
