...
_FILE_UPLOAD_MAX_BYTES_DEFAULT = 10 * 1024 * 1024
...
_SENSITIVE_UPLOAD_SUFFIXES = (".key", ".pem", ".p12", ".pfx")
...
def _safe_upload_root() -> Path | None:
    root = os.environ.get("RETAINDB_FILE_UPLOAD_ROOT", "").strip()
    if not root:
        return None
    return Path(root).expanduser().resolve(strict=True)


def _file_upload_max_bytes() -> int:
    raw = os.environ.get("RETAINDB_FILE_UPLOAD_MAX_BYTES", "").strip()
    if not raw:
        return _FILE_UPLOAD_MAX_BYTES_DEFAULT
    try:
...
def _is_sensitive_upload_name(path: Path) -> bool:
    name = path.name.lower()
    return name in _SENSITIVE_UPLOAD_NAMES or name.endswith(_SENSITIVE_UPLOAD_SUFFIXES)


def _validate_local_upload_path(local_path: str) -> tuple[Path | None, str | None]:
    """Return a safe regular file path under RETAINDB_FILE_UPLOAD_ROOT."""
    try:
        root = _safe_upload_root()
    except OSError as exc:
        return None, f"RetainDB file upload root is invalid: {exc}"
    if root is None:
        return None, "RetainDB file uploads require RETAINDB_FILE_UPLOAD_ROOT to be set"
    ...
    try:
        ...
        return None, f"Unable to resolve file: {exc}"

    try:
        candidate_resolved.relative_to(root)
    except ValueError:
        return None, f"File must be under RETAINDB_FILE_UPLOAD_ROOT: {root}"
    ...
    try:
        for part in candidate_abs.relative_to(root).parts:
            ...
            if current.is_symlink():
                return None, "Symlinks are not allowed for RetainDB file uploads"

        st = candidate_abs.stat(follow_symlinks=False)
    except FileNotFoundError:
        return None, f"File not found: {local_path}"
    except OSError as exc:
    ...
    if not stat.S_ISREG(st.st_mode):
        return None, "RetainDB file uploads only allow regular files"

    if _is_sensitive_upload_name(candidate_abs) and not _env_truthy("RETAINDB_ALLOW_SENSITIVE_FILE_UPLOADS"):
        return None, "Sensitive filenames require explicit RETAINDB_ALLOW_SENSITIVE_FILE_UPLOADS=1"

    max_bytes = _file_upload_max_bytes()
    if st.st_size > max_bytes:
        return None, f"File exceeds RetainDB upload size limit of {max_bytes} bytes"
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        return [
            ...
            {"key": "file_tools_enabled", "description": "Expose RetainDB shared file-store tools (default: disabled)", "default": False, "env_var": "RETAINDB_FILE_TOOLS_ENABLED"},
            {"key": "file_upload_root", "description": "Required safe local root for retaindb_upload_file", "default": "", "env_var": "RETAINDB_FILE_UPLOAD_ROOT"},
            {"key": "file_upload_max_bytes", "description": "Maximum bytes read by retaindb_upload_file", "default": _FILE_UPLOAD_MAX_BYTES_DEFAULT, "env_var": "RETAINDB_FILE_UPLOAD_MAX_BYTES"},
        ]
    ...
    def system_prompt_block(self) -> str:
        ...
        if self._file_tools_enabled():
            block += " RetainDB file tools are explicitly enabled for shared file-store operations."
        return block
    ...
    def get_tool_schemas(self) -> List[Dict[str, Any]]:
        ...
        if self._file_tools_enabled():
            schemas.extend(_FILE_TOOL_SCHEMAS)
        return schemas

    @staticmethod
    def _file_tools_enabled() -> bool:
        return _env_truthy("RETAINDB_FILE_TOOLS_ENABLED")
    ...
    def _dispatch(self, tool_name: str, args: dict) -> Any:
        ...
        if tool_name in _FILE_TOOL_NAMES and not self._file_tools_enabled():
            return {"error": "RetainDB file tools are disabled; set RETAINDB_FILE_TOOLS_ENABLED=1 to enable them"}
