...
class HonchoSessionManager:
    ...
    def dialectic_query(
        ...
        Args:
            ...
            peer: Which session peer to query — "user" (default) or "ai".
    ...
    def _resolve_peer_id(self, session: HonchoSession, peer: str | None) -> str | None:
        """Resolve a peer selector to a peer ID authorized for this session.

        Honcho tools run in end-user conversations, so caller-supplied peer
        selectors must not be allowed to name arbitrary workspace peers. Only
        the current session's user/assistant aliases or their exact sanitized
        IDs are accepted.
        """
        ...
        logger.warning(
            "Rejected unauthorized Honcho peer selector '%s' for session '%s'",
            peer,
            session.key,
        )
        return None
    ...
    def get_peer_card(self, session_key: str, peer: str = "user") -> list[str]:
        ...
        try:
            resolved = self._resolve_observer_target(session, peer)
            if resolved is None:
                return []
            observer_peer_id, target_peer_id = resolved
    ...
    def search_context(
        ...
        Args:
            ...
            peer: Session peer alias or current session peer ID to search about.
        ...
        try:
            ...
            if resolved is None:
                return ""
            observer_peer_id, target = resolved
    ...
    def create_conclusion(self, session_key: str, content: str, peer: str = "user") -> bool:
        ...
        Args:
            ...
            peer: Session peer alias or current session peer ID. "user" is the default alias.
    ...
    def delete_conclusion(self, session_key: str, conclusion_id: str, peer: str = "user") -> bool:
        ...
        Args:
            ...
            peer: Session peer alias or current session peer ID.
