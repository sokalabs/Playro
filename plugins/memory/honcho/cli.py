...
def clone_honcho_for_profile(profile_name: str) -> bool:
    ...
    # AI peer is profile-specific. Keep the workspace profile-scoped by
    # default so Honcho memory and sessions preserve Hermes profile isolation.
...
def cmd_enable(args) -> None:
    ...
    if not block.get("aiPeer"):
        ...
        # New profile blocks must not inherit the default/root workspace by
        # default; that would make separate Hermes profiles share Honcho
        # sessions and memory. Explicit existing workspace values are still
