...
PROFILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            "peer": {
                "type": "string",
                "description": "Peer to query. Allowed values: 'user' (default) or 'ai'.",
            },
            "card": {
                "type": "array",
                "items": {"type": "string"},
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        platform = (kwargs.get("platform") or "cli").lower()
        if not gateway_session_key and platform != "cli" and session_id:
            # Gateway/API callers already pass a unique Hermes session_id.  Do
            # not let cwd-based default strategies collapse distinct remote
            # users/chats that happen to share the same server working directory.
