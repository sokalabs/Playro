...
_LOCAL_UPLOAD_ROOT_ENV = "OPENVIKING_LOCAL_UPLOAD_ROOT"
_MAX_LOCAL_UPLOAD_FILE_BYTES = 10 * 1024 * 1024
_MAX_LOCAL_UPLOAD_TOTAL_BYTES = 25 * 1024 * 1024
_MAX_LOCAL_UPLOAD_FILES = 1000
_SENSITIVE_LOCAL_UPLOAD_NAMES = {
    ".env",
    ".netrc",
    "credentials",
    "id_dsa",
...
_SENSITIVE_LOCAL_UPLOAD_SUFFIXES = (".key", ".pem", ".p12", ".pfx")
...
class _VikingClient:
    ...
    def _headers(self) -> dict:
        # Send tenant headers only when explicitly configured. Literal
        # schema defaults ("default"/"default") must not override user-scoped
        # API key tenancy on the server.
        h = {
...
ADD_RESOURCE_SCHEMA = {
    ...
    "parameters": {
        "type": "object",
        "properties": {
            "url": {"type": "string", "description": "Public http(s), git, or ssh URL to add."},
            "reason": {
                "type": "string",
...
def _configured_local_upload_root() -> Optional[Path]:
    root = os.environ.get(_LOCAL_UPLOAD_ROOT_ENV, "").strip()
    if not root:
        return None
    return Path(root).expanduser().resolve()


def _is_sensitive_local_upload_path(path: Path) -> bool:
    for part in path.parts:
        lowered = part.lower()
        if lowered in _SENSITIVE_LOCAL_UPLOAD_NAMES:
            return True
        if lowered.endswith(_SENSITIVE_LOCAL_UPLOAD_SUFFIXES):
            return True
    return False


def _validate_local_upload_path(source_path: Path) -> Path | str:
    upload_root = _configured_local_upload_root()
    if upload_root is None:
        return (
            f"Local file and directory uploads are disabled for viking_add_resource. "
            f"Set {_LOCAL_UPLOAD_ROOT_ENV} to an approved workspace and provide a "
            "path inside it if local ingestion is required."
        )

    if source_path.is_symlink():
        return "Local resource path must not be a symbolic link."

    try:
        resolved = source_path.resolve(strict=True)
    except OSError as exc:
        return f"Local resource path does not exist or cannot be resolved: {source_path} ({exc})"

    if not resolved.is_relative_to(upload_root):
        return f"Local resource path must be inside {_LOCAL_UPLOAD_ROOT_ENV}: {upload_root}"
    if _is_sensitive_local_upload_path(resolved):
        return "Local resource path is blocked because it appears to contain secrets or credentials."
    return resolved


def _validated_local_upload_files(source_path: Path) -> List[Path] | str:
    resolved = _validate_local_upload_path(source_path)
    if isinstance(resolved, str):
        return resolved

    candidates = [resolved] if resolved.is_file() else list(resolved.rglob("*")) if resolved.is_dir() else []
    if not candidates:
        return f"Unsupported local resource path: {source_path}"
    ...
    for candidate in candidates:
        try:
            ...
            return f"Local resource path cannot be read: {candidate} ({exc})"
        if candidate.is_symlink():
            return f"Local resource path contains a symbolic link: {candidate}"
        if candidate.is_dir():
            continue
        if not candidate.is_file():
            continue
        if not candidate.resolve().is_relative_to(resolved if resolved.is_dir() else resolved.parent):
            return f"Local resource path escapes the approved upload tree: {candidate}"
        if _is_sensitive_local_upload_path(candidate):
            return "Local resource path is blocked because it appears to contain secrets or credentials."
        file_size = stat_result.st_size
        if file_size > _MAX_LOCAL_UPLOAD_FILE_BYTES:
            return f"Local resource file exceeds the {_MAX_LOCAL_UPLOAD_FILE_BYTES} byte limit: {candidate}"
        files.append(candidate)
        total_size += file_size
        if len(files) > _MAX_LOCAL_UPLOAD_FILES:
            return f"Local resource directory exceeds the {_MAX_LOCAL_UPLOAD_FILES} file limit."
        if total_size > _MAX_LOCAL_UPLOAD_TOTAL_BYTES:
            return f"Local resource upload exceeds the {_MAX_LOCAL_UPLOAD_TOTAL_BYTES} byte total limit."
    return files


def _zip_directory(dir_path: Path, files: List[Path]) -> Path:
    """Create a temporary zip file containing a validated directory tree."""
        ...
        try:
            if source_path is not None:
                if source_path.exists():
                    local_files = _validated_local_upload_files(source_path)
                    if isinstance(local_files, str):
                        return tool_error(local_files)

                    source_path = source_path.resolve()
                    payload["source_name"] = source_path.name
                    if source_path.is_dir():
                        cleanup_path = _zip_directory(source_path, local_files)
