...
def _scope_component(name: str, value: Any) -> str:
...
def _runtime_scope_key(session_id: str, runtime: dict) -> str:
    """Build a stable, non-PII namespace key from runtime isolation context."""
    platform = _scope_component("platform", runtime.get("platform"))
    principal_parts = [
        _scope_component("user", runtime.get("user_id")),
        _scope_component("identity", runtime.get("agent_identity")),
        _scope_component("workspace", runtime.get("agent_workspace")),
    ]
    parts = [platform, *principal_parts]
    if not any(principal_parts):
        parts.append(_scope_component("session", session_id))
    parts = [part for part in parts if part]
    if not parts:
        parts = [_scope_component("session", session_id) or "session=default"]
    return "|".join(parts)


def _scoped_container_tag(base_tag: str, scope_key: str) -> str:
    base = _sanitize_tag(base_tag)
    digest = hashlib.sha256(scope_key.encode("utf-8")).hexdigest()[:_SCOPE_DIGEST_LENGTH]
    return _sanitize_tag(f"{base}_scope_{digest}")
...
class SupermemoryMemoryProvider(MemoryProvider):
    def __init__(self):
        ...
        self._allowed_container_bases: List[str] = []
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Resolve container tag: env var > config > default. The configured tag
        # is only a base prefix; every Supermemory operation uses the runtime
        # scoped tag so gateway users/sessions cannot share one namespace.
        ...
        self._container_scope_key = _runtime_scope_key(session_id, kwargs)
        self._container_tag = _scoped_container_tag(self._base_container_tag, self._container_scope_key)
        ...
        # Multi-container setup. Custom tags are public aliases that resolve to
        # the same runtime scope as the primary tag. This preserves the allowlist
        # while preventing a model/tool call from escaping the user's namespace.
        ...
        self._allowed_container_bases = [self._base_container_tag] + list(self._custom_containers)
        self._allowed_containers = [
            _scoped_container_tag(tag, self._container_scope_key)
            for tag in self._allowed_container_bases
        ]
    ...
    def system_prompt_block(self) -> str:
        ...
        if self._enable_custom_containers and self._custom_containers:
            tags_str = ", ".join(self._allowed_container_bases)
    ...
    def _resolve_tool_container_tag(self, args: dict) -> Optional[str]:
        ...
        if sanitized not in self._allowed_container_bases:
            raise ValueError(
                f"Container tag '{sanitized}' is not allowed. "
                f"Allowed: {', '.join(self._allowed_container_bases)}"
            )
        return _scoped_container_tag(sanitized, self._container_scope_key)
    ...
    def get_tool_schemas(self) -> List[Dict[str, Any]]:
        ...
        container_param = {
            "type": "string",
            "description": f"Optional container tag. Allowed: {', '.join(self._allowed_container_bases)}. Defaults to primary ({self._base_container_tag}).",
        }
        schemas = []
