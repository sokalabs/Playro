...
def _quote_shell_arg(value: str) -> str:
    """Return a shell-safe representation of a single command argument."""
...
def load_cli_config() -> Dict[str, Any]:
    ...
    if "docker_env" in terminal_config:
        from tools.docker_env_store import write_docker_env

        write_docker_env(terminal_config.get("docker_env") or {})
...
class HermesCLI:
    ...
    def _handle_browser_command(self, cmd: str):
        """Handle /browser status and disable unsafe live Chrome CDP bridging."""
        ...
        if sub.startswith("connect"):
            ...
            print()
            print("⚠ /browser connect is disabled for safety")
            print("   Browser tools continue to use isolated local/headless or configured cloud sessions.")
            print("   Live Chrome CDP access can expose logged-in tabs and cookies to untrusted page content.")
            print()
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    "[System note: /browser connect was rejected for safety. Browser tools must remain "
                    "on isolated local/headless or managed cloud sessions, not the user's live Chrome profile.]"
                )
            return
            ...
            if current:
                ...
                try:
                    ...
                    pass
                print()
                print("🌐 Browser CDP override cleared")
                print("   Browser tools are using default isolated mode (headless local browser or cloud provider).")
                print()
                if hasattr(self, '_pending_input'):
                    self._pending_input.put(
                        "[System note: Browser tools are using default isolated mode "
                        "(headless local browser or cloud provider).]"
                    )
            else:
                print()
                print("Browser is already using default isolated mode")
                print()

        elif sub == "status":
            print()
            if current:
                print("🌐 Browser: default isolated mode (unsafe live Chrome CDP override ignored)")
            else:
                try:
                ...
                if provider is not None:
                    provider_name = getattr(provider, "provider_name", lambda: type(provider).__name__)()
                    print(f"🌐 Browser: {provider_name} (cloud)")
                else:
                    try:
            ...
            print()
            print("   /browser connect      — disabled to protect live browser sessions")
            print("   /browser disconnect   — clear any legacy CDP override")
            print("   /browser status       — show current browser mode")
            print()

        else:
            print()
            print("Usage: /browser disconnect|status")
            print()
            print("   connect      Disabled: live Chrome CDP access is unsafe")
            print("   disconnect   Clear any legacy CDP override")
    ...
    def _check_config_mcp_changes(self) -> None:
        """Detect mcp_servers changes in config.yaml and notify the user.
        ...
        known state.  When a change is detected, do not reload automatically:
        MCP stdio servers can launch configured commands, so an explicit
        user-run /reload-mcp remains required before new config is trusted.
        """
        ...
        # Notify only. Reloading MCP stdio servers can execute configured
        # commands, so require an explicit user /reload-mcp command rather
        # than trusting config edits detected by the idle watcher.
        print()
        print("⚠️  MCP server config changed. Run /reload-mcp to reload after reviewing it.")
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  hermes --resume {_quote_shell_arg(self.session_id)}")
            if session_title:
                print(f"  hermes -c {_quote_shell_arg(session_title)}")
            print()
    ...
    def run(self):
        ...
        # Config file watcher — detect mcp_servers changes and notify the user
        from hermes_cli.config import get_config_path as _get_config_path
            ...
            while not self._should_exit:
                try:
                    ...
                    try:
                        ...
                        # Periodic config watcher — notify on mcp_servers change
                        if not self._agent_running:
